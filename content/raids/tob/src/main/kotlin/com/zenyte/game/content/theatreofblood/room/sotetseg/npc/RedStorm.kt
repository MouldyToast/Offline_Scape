 ...
 * @author Jire
 */
internal class RedStorm(
    room: ShadowRealmRoom,
    private val mazeTiles: ObjectSet<Location>,
    val tile: Location
    ...
    override fun processNPC() {
        ...
        if (lastPath == null || lastPath?.matches(location) == true) {
            if (combat.target == null) {
                ...
                val next = transformCoord(combat.target.location)
                addWalkSteps(next.x, next.y, -1, false)
                lastPath = next
            }
        }
    ...
    private fun checkForPlayers(): Boolean {
        ...
        if (validTargets.isNotEmpty()) {
            validTargets.forEach {
                if (location.matches(it.location)) {
                    it.applyHit(Hit(Utils.random(35, 45), HitType.REGULAR))
                    hitPlayer = true
                }
            }
            resetWalkSteps()
