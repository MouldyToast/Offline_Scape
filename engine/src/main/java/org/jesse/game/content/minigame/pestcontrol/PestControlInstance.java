...
import static org.jesse.game.content.minigame.pestcontrol.PestControlUtilities.*;
...
public final class PestControlInstance extends DynamicArea implements DeathPlugin, CycleProcessPlugin, CannonRestrictionPlugin {
    private static final Logger log = NearRealityLogger.getLogger(PestControlInstance.class);
        ...
        WorldTasksManager.schedule(() -> {
            if (successful) {
                ...
                if (rank.equalToOrGreaterThan(MemberRank.DRAGONSTONE)) {
                    ...
                    points += 3;
                } else if (rank.equalToOrGreaterThan(MemberRank.TOPAZ)) {
                    points += 1;
                }
