...
public abstract class ContainerWrapper {
	...
	public long getAmountOf(final int... itemIds) {
        ...
        for (int itemId : itemIds) {
            amount += container.getAmountOf(itemId);
        }
		return amount;
	}
	...
	public boolean containsItems(final Collection<Item> items) {
        return items.stream().allMatch(item -> container.contains(item));
	}

	public boolean containsAll(final Collection<Integer> items) {
        return items.stream().allMatch(itemId -> containsItem(itemId, 1));
	}

	public boolean containsItems(final Item... item) {
        return Arrays.stream(item).allMatch(value -> container.contains(value));
	}
	...
	public void refresh(final int slot) {
		container.getModifiedSlots().add(slot);
		container.refresh(player);
	}
