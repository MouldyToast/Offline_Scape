...
public class StructDefinitions implements Definitions, Cloneable {
    ...
    @Override
    public void load() {
        // Because we have custom values ranging up to 30K and Jagex Cache 228 has 12,006
        final int highestFileId = CacheManager.getCache()
            .getArchive(ArchiveType.CONFIGS)
            .findGroupByID(GroupType.STRUCT)
            .getHighestFileId() + 18_000;
        ...
        definitions = new StructDefinitions[highestFileId];
        for (int id = 0; id < highestFileId; id++) {
