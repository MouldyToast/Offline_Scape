package org.jesse.cache_tool.packing
...
object JagexColor {
    ...
    fun packHSL(hue: Int, saturation: Int, luminance: Int): Short {
        return ((hue and 63).toShort().toInt() shl 10 or ((saturation and 7).toShort().toInt() shl 7
    ...
    fun rgbToHSL(rgb: Int, brightness: Double): Short {
        ...
        var r = (rgb shr 16 and 255).toDouble() / 256.0
        var g = (rgb shr 8 and 255).toDouble() / 256.0
        var b = (rgb and 255).toDouble() / 256.0
        ...
        val hsv = Color.RGBtoHSB((r * 256.0).toInt(), (g * 256.0).toInt(), (b * 256.0).toInt(), null)
        val hue = hsv[0].toDouble()
        val luminance = (hsv[2] - ((hsv[2] * hsv[1]) / 2f)).toDouble()
        val saturation = (hsv[2] - luminance) / min(luminance, 1 - luminance)

        return packHSL(
            (ceil(hue * 64.0) % 63.0).toInt(),
            ceil(saturation * 7.0).toInt(),
            ceil(luminance * 127.0).toInt()
        )
    }
}
