...
import org.jesse.game.content.boss.wildernessbosses.spiders.WebObject;
...
public class Spindel extends NPC implements CombatScript, Spawnable {
    ...
    private final Projectile RANGED_ATTACK_PROJ = new Projectile(2356, 172, 100, 30, 15, 10, 64, 0);
    private final Graphics RANGED_HIT_GFX = new Graphics(2357);
    private final Animation MAGIC_ATTACK_ANIM = new Animation(9990);
    private final Projectile MAGIC_ATTACK_PROJ = new Projectile(2358, 172, 100, 30, 15, 10, 64, 0);
    ...
    public int attack(Entity target) {
        ...
        if (target instanceof final Player player) {
            ...
            if (attacks == 0 || attacks % 4 == 0) {
                ...
                walkingTicks = WorldThread.getCurrentCycle() + getWalkSteps().size() - 1;
    ...
    @Override public void setFaceEntity(Entity entity) {
        if (walkingTicks <= WorldThread.getCurrentCycle()) {
            super.setFaceEntity(entity);
        }
    }

    @Override public boolean checkAggressivity() {
        return walkingTicks <= WorldThread.getCurrentCycle() && super.checkAggressivity();
    }
