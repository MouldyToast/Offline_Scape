...
public class Location extends _Location implements Position {
    ...
    public Location transformAngle(int jagAngle, int distance) {
        double radians = jagAngle * Math.PI / 1024 - Math.PI;
        int xOffset = (int) Math.round(Math.sin(radians) * distance);
        int yOffset = (int) Math.round(Math.cos(radians) * distance);
        return transform(xOffset, yOffset);
    }
    ...
    public String toString() {
        return "X: " + getX() + ", Y: " + getY() + ", Plane: " + getPlane();
    }
}
