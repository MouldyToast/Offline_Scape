...
public class PrayerManager {
	...
	public PrayerManager(final Player player) {
		...
		this.drainConsumer = (k, v) -> {
			if (v < (int) WorldThread.getCurrentCycle()) {
	 ...
	 * @param quickPrayers
	 */
	public void togglePrayer(Prayer prayer, final boolean quickPrayers) {
		...
		if (prayer == null || player.isDead() || !player.getControllerManager().canUsePrayer(prayer) || !hasRequirements(prayer) || (area instanceof PrayerPlugin && !((PrayerPlugin) area).activatePrayer(player, prayer))) {
			player.getPacketDispatcher().syncClientVarCache();
			return;
		}
	...
	public void activatePrayer(final Prayer prayer) {
		if ((prayer == Prayer.PROTECT_FROM_MAGIC || prayer == Prayer.PROTECT_FROM_MISSILES || prayer == Prayer.PROTECT_FROM_MELEE) && player.getNumericTemporaryAttribute("prayer delay").longValue() > WorldThread.getCurrentCycle()) {
	...
	public static Prayer cursePrayerTypeReverse(Prayer input) {
        return switch (input) {
            case PROTECT_FROM_MISSILES -> Prayer.PROTECT_FROM_MAGIC;
            case PROTECT_FROM_MELEE -> Prayer.PROTECT_FROM_MISSILES;
            case PROTECT_FROM_MAGIC -> Prayer.PROTECT_FROM_MELEE;
            default -> input;
        };
	}
	...
	public double getMagicBoost(final int style) {
		double boost = 1.0;
		// Apply magical attack/defense boosts based on active prayers
		if (activePrayers.containsKey(Prayer.MYSTIC_WILL)) {
			boost = 1.05; // +5% magical attack/defense boost
		} else if (activePrayers.containsKey(Prayer.MYSTIC_LORE)) {
			boost = 1.10; // +10% magical attack/defense boost
        } else if (activePrayers.containsKey(Prayer.MYSTIC_MIGHT)) {
			boost = 1.15; // +15% magical attack/defense boost
        } else if (activePrayers.containsKey(Prayer.AUGURY)) {
			if (style == SkillConstants.DEFENCE) {
				boost = 1.25; // +25% boost for Defense-specific styles
			} else {
				boost = 1.20; // +20% standard attack/defense boost
			}
        }
		return boost;
	}
	...
	private final transient BiConsumer<Prayer, Integer> drainConsumer;
	...
	public void process() {
		...
		var restricted = ((area instanceof PrayerPlugin restrictedArea) && !restrictedArea.restrictMemberPerk());
		if (!restricted && !WildernessArea.isWithinWilderness(player)) {
			if (player.getMemberRank().equalToOrGreaterThan(MemberRank.CATALYTIC))
				resistance = (int) ((double) resistance * 5.0D);
			else if (player.getMemberRank().equalToOrGreaterThan(MemberRank.NEBULA))
				resistance = (int) ((double) resistance * 4.5D);
			else if (player.getMemberRank().equalToOrGreaterThan(MemberRank.ETERNAL))
				resistance = (int) ((double) resistance * 4.0D);
			else if (player.getMemberRank().equalToOrGreaterThan(MemberRank.GOLD))
				resistance = (int) ((double) resistance * 3.5D);
			else if (player.getMemberRank().equalToOrGreaterThan(MemberRank.ENCHANTED))
				resistance = (int) ((double) resistance * 3.0D);
			else if (player.getMemberRank().equalToOrGreaterThan(MemberRank.ZENYTE))
				resistance = (int) ((double) resistance * 2.5D);
			else if (player.getMemberRank().equalToOrGreaterThan(MemberRank.ONYX))
				resistance = (int) ((double) resistance * 2.0D);
			else if (player.getMemberRank().equalToOrGreaterThan(MemberRank.DRAGONSTONE))
	...
	public void restorePrayerPoints(int amount) {
		restorePrayerPoints(amount, true);
	}

	public void restorePrayerPoints(int amount, boolean ignoreRemPet) {
