...
import org.jesse.game.content.dt2.area.obj.FermentationVat
import org.jesse.game.content.dt2.npc.DT2BossDifficulty
import org.jesse.game.content.dt2.npc.deathsToAwakenedDuke
import org.jesse.game.content.dt2.npc.deathsToDuke
import org.jesse.game.content.dt2.npc.findObject
import org.jesse.game.content.dt2.npc.theduke.*
 ...
 * @date 8.14.2024
 */
data class DukeSucellusInstance(
    val difficulty: DT2BossDifficulty,
    val player: Player
) : PrebuiltDynamicArea(REGION_ID),
    CycleProcessPlugin,
    DeathPlugin {
    ...
    val slumbering: Boolean get() = duke.slumbering || duke.isDead
    ...
    override fun enter(player: Player) = with(player) {
        ...
        Music.getOrNull("Eye See You")?.let { music.unlock(it) }
    ...
    private fun buildExtremities() {
        val left = ExtremityGroup(
            ExtremitySide.LEFT, this,
            members = listOf(
                Extremity(leftTopExtremityCoord, 47546, 47547, 3, 10188, 12199, 10187),
                Extremity(leftMiddleExtremityCoord, 47544, 47548, 3, 10187, 12200, 10188),
                Extremity(leftBottomExtremityCoord, 47545, 47549, 3, 10189, 12201, 10189)
            )
        )

        val right = ExtremityGroup(
            ExtremitySide.RIGHT, this,
            members = listOf(
                Extremity(rightTopExtremityCoord, 47545, 47549, 1, 10188, 12199, 10189),
                Extremity(rightMiddleExtremityCoord, 47546, 47547, 1, 10187, 12200, 10187),
                Extremity(rightBottomExtremityCoord, 47544, 47548, 1, 10189, 12201, 10188)
            )
        )
    ...
    private fun transferPlayer() {
        ...
        WorldTasksManager.schedule(2) {
            screen.unfade()
            awakenExtremities()
            FallingIceZone(this, ExtremitySide.RIGHT).executeFallingIce()
            FallingIceZone(this, ExtremitySide.LEFT).executeFallingIce()
        }
        player.unlock()
    }
    ...
    fun fillVat(player: Player, loc: WorldObject) = when {
        ...
        loc.position.matches(rightVatCoord) -> rightVat.fill(player, loc)
            ...
            player.sendDeveloperMessage("Unable to find which vat to fill")
        }
    }
    ...
    fun checkVat(player: Player, loc: WorldObject) = when {
        ...
        loc.position.matches(rightVatCoord) -> rightVat.check(player)
            ...
            player.sendDeveloperMessage("Unable to find which vat to check")
        }
    }
    ...
    private fun addNpcs() {
        ...
        repeat(3) {
            when (it) {
                0 -> gasVentPatterns[it] = listOf(ventTiles[2].copy(), ventTiles[4].copy(), ventTiles[6].copy())
                1 -> gasVentPatterns[it] = listOf(ventTiles[0].copy(), ventTiles[5].copy(), ventTiles[7].copy())
                else -> gasVentPatterns[it] = listOf(ventTiles[1].copy(), ventTiles[3].copy(), ventTiles[8].copy())
            }
        }
    ...
    override fun process() {
        extremityGroups.values.forEach { extremityGroup ->
    ...
    override fun sendDeath(player: Player?, source: Entity?): Boolean {
        player ?: return true
        DT2Module.getDukeStatistics(awakened).globalDeathCount++
        if (difficulty == DT2BossDifficulty.AWAKENED)
            player.deathsToAwakenedDuke++
        else
            player.deathsToDuke++
        return false
    }
