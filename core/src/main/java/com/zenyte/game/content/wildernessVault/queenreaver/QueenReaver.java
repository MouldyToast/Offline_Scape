...
public class QueenReaver extends NPC implements CombatScript {
    ...
    public void handleIngoingHit(Hit hit) {
        ...
        if (hit.getSource() instanceof Player player) {
            var counter = player.getNumericAttribute("WILDERNESS_VAULT_DAMAGE_COUNTER").intValue();
            var damage = hit.getDamage();
            var updatedCounter = counter + damage;
            player.getTemporaryAttributes().put("WILDERNESS_VAULT_DAMAGE_COUNTER", updatedCounter);
        }
    }
    ...
    protected void drop(Location tile) {
        ...
        getReceivedDamage().forEach((key, value) -> {
            Player player = World.getPlayer(key.getFirst()).orElse(null);
            if (player != null) {
                int damage = 0;
                for (ReceivedDamage pair : value) {
        ...
        getPlayers().stream()
            .filter(player -> !WildernessVaultHandler.canLoot.contains(player.getName()))
            .forEach(player -> player.sendMessage("You failed to do enough damage to receive the kill count and the rewards."));

        super.drop(tile);
    }

    public void clear() {
        fires.forEach(World::removeObject);

        fires.clear();

        bloodSplatTiles.stream()
            .map(bloodSplatTile -> World.getObjectWithType(bloodSplatTile, DEFAULT_TYPE))
            .forEach(World::removeObject);
        bloodSplatTiles.clear();

        bloodSpawns.forEach(NPC::finish);
        bloodSpawns.clear();

        bloodTrails.forEach(World::removeObject);
        bloodTrails.clear();
    }
    ...
    protected void postHitProcess(Hit hit) {
        ...
        getPlayers().forEach(player -> player.getHpHud().updateValue(getHitpoints()));
    }

    private void openHud() {
        for (Player player : getPlayers()) {
            player.getHpHud().open(WildernessVaultConstants.BOSS_ID, getMaxHitpoints());
            player.getBossTimer().startTracking(notificationName(player));
        }
    }
