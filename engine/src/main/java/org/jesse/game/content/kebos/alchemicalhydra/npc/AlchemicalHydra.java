package org.jesse.game.content.kebos.alchemicalhydra.npc;
...
import org.jesse.game.content.kebos.alchemicalhydra.npc.combat.phases.EnragedPhase;
import org.jesse.game.content.kebos.alchemicalhydra.npc.combat.phases.FlamePhase;
...
public class AlchemicalHydra extends NPC implements CombatScript {
    ...
    protected void onFinish(Entity source) {
        ...
        if (source instanceof final Player player) {
            ...
            int kc = Integer.parseInt(player.getAttributes().getOrDefault(CA_TASK_INSTANCE_KC_ATT, 0).toString()) + 1;
            if (kc >= 15) {
    ...
    private void resetNPC(final HydraPhase nextPhase) {
        ...
        phase = nextPhase;
        try {
            sequence = phase.getPhaseSequence().getDeclaredConstructor().newInstance();
