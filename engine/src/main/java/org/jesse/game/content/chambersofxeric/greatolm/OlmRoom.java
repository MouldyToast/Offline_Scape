...
import org.jesse.game.content.chambersofxeric.CombatPointCapCalculator;
...
import org.jesse.game.content.chambersofxeric.map.BossChunk;
...
public final class OlmRoom extends BossChunk implements CycleProcessPlugin, FullMovementPlugin, PlayerCombatPlugin {
    ...
    void sendAnimation(@NotNull final WorldObject object, @NotNull final OlmAnimation animation) {
        if (object == leftClawObject) {
            ...
            this.animationEndTick = WorldThread.getCurrentCycle() + (Math.min(TimeUnit.MILLISECONDS.toTicks(animation.getAnimationLength()), 1));
            //}
    ...
    void queueOrExecute(@NotNull final Runnable runnable) {
        if (WorldThread.getCurrentCycle() < this.animationEndTick) {
            WorldTasksManager.schedule(runnable::run, (int) (this.animationEndTick - WorldThread.getCurrentCycle()));
            return;
        }
        runnable.run();
    }
