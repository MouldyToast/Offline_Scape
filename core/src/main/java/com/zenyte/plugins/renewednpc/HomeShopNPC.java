...
public class HomeShopNPC extends NPCPlugin {
    ...
    public void handle() {
        ...
        bind("Vote shop", openShop(13));
        bind("Loyalty shop", openShop(12));
    ...
    private OptionHandler openShop(int storeId) {
        return new OptionHandler() {

            @Override
            public void handle(final Player player, final NPC npc) {
                if (UNIVERSAL_SHOP_FLOODGATE)
                    UniversalShopInterface.openInterfaceToTab(player, storeId);
                else
                    player.openShop("Loyalty Shop");
                informPlayerOfPoints(player);
            }
    ...
    private void informPlayerOfPoints(Player player) {
        var votePoints = Utils.formatNumberWithCommas(VotePlayerAttributesKt.getTotalVoteCredits(player));
        var loyaltyPoints = Utils.formatNumberWithCommas(player.getLoyaltyManager().getLoyaltyPoints());
        var slayerPoints = Utils.formatNumberWithCommas(player.getSlayer().getSlayerPoints());

        player.sendMessage("You have - Vote: %s pts, Loyalty: %s pts, Slayer: %spts"
            .formatted(
                Colour.RED.wrap(votePoints),
                Colour.RED.wrap(loyaltyPoints),
                Colour.RED.wrap(slayerPoints)
            )
        );
    }
