...
public class BurningBarrageSpecial implements SpecialAttackScript {
    ...
    public void attack(Player player, PlayerCombat combat, Entity target) {
        ...
        switch (probability) {
            case 0: {
                ...
                rollForBurningDamage(player, combat, target);
                return;
            }
            case 1: {
    ...
    private void rollForBurningDamage(Player player, PlayerCombat combat, Entity target) {
        for (int i = 0; i < 3; i++) {
            if (!combat.isSuccessful(player, target, 1, AttackType.SLASH)) continue;
            // apply burn
            if (target instanceof NPC npc) {
                var currentBurns = npc.getNumericTemporaryAttribute("burningDamage").intValue();
                npc.getTemporaryAttributes().put("burningDamage", currentBurns + 5);
            }
            if (target instanceof Player prey) {
                var currentBurns = prey.getNumericTemporaryAttribute("burningDamage").intValue();
                prey.getTemporaryAttributes().put("burningDamage", currentBurns + 5);
            }
        }
    }
