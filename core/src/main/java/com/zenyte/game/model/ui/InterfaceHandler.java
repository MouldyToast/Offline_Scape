...
public class InterfaceHandler {
	...
	public void sendInterface(final InterfacePosition position, final int id, final boolean walkable) {
		...
		if (position.equals(InterfacePosition.CENTRAL)) {
			player.getPacketDispatcher().sendClientScript(2524, EXPANDED_WIDTH_HEIGHT_INTERFACES.contains(id) ? EXPANDED_WIDTH_HEIGHT_ARGS : EXPANDED_HEIGHT_INTERFACES.contains(id) ? EXPANDED_HEIGHT_ARGS : DEFAULT_ARGS);
	...
	public void setPane(final PaneType pane) {
		...
		int magicComponent = InterfacePosition.SPELLBOOK_TAB.getComponent(pane);
		if (magicComponent != -1) {
			player.getPacketDispatcher().sendComponentSettings(pane.getId(), magicComponent, -1, -1, AccessMask.CLICK_OP1, AccessMask.CLICK_OP2, AccessMask.CLICK_OP3, AccessMask.CLICK_OP4);
		}
	}
