...
import org.jesse.game.content.skills.hunter.node.tables.*;
...
public class ImplingJarPlugin extends ItemPlugin {
    ...
    public enum ImplingJar {
        ...
        public Item[] generateLoot(Player player) {
            for (ClueDrop drop : clueDrops) {
                ...
                if (drop.itemId() == ClueItem.EASY.getScrollBox() && player.getCombatAchievements().hasTierCompleted(CATierType.EASY)) {
                    chance *= 0.90;
                }
                if (drop.itemId() == ClueItem.MEDIUM.getScrollBox() && player.getCombatAchievements().hasTierCompleted(CATierType.MEDIUM)) {
                    chance *= 0.90;
                }
                if (drop.itemId() == ClueItem.HARD.getScrollBox() && player.getCombatAchievements().hasTierCompleted(CATierType.HARD)) {
                    chance *= 0.90;
                }
                if (drop.itemId() == ClueItem.ELITE.getScrollBox() && player.getCombatAchievements().hasTierCompleted(CATierType.ELITE)) {
                    chance *= 0.90;
                }
    ...
    private record ClueDrop(int itemId, int chance) {
    }
}
