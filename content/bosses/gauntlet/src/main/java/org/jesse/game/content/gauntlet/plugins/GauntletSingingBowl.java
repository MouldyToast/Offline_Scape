    ...
    private void openInterface(Player player, boolean corrupted) {
        ...
        player.getDialogueManager().start(new SkillDialogue(player, "What would you like to make?", items) {
            ...
            public void run(int slotId, int amount) {
                SingingBowlItem bowlItem = options[slotId];

                boolean hasRequirements = true;
                for (Item req : bowlItem.requirements) {
                ...
                if (!player.getInventory().containsItem(shards, bowlItem.shards) || !hasRequirements) {
                    player.getDialogueManager().start(new PlainChat(player, "You do not have the materials required to make that."));
                    return;
                }

                switch (bowlItem) {
                    case CRYSTAL_BOW_ATTUNED, CRYSTAL_HALBERD_ATTUNED, CRYSTAL_STAFF_ATTUNED, CORRUPTED_BOW_ATTUNED, CORRUPTED_HALBERD_ATTUNED,
                         CORRUPTED_STAFF_ATTUNED -> {
                        final Gauntlet gauntlet = GauntletPlayerAttributesKt.getGauntlet(player);
                        if (gauntlet != null) {
                            gauntlet.attunedWeaponCount++;
                        }
                    }
                    case CORRUPTED_BODY_BASIC,
                         CORRUPTED_BODY_ATTUNED,
                         CORRUPTED_BODY_PERFECTED,
                         CORRUPTED_HELM_BASIC,
                         CORRUPTED_HELM_ATTUNED,
                         CORRUPTED_HELM_PERFECTED,
                         CORRUPTED_LEGS_BASIC,
                         CORRUPTED_LEGS_ATTUNED,
                         CORRUPTED_LEGS_PERFECTED,

                         CRYSTAL_BODY_BASIC,
                         CRYSTAL_BODY_ATTUNED,
                         CRYSTAL_BODY_PERFECTED,
                         CRYSTAL_HELM_BASIC,
                         CRYSTAL_HELM_ATTUNED,
                         CRYSTAL_HELM_PERFECTED,
                         CRYSTAL_LEGS_BASIC,
                         CRYSTAL_LEGS_ATTUNED,
                         CRYSTAL_LEGS_PERFECTED -> {
                        final Gauntlet gauntlet = GauntletPlayerAttributesKt.getGauntlet(player);
                        if (gauntlet != null)
                ...
                player.getInventory().deleteItem(shards, bowlItem.shards);

                final int id = bowlItem.id;
                final int slot = Optional.ofNullable(ItemDefinitions.get(id)).map(ItemDefinitions::getSlot).orElse(-1);
                Item item = new Item(id);
                item.setCharges(DegradableItem.getDefaultCharges(id, 0));
                if (replaceEquipment && slot != -1)
                    player.getEquipment().set(slot, item);
                else
                    player.getInventory().addOrDrop(item);

                player.getSkills().addXp(SkillConstants.CRAFTING, bowlItem.experienceGain);
                player.getSkills().addXp(SkillConstants.SMITHING, bowlItem.experienceGain);
