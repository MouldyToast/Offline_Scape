...
import static org.jesse.game.world.entity.player.var.EventType.POST_LOGIN;
import static org.jesse.game.world.entity.player.var.EventType.PRE_LOGIN;
import static org.jesse.game.world.entity.player.var.VarType.VAR;
import static org.jesse.game.world.entity.player.var.VarType.VARBIT;
...
public enum VarCollection {
    ...
    LEVEL_UP_DIALOGUES(VAR, 3601, p -> attributeMaxInit(p, "LEVEL_UP_DIALOGUES", 99), POST_LOGIN),
    ...
    DIALOGUE_FULL_SIZE(VARBIT, 10670, p -> 1),
    ...
    public final void updateSingle(final Player player) {
        sendVarInstant(player, id);
    }

//    public final void updateSingleInstant(final Player player) {
//        sendVarInstant(player, function.getValue(player));
//    }
    ...
    public void sendVarInstant(Player player, int i) {
        if (type == VAR) {
            player.getVarManager().sendVarInstant(i, function.getValue(player));
        } else {
            player.getVarManager().sendBitInstant(i, function.getValue(player));
        }
    }
}
