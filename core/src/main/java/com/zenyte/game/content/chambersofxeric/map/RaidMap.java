...
import com.zenyte.game.content.chambersofxeric.room.EntranceRoom;
...
public class RaidMap {
    private static final Logger log = NearRealityLogger.getLogger(RaidMap.class);
    /**
    ...
    public void construct() throws OutOfSpaceException {
        ...
        var entryRoom = getRaidChunks().stream().filter(room -> room instanceof EntranceRoom).findFirst();
        if (entryRoom.isPresent()) {
            var room = entryRoom.get();
            var stairLocation = room.getBoundTile().copy().transform(0, -3);
            World.spawnObject(new WorldObject(ObjectId.STEPS_29778, 10, 0, stairLocation));
        }
    ...
    private final void bindTeleportationPositions() {
        ...
        for (int index = 0; index < raidChunks.size(); index++) {
            ...
            if (type == RaidRoom.RAID_START) {
                chunk.setBoundTile(chunk.getRespectiveTile(chunk.getLocation(3278, 5187, chunk.getToPlane()),
                        chunk.getLocation(3298, 5188, 0),
                        chunk.getLocation(3330, 5188, chunk.getToPlane())));
