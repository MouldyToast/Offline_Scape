...
import org.jesse.game.content.tombsofamascut.npc.BaboonNPC;
import org.jesse.game.content.tombsofamascut.npc.TOANPC;
...
public class ApmekenEncounter extends TOARaidArea implements CycleProcessPlugin {
    ...
    private static final Location[] BABOON_SPAWN_LOCATIONS = {new Location(3800, 5288), new Location(3808, 5288),
            new Location(3816, 5288), new Location(3816, 5272), new Location(3808, 5272), new Location(3800, 5272)};
    public static final Location[] VENT_LOCATIONS = {new Location(3800, 5284), new Location(3816, 5284),
            new Location(3816, 5276), new Location(3800, 5276)};
    public static final Location[] ROOF_SKULL_LOCATIONS = {new Location(3804, 5288), new Location(3812, 5288),
            new Location(3812, 5272), new Location(3804, 5272)};
    private static final Location[] ROOF_LOCATIONS = {new Location(3803, 5289), new Location(3811, 5289),
            new Location(3811, 5269), new Location(3803, 5269)};
    ...
    private static final Tinting RESET_TINTING = Tinting.RESET_TINTING;
    ...
    private static final int[][] WAVE_BABOONS = {{BABOON_MELEE_ID, BABOON_MELEE_ID}, {BABOON_RANGE_ID, BABOON_MAGE_ID},
            {BABOON_MELEE_ID, BABOON_MELEE_ID, BABOON_SHAMAN_ID}, {BABOON_MELEE_ID, BABOON_MELEE_ID, -1},
            {BABOON_RANGE_ID, BABOON_RANGE_ID, BABOON_VOLATILE_ID}, {BABOON_MAGE_ID, BABOON_MAGE_ID, BABOON_CURSED_ID},
            {BABOON_STRONGER_RANGE_ID, BABOON_STRONGER_RANGE_ID, -1, -1, -1},
            {BABOON_STRONGER_MAGE_ID, BABOON_STRONGER_MAGE_ID, -1, -1, -1},
            {BABOON_STRONGER_MELEE_ID, BABOON_STRONGER_MELEE_ID, -1, -1, -1},
            {BABOON_STRONGER_MELEE_ID, BABOON_STRONGER_RANGE_ID, -1, -1, -1},
            {BABOON_SHAMAN_ID, BABOON_SHAMAN_ID, BABOON_VOLATILE_ID, BABOON_VOLATILE_ID, BABOON_VOLATILE_ID}};
    private static final int[] EXTRA_COMBAT_BABOONS = {BABOON_MELEE_ID, BABOON_MAGE_ID, BABOON_MELEE_ID, BABOON_MAGE_ID,
            BABOON_MELEE_ID, BABOON_RANGE_ID, BABOON_STRONGER_MELEE_ID, BABOON_STRONGER_MAGE_ID,
            BABOON_STRONGER_RANGE_ID, BABOON_STRONGER_MAGE_ID, BABOON_CURSED_ID};
    private int wave;
    private int waveDelay = 6;
    private int criticalNpcCount;

    private final List<TOANPC> spawnedBaboons = new ArrayList<>();
    ...
    private final boolean[] actionsDone = new boolean[4];
    private final List<WorldObject> venomPools = new ArrayList<>();
            ...
            if (cycleTicks > 0 && --cycleTicks == 0) {
                if (!usedSpecial && (sightPlayer == null || sightPlayer.isDying() || sightPlayer.isFinished() || !insideChallengeArea(sightPlayer)
                    ...
                    if (!usedSpecial) {
                        ...
                        switch (specialIndex) {
                            case 0:
                                sightPlayer.sendMessage("<col=6800bf>You sense an issue with the floor vents.");
                                for (int i = 0; i < VENT_LOCATIONS.length; i++) {
                                    final Location location = getLocation(VENT_LOCATIONS[i]);
                                    final Location lastLoaded = sightPlayer.getLastLoadedMapRegionTile();
                                    sightPlayer.getPacketDispatcher().mapAnimSpecific(
                                        i < 2 ? SENSE_NORTHERN_SKULL_GFX_ID : SENSE_SOUTHERN_SKULL_GFX_ID,
                                        0,
                                        32,
    ...
    public void spawnWave() {
        ...
        final List<Integer> indices = IntStream.range(0, BABOON_SPAWN_LOCATIONS.length).boxed().collect(Collectors.toList());
    ...
    private void resetRoofs() {
        for (var roofLocation : ROOF_LOCATIONS) {
            World.spawnObject(new WorldObject(ROOF_REGULAR_OBJECT_ID, 10, 3, getLocation(roofLocation)));
        }
    }
    ...
    public List<TOANPC> getSpawnedBaboons() {
        return spawnedBaboons;
    }
}
