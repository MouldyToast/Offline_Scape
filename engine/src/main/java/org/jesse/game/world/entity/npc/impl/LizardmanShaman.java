...
public class LizardmanShaman extends NPC implements Spawnable, CombatScript {
    ...
    public void dropItem(final Player killer, final Item item, final Location tile, boolean guaranteedDrop) {
        if (tile.getPlane() != respawnTile.getPlane()) tile.setLocation(tile.getX(), tile.getY(), respawnTile.getPlane());
    ...
    public void processNPC() {
        super.processNPC();
        if (lastTime < Utils.currentTimeMillis() && Utils.random(60) == 0) spawnFollowers();
    }

    @Override public void handleOutgoingHit(Entity target, Hit hit) {
        super.handleOutgoingHit(target, hit);
        if (hit.getDamage() > 0) hitAnyone = true;
    }
    ...
    private void spawnFollowers() {
        ...
        if (!(target instanceof Player player) || isProjectileClipped(target, true)) return;
        ...
        for (final Location spawnTile : spawnTiles) new LizardmanSpawn(spawnTile, player, isTemple).spawn();
        //Cause camera shake for all the players standing near the Shaman.
        CharacterLoop.forEach(getMiddleLocation(), 2, Player.class, p -> {
            if (p.isDead() || p.isFinished()) return;
    ...
    @NotNull
    private Location getBaseSpawnLocation(@NotNull final Player target) {
    ...
    @NotNull
    private List<Location> getSpawnLocations(@NotNull final Location tile) {
        ...
        for (int i = list.size(); i < 3; i++) list.add(new Location(tile));
    ...
    private void jump(@NotNull final Entity target, @NotNull final Location tile) {
    ...
    private void shootPoison(@NotNull final Entity target) {
        setAnimation(attack);
            ...
            if (!isDead())
                for (final Player p : CharacterLoop.find(tt, 1, Player.class, player -> !player.isDead() && (isMultiArea() || player == target)))
                    delayHit(-1, target, new Hit(LizardmanShaman.this, (int) (Utils.random(20, 35) * protectionModifier(p)), HitType.POISON).onLand(hit -> {
                        if (hit.getDamage() > 0) p.getToxins().applyToxin(Toxins.ToxinType.POISON, 10, LizardmanShaman.this);
                    }));
        }, delay);
    }
    ...
    public int attack(final Entity target) {
        ...
        if (style == 5) {
            ...
            if ((style = Utils.random(isWithinMeleeDistance(this, target) ? 5 : 4)) == 5) style = 6;
        }
        switch (style) {
        case 6: 
    ...
    public static float protectionModifier(final Player player) {
        float modifier = 1.0F;
