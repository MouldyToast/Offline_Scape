...
public class DeathNpcPlugin extends NPCPlugin {
    ...
    private void attemptToOpenCollection(Player player) {
        // If the account has a PIN & Required unlocking; STOP HERE
        if (player.getBankPin().requiresVerification(player, () -> openCollectionInterface(player)))
            return;
        openCollectionInterface(player);
    }
    ...
    public void handle() {
                ...
                player.getDialogueManager().start(new Dialogue(player, npc) {
                    ...
                    public void buildDialogue() {
                        if (player.getVarManager().getBitValue(10465) > 0) {
                            ...
                            options(
                                    new DialogueOption("Can I collect the items from that gravestone now?", () -> attemptToOpenCollection(player)),
                        ...
                        player(300, "Yes, have you got anything for me?").executeAction(() -> attemptToOpenCollection(player));
        ...
        bind("Collect", new OptionHandler() {
            ...
            @Override
            public void handle(Player player, NPC npc) {
                attemptToOpenCollection(player);
            }
