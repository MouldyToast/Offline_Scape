 ...
 */
@SkipPluginScan
public class IncubatorControlsObject implements ObjectAction {
    
    public static boolean isFixed(final Player player) {
        return player.getVarManager().getBitValue(EasterConstants.INCUBATOR_CONTROLS_VARBIT) == 1;
    }
    
    public static void fix(final Player player) {
        player.getVarManager().sendBit(EasterConstants.INCUBATOR_CONTROLS_VARBIT, 1);
    }
    
    @Override
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        player.getDialogueManager().start(new PlainChat(player, "This looks like the panel which controls the incubator."));
    }
    
    @Override
    public Object[] getObjects() {
        return new Object[]{EasterConstants.INCUBATOR_CONTROLS};
    }
    ...
    public static class ItemOnCoalSupply implements ItemOnObjectAction {
        ...
        public void handleItemOnObjectAction(Player player, Item item, int slot, WorldObject object) {
            ...
            if (item.getId() != EasterItem.CLEAN_PIPE.getItemId()) {
            ...
            player.sendMessage("You connect the controls to the incubator with the clean pipe.");
            fix(player);
