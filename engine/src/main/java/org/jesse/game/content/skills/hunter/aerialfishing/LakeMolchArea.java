package org.jesse.game.content.skills.hunter.aerialfishing;

import org.jesse.game.content.skills.hunter.aerialfishing.npc.FishingSpotNpc;
...
public class LakeMolchArea extends GreatKourend implements CycleProcessPlugin {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(LakeMolchArea.class.getName());
    ...
    public void process() {
        if (fishingSpots == 0) {
            if (!USED_SPAWN_LOCATIONS.isEmpty())
                LOGGER.warn("Fishing spots are empty but there are still " + USED_SPAWN_LOCATIONS.size() + " used spawn locations");
    ...
    public static void spawnSpot() {
        generateRandomSpotLocation()
                .ifPresentOrElse(fishSpawnTask(), () -> LOGGER.warn("Failed to spawn fishing spot"));
    }
...
//        LOGGER.info("Removing fishing spot at " + location.toString());
    ...
    private static @NotNull Optional<Location> findRandomLocation(Set<Location> possibleLocations) {
        ...
        if (World.findNPCAt(8523, randomLocation).isPresent()) {
            LOGGER.warn("Supposedly valid location is already occupied by a fishing spot: " + randomLocation.toString());
            return Optional.empty();
        } else
