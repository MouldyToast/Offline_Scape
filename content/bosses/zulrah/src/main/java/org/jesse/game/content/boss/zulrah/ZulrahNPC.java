    ...
    private Sequence getNextSequence() {
        ...
        if (phase >= seq.length) {
            ...
            if (++sequence >= sequences[rotation].length) {
                ...
                rotation = Utils.random(3);
                if (!firstRotation) {
                    // After the first rotation, Zulrah attacks with Ranged×5
                    // before filling the area with venom clouds.
                    return new RangedSequence(5);
                }
                firstRotation = false;
            }
