...
import org.jesse.game.world.entity.player.ClockMap;
...
import org.jesse.utils.IntLinkedList;
...
public abstract class AbstractEntity implements Entity {
    private static final Logger log = NearRealityLogger.getLogger(AbstractEntity.class);
    ...
    public void addHitbar(HitBar hitBar) {
    ...
    public void faceAngle(final int angle) {
        setFaceLocation(middleTile.transformAngle(angle, 10));
    }
    ...
    public void setGraphics(final Graphics graphics) {
        ...
        updateFlags.flag(UpdateFlag.SPOTANIM);
    }
    ...
    @Override
    public void lock(final int ticks) {
        lockDelay = WorldThread.getCurrentCycle() + ticks;
    }
    ...
    public boolean isLocked() {
        return lockDelay > WorldThread.getCurrentCycle();
    }
    ...
    public void setFaceEntityCombat(Entity entity) {
    ...
    public void setForceTalk(final ForceTalk talk) {
        ...
        if (this instanceof Player player) {
            if (player.getAvatar() == null) return;
            player.getAvatar().getExtendedInfo().setSay(talk.getText());
        } else if (this instanceof NPC npc) {
            if (npc.getAvatar() == null) return;
            npc.getAvatar().getExtendedInfo().setSay(talk.getText());
        }
    }
