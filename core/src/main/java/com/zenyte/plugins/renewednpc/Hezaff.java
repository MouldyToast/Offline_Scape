...
public class Hezaff extends NPCPlugin {
    ...
    public void handle() {
        bind("Buy-teletabs", (player, npc) -> {
            ...
            if (!inventory.hasFreeSlots() && !inventory.containsItem(ItemId.NR_TABLET, 1)) {
            ...
            final int count = inventory.getAmountOf(ItemId.NR_TABLET);
            ...
            player.sendInputInt("How many would you like to purchase for " + price + " each? (0-" + maxAvailablePurchase + ")", value -> {
                ...
                final int currentCount = inventory.getAmountOf(ItemId.NR_TABLET);
                ...
                inventory.addOrDrop(new Item(ItemId.NR_TABLET, purchaseQuantity));
