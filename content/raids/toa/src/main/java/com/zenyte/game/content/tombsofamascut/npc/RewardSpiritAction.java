package com.zenyte.game.content.tombsofamascut.npc;

import com.zenyte.game.content.tombsofamascut.TOAAccess;

import com.zenyte.game.content.tombsofamascut.encounter.RewardEncounter;
import com.zenyte.game.world.entity.npc.actions.NPCPlugin;
import com.zenyte.plugins.dialogue.OptionDialogue;

import static com.zenyte.game.world.entity.npc.NpcId.OSMUMTEN_11693;

/**
 * @author Savions
 */
public class RewardSpiritAction extends NPCPlugin {

    @Override
    public void handle() {
        bind("Talk-to", (player, npc) -> {
            final OptionDialogue dialogue = new OptionDialogue(player, "Are you ready to leave the Tombs of Amascut?", new String[] {"Yes.", "No."},
                    new Runnable[] {() -> TOAAccess.getToaManager(player).leaveTombs("You leave the Tombs of Amascut."), null});
            player.getDialogueManager().start(dialogue);
        });
        bind("Leave", (player, npc) -> {
            TOAAccess.getToaManager(player).leaveTombs("You leave the Tombs of Amascut.");
        });
    }

    @Override
    public int[] getNPCs() {
        return new int[] {OSMUMTEN_11693};
    }
}
