...
object SlayerHelmetEffects {
    ...
    fun immuneToPoison(player: Player, target: Entity?): Boolean {
        if (wearsCombatAchievementSlayerHelmet(player))
            return true
        return (araxyteHelmet(player, true) || hydraHelmetNoAssignment(player, false) || greenHelmetNoAssignment(player, false)) && (player.slayer.isCurrentAssignment(target) || (target !is Player && player.hasBoon(SlayersSovereignty::class.java)))
    }
    ...
    fun araxyteHelmet(player: Player, checkImbued: Boolean): Boolean {
        return SlayerMountType.ARAXYTE.mounted(player) || wearsSlayerHelmet(player, "araxyte", checkImbued)
    }
    ...
    fun wearsCombatAchievementSlayerHelmet(player: Player, checkImbued: Boolean = false): Boolean {
        return wearsTztokSlayerHelmet(player, checkImbued) ||
                wearsVampiricSlayerHelmet(player, checkImbued) ||
                wearsTzkalSlayerHelmet(player, checkImbued)
    }
