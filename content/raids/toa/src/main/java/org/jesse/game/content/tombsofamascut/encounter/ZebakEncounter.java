...
import org.jesse.game.content.tombsofamascut.npc.CrondisJug;
import org.jesse.game.content.tombsofamascut.npc.Zebak;
...
public class ZebakEncounter extends TOARaidArea implements FullMovementPlugin, CycleProcessPlugin {
	...
	private int getPoisonDamage() {
		final int base = Utils.random(5, 10);
		return Utils.random(base, base + 10);
	}
		...
		if (zebak != null && !zebak.isDying() && !zebak.isFinished() && EncounterStage.STARTED.equals(stage) && players != null && players.length > 0) {
			Arrays.stream(players).forEach(p -> {
				if (onActivePoison(p.getLocation())) {
					if (p.getToxins().isPoisoned()) return;
					var poisonHit = new Hit(zebak, getPoisonDamage(), HitType.POISON);
					zebak.delayHit(-1, p, poisonHit.onLand(hit ->
						p.getToxins().applyToxin(Toxins.ToxinType.POISON, getPoisonDamage(), zebak))
					);
				}
			});
	...
	@Override public Player[] getChallengePlayers() {
		return players.stream()
			// the player is not dead or dying
			.filter(p -> p != null && !p.isDying())
			// lastly they are inside the arena
			.filter(this::insideChallengeArea)
			// the player is not a ghost
			.filter(p -> !p.getAppearance().isTransformedIntoNpc())
			// collect it all to an array
			.toArray(Player[]::new);
	}
