package org.jesse.game.world.entity.pathfinding;

import org.jesse.game.world.World;
import org.jesse.game.world.region.Region;
import org.jesse.game.world.region.RegionMap;
import org.jesse.logger.NearRealityPrintStream;

/**
 * @author Kris | 28/01/2019 03:30
 * @see <a href="https://www.rune-server.ee/members/kris/">Rune-Server profile</a>
 */
public class IWalkRouteFinder {
    private final int GRAPH_SIZE = 128;
    private final int QUEUE_SIZE = (GRAPH_SIZE * GRAPH_SIZE) / 4; // we do /4 because each tile can only be accessed from single direction
    private final int ALTERNATIVE_ROUTE_MAX_DISTANCE = 100;
    private final int ALTERNATIVE_ROUTE_RANGE = 15;

    private final int DIR_NORTH = 0x1;
    private final int DIR_EAST = 0x2;
    private final int DIR_SOUTH = 0x4;
    private final int DIR_WEST = 0x8;
     ...
     */
    public synchronized int findRoute(final int srcX, final int srcY, final int srcZ, final int srcSizeXY,
     ...
     */
    private boolean performCalculationS1(final int srcX, final int srcY, final RouteStrategy strategy) {
        // first, we will cache our fields to local variables, this is done for performance, because
        ...
        while (read != write) {
            ...
            if (currentGraphX > 0 && tileDirections[currentGraphX - 1][currentGraphY] == 0
                    && (clipMasks[currentGraphX - 1][currentGraphY] & (Flags.BLOCK_EAST)) == 0) {
            ...
            if (currentGraphX < (GRAPH_SIZE - 1) && tileDirections[currentGraphX + 1][currentGraphY] == 0
                    && (clipMasks[currentGraphX + 1][currentGraphY] & (Flags.BLOCK_WEST)) == 0) {
            ...
            if (currentGraphY > 0 && tileDirections[currentGraphX][currentGraphY - 1] == 0
                    && (clipMasks[currentGraphX][currentGraphY - 1] & (Flags.BLOCK_NORTH)) == 0) {
            ...
            if (currentGraphY < (GRAPH_SIZE - 1) && tileDirections[currentGraphX][currentGraphY + 1] == 0
                    && (clipMasks[currentGraphX][currentGraphY + 1] & (Flags.BLOCK_SOUTH)) == 0) {
            ...
            if (currentGraphX > 0 && currentGraphY > 0 && tileDirections[currentGraphX - 1][currentGraphY - 1] == 0
                    && (clipMasks[currentGraphX - 1][currentGraphY - 1] & (Flags.BLOCK_NORTH_EAST)) == 0
                    && (clipMasks[currentGraphX - 1][currentGraphY] & (Flags.BLOCK_EAST)) == 0
                    && (clip[currentGraphX][currentGraphY - 1] & (Flags.BLOCK_NORTH)) == 0) {
            ...
            if (currentGraphX < (GRAPH_SIZE - 1) && currentGraphY > 0 && tileDirections[currentGraphX + 1][currentGraphY - 1] == 0
                    && (clipMasks[currentGraphX + 1][currentGraphY - 1] & (Flags.BLOCK_NORTH_WEST)) == 0
                    && (clipMasks[currentGraphX + 1][currentGraphY] & (Flags.BLOCK_WEST)) == 0
                    && (clipMasks[currentGraphX][currentGraphY - 1] & (Flags.BLOCK_NORTH)) == 0) {
            ...
            if (currentGraphX > 0 && currentGraphY < (GRAPH_SIZE - 1) && tileDirections[currentGraphX - 1][currentGraphY + 1] == 0
                    && (clipMasks[currentGraphX - 1][currentGraphY + 1] & (Flags.BLOCK_SOUTH_EAST)) == 0
                    && (clipMasks[currentGraphX - 1][currentGraphY] & (Flags.BLOCK_EAST)) == 0
                    && (clipMasks[currentGraphX][currentGraphY + 1] & (Flags.BLOCK_SOUTH)) == 0) {
            ...
            if (currentGraphX < (GRAPH_SIZE - 1) && currentGraphY < (GRAPH_SIZE - 1) && tileDirections[currentGraphX + 1][currentGraphY + 1] == 0
                    && (clipMasks[currentGraphX + 1][currentGraphY + 1] & (Flags.BLOCK_SOUTH_WEST)) == 0
                    && (clipMasks[currentGraphX + 1][currentGraphY] & (Flags.BLOCK_WEST)) == 0
                    && (clipMasks[currentGraphX][currentGraphY + 1] & (Flags.BLOCK_SOUTH)) == 0) {
     ...
     */
    private boolean performCalculationS2(final int srcX, final int srcY, final RouteStrategy strategy) {
     ...
     */
    private boolean performCalculationSX(final int srcX, final int srcY, final int size, final RouteStrategy strategy) {
        // first, we will cache our fields to local variables, this is done for performance, because
        // modern jit compiler's usually takes advantage of things like this
        ...
        while (read != write) {
            ...
            if (currentGraphX > 0 && _directions[currentGraphX - 1][currentGraphY] == 0 && (_clip[currentGraphX - 1][currentGraphY] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_EAST | Flags.CORNER_NORTH_EAST)) == 0 && (_clip[currentGraphX - 1][currentGraphY + (size - 1)] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_EAST | Flags.WALL_SOUTH | Flags.CORNER_SOUTH_EAST)) == 0) {
                exit:
                {
                    for (int y = 1; y < (size - 1); y++) {
                        if ((_clip[currentGraphX - 1][currentGraphY + y] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_EAST | Flags.WALL_SOUTH | Flags.CORNER_NORTH_EAST | Flags.CORNER_SOUTH_EAST)) != 0) {
                            break exit;
                        }
                    }
                    // we can go to west, queue it
            ...
            if (currentGraphX < (GRAPH_SIZE - size) && _directions[currentGraphX + 1][currentGraphY] == 0 && (_clip[currentGraphX + size][currentGraphY] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_WEST | Flags.CORNER_NORTH_WEST)) == 0 && (_clip[currentGraphX + size][currentGraphY + (size - 1)] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_SOUTH | Flags.WALL_WEST | Flags.CORNER_SOUTH_WEST)) == 0) {
                exit:
                {
                    for (int y = 1; y < (size - 1); y++) {
                        if ((_clip[currentGraphX + size][currentGraphY + y] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_SOUTH | Flags.WALL_WEST | Flags.CORNER_NORTH_WEST | Flags.CORNER_SOUTH_WEST)) != 0) {
                            break exit;
                        }
                    }
                    // we can go to east, queue it
            ...
            if (currentGraphY > 0 && _directions[currentGraphX][currentGraphY - 1] == 0 && (_clip[currentGraphX][currentGraphY - 1] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_EAST | Flags.CORNER_NORTH_EAST)) == 0 && (_clip[currentGraphX + (size - 1)][currentGraphY - 1] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_WEST | Flags.CORNER_NORTH_WEST)) == 0) {
                exit:
                {
                    for (int y = 1; y < (size - 1); y++) {
                        if ((_clip[currentGraphX + y][currentGraphY - 1] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_EAST | Flags.WALL_WEST | Flags.CORNER_NORTH_WEST | Flags.CORNER_NORTH_EAST)) != 0) {
                            break exit;
                        }
                    }
                    // we can go to south, queue it
            ...
            if (currentGraphY < (GRAPH_SIZE - size) && _directions[currentGraphX][currentGraphY + 1] == 0 && (_clip[currentGraphX][currentGraphY + size] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_EAST | Flags.WALL_SOUTH | Flags.CORNER_SOUTH_EAST)) == 0 && (_clip[currentGraphX + (size - 1)][currentGraphY + size] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_SOUTH | Flags.WALL_WEST | Flags.CORNER_SOUTH_WEST)) == 0) {
                exit:
                {
                    for (int y = 1; y < (size - 1); y++) {
                        if ((_clip[currentGraphX + y][currentGraphY + size] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_EAST | Flags.WALL_SOUTH | Flags.WALL_WEST | Flags.CORNER_SOUTH_EAST | Flags.CORNER_SOUTH_WEST)) != 0) {
                            break exit;
                        }
                    }
                    // we can go to north, queue it
            ...
            // diagonal checks, comment them to disable diagonal routes.
            if (currentGraphX > 0 && currentGraphY > 0 && _directions[currentGraphX - 1][currentGraphY - 1] == 0 && (_clip[currentGraphX - 1][currentGraphY - 1] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_EAST | Flags.CORNER_NORTH_EAST)) == 0) {
                exit:
                {
                    for (int y = 1; y < size; y++) {
                        if ((_clip[currentGraphX - 1][currentGraphY + (y - 1)] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_EAST | Flags.WALL_SOUTH | Flags.CORNER_NORTH_EAST | Flags.CORNER_SOUTH_EAST)) != 0 || (_clip[currentGraphX + (y - 1)][currentGraphY - 1] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_EAST | Flags.WALL_WEST | Flags.CORNER_NORTH_WEST | Flags.CORNER_NORTH_EAST)) != 0) {
                            break exit;
                        }
                    }
                    // we can go to south west, queue it
            ...
            if (currentGraphX < (GRAPH_SIZE - size) && currentGraphY > 0 && _directions[currentGraphX + 1][currentGraphY - 1] == 0 && (_clip[currentGraphX + size][currentGraphY - 1] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_WEST | Flags.CORNER_NORTH_WEST)) == 0) {
                exit:
                {
                    for (int y = 1; y < size; y++) {
                        if ((_clip[currentGraphX + size][currentGraphY + (y - 1)] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_SOUTH | Flags.WALL_WEST | Flags.CORNER_NORTH_WEST | Flags.CORNER_SOUTH_WEST)) != 0 || (_clip[currentGraphX + y][currentGraphY - 1] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_EAST | Flags.WALL_WEST | Flags.CORNER_NORTH_WEST | Flags.CORNER_NORTH_EAST)) != 0) {
                            break exit;
                        }
                    }
                    // we can go to south east, queue it
            ...
            if (currentGraphX > 0 && currentGraphY < (GRAPH_SIZE - size) && _directions[currentGraphX - 1][currentGraphY + 1] == 0 && (_clip[currentGraphX - 1][currentGraphY + size] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_EAST | Flags.WALL_SOUTH | Flags.CORNER_SOUTH_EAST)) == 0) {
                exit:
                {
                    for (int y = 1; y < size; y++) {
                        if ((_clip[currentGraphX - 1][currentGraphY + y] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_EAST | Flags.WALL_SOUTH | Flags.CORNER_NORTH_EAST | Flags.CORNER_SOUTH_EAST)) != 0 || (_clip[currentGraphX + (y - 1)][currentGraphY + size] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_EAST | Flags.WALL_SOUTH | Flags.WALL_WEST | Flags.CORNER_SOUTH_EAST | Flags.CORNER_SOUTH_WEST)) != 0) {
                            break exit;
                        }
                    }
                    // we can go to north west, queue it.
            ...
            if (currentGraphX < (GRAPH_SIZE - size) && currentGraphY < (GRAPH_SIZE - size) && _directions[currentGraphX + 1][currentGraphY + 1] == 0 && (_clip[currentGraphX + size][currentGraphY + size] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_SOUTH | Flags.WALL_WEST | Flags.CORNER_SOUTH_WEST)) == 0) {
                exit:
                {
                    for (int y = 1; y < size; y++) {
                        if ((_clip[currentGraphX + y][currentGraphY + size] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_EAST | Flags.WALL_SOUTH | Flags.WALL_WEST | Flags.CORNER_SOUTH_EAST | Flags.CORNER_SOUTH_WEST)) != 0 || (_clip[currentGraphX + size][currentGraphY + y] & (Flags.FLOOR | Flags.FLOOR_DECORATION | Flags.OBJECT | Flags.WALL_NORTH | Flags.WALL_SOUTH | Flags.WALL_WEST | Flags.CORNER_NORTH_WEST | Flags.CORNER_SOUTH_WEST)) != 0) {
                            break exit;
                        }
                    }
                    // we can go to north east, queue it.
     ...
     */
    private void transmitClipData(final int x, final int y, final int z) {
            ...
            for (int transmitRegionY = graphBaseY >> 6; transmitRegionY <= (graphBaseY + (GRAPH_SIZE - 1)) >> 6; transmitRegionY++) {
                ...
                if (map == null || region.getLoadStage() != 2) {
                    for (int fillX = startX; fillX < endX; fillX++) {
                        final int[] clips = clip[fillX - graphBaseX];
                        for (int fillY = startY; fillY < endY; fillY++) {
                            clips[fillY - graphBaseY] = -1;
                        }
                    }
                    continue;
                }
                ...
                final int[] masks = map.getMasks();
                for (int fillX = startX; fillX < endX; fillX++) {
                    final int[] clips = clip[fillX - graphBaseX];
                    for (int fillY = startY; fillY < endY; fillY++) {
                        final int maskIndex = RegionMap.getMaskIndex(z, fillX & 0x3F, fillY & 0x3F);
                        clips[fillY - graphBaseY] = masks[maskIndex];
                    }
                }
            }
        }
     ...
     */
    protected int[] getLastPathBufferX() {
        return bufferX;
    }
     ...
     */
    protected int[] getLastPathBufferY() {
        return bufferY;
    }
