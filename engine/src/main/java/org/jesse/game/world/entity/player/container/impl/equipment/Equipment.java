package org.jesse.game.world.entity.player.container.impl.equipment;
...
import org.jesse.game.world.entity.player.action.combat.AttackStyleDefinition;
...
import org.jesse.game.world.region.area.plugins.EquipmentPlugin;
...
public class Equipment extends ContainerWrapper implements TempPlayerStatePlugin.State {
	private static final Logger log = NearRealityLogger.getLogger(Equipment.class);
	...
	private void onRefresh() {
		player.setTorvaBoosted(CombatUtilities.hasFullTorva(player));
	}
	...
	@Override
	public void refresh(final int slot) {
		super.refresh(slot);
		onRefresh();
		player.getUpdateFlags().flag(UpdateFlag.APPEARANCE);
	}
	...
	public final boolean wear(final int slotId) {
		...
		final DiaryReward reward = DiaryReward.get(id);
		...
		switch (equipmentSlot) {
			...
			case 13: {//Ammo
				if (CombatUtilities.hasQuiverEquipped(player) && PlayerAttributesKt.getDizanasQuiverAmmo(player) == id) {
					final int wornAmount = PlayerAttributesKt.getDizanasQuiverAmmoAmount(player);
					final int itemAmount = item.getAmount();
					if (itemAmount + wornAmount < 0) {
						final int amountToAdd = Integer.MAX_VALUE - wornAmount;
						if (amountToAdd > 0) {
							PlayerAttributesKt.setDizanasQuiver(player, id, Integer.MAX_VALUE);
						...
						PlayerAttributesKt.setDizanasQuiver(player, id, wornAmount + itemAmount);
		...
		if (definitions.isStackable() && worn != null && worn.getId() == id) {
			...
			if (itemAmount + wornAmount < 0) {
				...
				set(equipmentSlot, new Item(id, wornAmount + itemAmount));
				refresh(equipmentSlot);
				inventory.set(slotId, null);
				inventory.refresh();
			}
	...
	public static int getAttackAnimation(final int weaponId, final int attackType) {
		if (weaponId == -1) {
        ...
        return switch (attackType) {
            case 0 -> definitions.getAccurateAnimation();
            case 1 -> definitions.getAggressiveAnimation();
            case 2 -> definitions.getControlledAnimation();
            default -> definitions.getDefensiveAnimation();
        };
	}
	...
	public static int getIndexByButton(final int interfaceId, final int componentId) {
		if (interfaceId == 84) {
	...
	public final class EquipmentContainer extends Container {
		...
		public void set(final int slot, final Item item) {
			...
			if (player != null) {
				final RegionArea area = player.getArea();
				if (area instanceof ContainerPlugin plugin) {
