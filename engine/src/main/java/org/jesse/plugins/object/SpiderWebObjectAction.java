...
public final class SpiderWebObjectAction implements ObjectAction {
    ...
    public static void slash(@NotNull final Player player, @NotNull final WorldObject web, @NotNull final Item weapon) {
        ...
        if (player.getAraneaBoots().isPlayerWearingBoots()) {
            var movementDirection = getMovingDirection(player, web);
            player.addWalkSteps(movementDirection, 1, 1, false);
            player.sendMessage("You step through the web...");
            return;
        }
        ...
        if (chance >= roll)
            WorldTasksManager.schedule(() ->
                World.spawnTemporaryObject(
                    new WorldObject(ObjectId.SLASHED_WEB, web.getType(), web.getRotation(), web),
                    web, 100
                )
            );
    }

    private static Direction getMovingDirection(Player player, WorldObject web) {
        var webLocation = web.getLocation();
        var playerLocation = player.getLocation();
        if (webLocation.matches(playerLocation))
            return web.getFaceDirection();
        return Direction.getDirection(playerLocation, webLocation);
