...
import org.jesse.game.content.area.prifddinas.zalcano.combat.impl.*;
...
import org.jesse.game.util.LocationUtil;
...
import static org.jesse.game.content.skills.thieving.actions.Pickpocket.STUN_GFX;
...
public class ZalcanoBoss extends NPC implements CombatScript, Spawnable {
    ...
    public boolean canAttack(Player source) {
        if (phase==ZalcanoPhase.DEAD) return false;

        if (isShieldPhase()) {
            // If we have tephra
            if (source.getEquipment().containsItem(ZalcanoConstants.IMBUED_TEPHRA_ITEM_ID))
    ...
    public void processNPC() {
        ...
        if (stopTick != -1 && downedTimer >= stopTick) {
    ...
    public void setTransformation(int id) {
        ...
        if (id == ZalcanoConstants.ZALCANO_MINABLE) {
            addHitbar(removeShieldBar);
        } else {
            addHitbar(removeRegularHpBar);
        }
        addHitbar(hitBar);
    }
