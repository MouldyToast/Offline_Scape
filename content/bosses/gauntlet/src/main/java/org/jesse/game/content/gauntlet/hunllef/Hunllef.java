...
import org.jesse.game.world.entity.npc.NpcOverhead;
...
public final class Hunllef extends NPC implements CombatScript {
            ...
            public void stop() {
                ...
                if (prayedCorrectly) {
                    if (gauntletType == GauntletType.STANDARD)
                        source.getCombatAchievements().complete(CAType.three_2_1__RANGE);
                    else
                        source.getCombatAchievements().complete(CAType.three_2_1__MAGE);
                }
                var tier = GauntletArmorTier.get(source, gauntletType);
                if (tier.isPresent() && tier.get() == GauntletArmorTier.PERFECTED)
                    source.getCombatAchievements().complete(CAType.CORRUPTED_WARRIOR);

                if (gauntlet.attunedWeaponCount < 2)
                    source.getCombatAchievements().complete(CAType.WOLF_PUNCHER);

                if (gauntletType == GauntletType.STANDARD) {
                    if (!source.getBooleanTemporaryAttribute("egniol_potion_made"))
                        source.getCombatAchievements().complete(CAType.EGNIOL_DIET);
                    if (!gauntlet.armorMade)
                        source.getCombatAchievements().complete(CAType.DEFENCE_DOESNT_MATTER);
                }
                else {
                    if (!gauntlet.armorMade)
                        source.getCombatAchievements().complete(CAType.DEFENCE_DOESNT_MATTER_II);
                }
    ...
    private void updateOverheadPrayers() {
        switch (type) {
            case MELEE_RESISTANT -> setPrayer(Prayer.PROTECT_FROM_MELEE);
            case MAGIC_RESISTANT -> setPrayer(Prayer.PROTECT_FROM_MAGIC);
            case RANGED_RESISTANT -> setPrayer(Prayer.PROTECT_FROM_MISSILES);
        }
    }

    private void setPrayer(Prayer prayer) {
        switch (prayer) {
            case PROTECT_FROM_MELEE -> setOverhead(NpcOverhead.MELEE);
            case PROTECT_FROM_MAGIC -> setOverhead(NpcOverhead.MAGE);
            case PROTECT_FROM_MISSILES -> setOverhead(NpcOverhead.RANGE);
        }
    ...
    private void spawnTilePattern(Player target) {
        ...
        if (randomPattern == null) {
            getLogger().warn("No spawn tile found... randomPattern was null");
            return;
        }
