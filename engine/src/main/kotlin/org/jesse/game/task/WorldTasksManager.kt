package org.jesse.game.task

import org.jesse.game.util.MessagePassingQueues.drain
import org.jesse.game.task.WorldTasksManager.NO_PERIOD_DELAY
import org.jesse.game.task.WorldTasksManager.active
...
object WorldTasksManager {
     ...
     */
    private val logger: Logger = NearRealityLogger.getLogger(WorldTasksManager::class.java)
    ...
    fun scheduleCreation(
        ...
        val creationException = Exception()
        return schedule(
            object : WorldTask {
                override fun run() {
                    try {
                ...
                override fun printCreationStackTrace() {
    ...
    @JvmStatic
    fun containsTask(worldTask: WorldTask): Boolean {
        synchronized(lock) {
            return continuations.containsKey(worldTask)
        }
    }
