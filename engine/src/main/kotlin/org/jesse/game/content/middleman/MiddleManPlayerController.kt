...
import org.jesse.game.GameInterface.MIDDLE_MAN_REQUEST
...
class MiddleManPlayerController(val player: Player) {
    ...
    fun openTradeRequestInterface(offerItemId: Int = -1, offerAmount: Int = -1, targetDisplayName: String) {
        ...
        if (player.bankPin.requiresVerification(player) { openTradeRequestInterface(offerItemId, offerAmount, targetDisplayName) }) return
    ...
    fun confirmRequestOrTrade() {
        ...
        when (val oldState = this.state) {
            MiddleManState.MakingRequest -> {
                ...
                player.sendMessage("Sent a middle-man trade request to ${target.username}.")
