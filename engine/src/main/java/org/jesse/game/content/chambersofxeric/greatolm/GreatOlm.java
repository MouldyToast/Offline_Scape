...
import org.jesse.game.content.chambersofxeric.greatolm.scripts.AcidDrip;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.AcidSpray;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.CrystalBomb;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.CrystalBurst;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.DeepBurn;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.FallingCrystal;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.FireWall;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.LeftClawProtection;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.LifeSiphon;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.Lightning;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.Sphere;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.StandardAttack;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.Swap;
import org.jesse.game.content.chambersofxeric.greatolm.scripts.TransitionalFallingCrystals;
import org.jesse.game.content.chambersofxeric.npc.RaidNPC;
import org.jesse.game.content.chambersofxeric.rewards.RaidRewards;
import org.jesse.game.content.chambersofxeric.score.Scoreboard;
...
import org.jesse.game.content.skills.magic.spells.lunar.Humidify;
...
import org.jesse.game.world.entity.BeigeProgressiveHitBar;
...
import static org.jesse.game.content.skills.farming.actions.Watering.*;
...
public final class GreatOlm extends RaidNPC<OlmRoom> implements CombatScript {
   private static final Logger log = NearRealityLogger.getLogger(GreatOlm.class);
   /**
   ...
   public static void douseFirewall(final Player player, final NPC npc) {
      player.getRaid().ifPresent(raid -> raid.ifInRoom(player.getLocation(), OlmRoom.class, room -> {
         ...
         for (int canId = ItemId.WATERING_CAN8; canId >= ItemId.WATERING_CAN1; canId--) {
            if (player.getInventory().containsItem(canId)) {
               player.faceEntity(npc);
               player.getInventory().deleteItem(canId, 1);
               int newId = (canId == ItemId.WATERING_CAN1) ? ItemId.WATERING_CAN : (canId - 1);
         ...
         public void run() {
            ...
            if (loop == 0) {
               try {
                  ...
                  room.setLeaveTime(WorldThread.getCurrentCycle());
