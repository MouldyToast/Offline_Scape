...
import org.jesse.game.content.boss.wildernessbosses.callisto.Callisto;
...
import org.jesse.game.world.entity.player.action.combat.melee.SoulreaperCombat;
import org.jesse.game.world.entity.player.action.combat.special.BurningBarrageSpecial;
...
import static org.jesse.game.world.entity.player.action.combat.SpecialAttackScript.*;
import static org.jesse.game.world.entity.player.action.combat.SpecialType.*;
...
public enum SpecialAttack implements ISpecialAttack {
    QUICK_SMASH(AttackType.CRUSH, new int[] {
        4153,
        12848,
        24227,
        24225,
        20557
    }, 0, MELEE, new Animation(1667), new Graphics(340, 0, 96), (player, combat, target) -> {
        combat.delayHit(0, combat.getHit(player, target, 1, 1, 1, false));
        player.sendSound(QUICKSMASH_SOUND);
    }),

    CLEAVE(AttackType.SLASH, new int[] {
        DRAGON_LONGSWORD,
        DRAGON_LONGSWORD_CR,
        DRAGON_LONGSWORD_BH
    }, WEAPON_SPEED, MELEE, new Animation(1058), new Graphics(248, 0, 100), (player, combat, target) -> {
    ...
    VIRULENCE(AttackType.SLASH, NOXIOUS_HALBERD, WEAPON_SPEED, MELEE, new Animation(-1), new Graphics(2930), new VirulenceSpecial()),

    PULVERIZE(AttackType.CRUSH, new int[] {
        ELDER_MAUL,
        ELDER_MAUL_OR
    }, WEAPON_SPEED, MELEE,
        ...
        if (target instanceof Tekton tekton) {
            if (player.getBooleanAttribute("firstElderMaulSpecOnTekton")) {
                tekton.drainSkill(SkillConstants.DEFENCE, 35.0);
                player.putBooleanAttribute("firstElderMaulSpecOnTekton", false);
                shouldApplyHit = true; // Ensure the first special always hits
            }

            if (shouldApplyHit) {
                combat.delayHit(0, hit);
                if (hit.getDamage() > 0) {
                    tekton.drainSkill(SkillConstants.DEFENCE, player.getBooleanAttribute("firstElderMaulSpecOnTekton") ? 0 : 5.0);
                }
            }
        } else if (shouldApplyHit) {
            combat.delayHit(0, hit);
            if (hit.getDamage() > 0) {
    ...
    DESCENT_OF_DARKNESS(AttackType.RANGED, new int[] {
        CORRUPTED_DARK_BOW,
        11235,
        12765,
        12766,
        ...
        20408,
        DARK_BOW_BH
    }, 8, RANGED, new Animation(426), new Graphics(1112, 0, 95), (player, combat, target) -> {
        ...
        if (combat instanceof RangedCombat ranged && bow != CORRUPTED_DARK_BOW) {
    ...
    DUALITY(AttackType.RANGED, new int[] {
        22804,
        22806,
        22808,
        22810,
        22812,
        22814
    }, WEAPON_SPEED, RANGED, null, null, (player, combat, target) -> {
    ...
    PUNCTURE(AttackType.SLASH, new int[] {
        1215,
        1231,
        5680,
        5698,
        ...
        DRAGON_DAGGER_CR,
        DRAGON_DAGGER_PCR,
        DRAGON_DAGGER_PCR_28023,
        DRAGON_DAGGER_PCR_28025
    }, WEAPON_SPEED, MELEE, new Animation(1062), new Graphics(252, 0, 100), (player, combat, target) -> {
    ...
    SEVER(AttackType.SLASH, new int[] {
        4587,
        20000,
        20406,
        DRAGON_SCIMITAR_CR
    }, WEAPON_SPEED, MELEE, new Animation(1872), new Graphics(347, 0, 100), (player, combat, target) -> {
    ...
    SHATTER(AttackType.SLASH, new int[] {
        DRAGON_MACE,
        DRAGON_MACE_CR,
        DRAGON_MACE_BH
    }, WEAPON_SPEED, MELEE, new Animation(1060), new Graphics(251, 0, 100), (player, combat, target) -> {
    ...
    SMASH(AttackType.CRUSH, new int[] {
        DRAGON_WARHAMMER,
        DRAGON_WARHAMMER_CR,
    ...
    SNAPSHOT(AttackType.RANGED, new int[] {
        861,
        20558,
        12788
    }, 2, RANGED, new Animation(1074), new Graphics(250, 25, 95), (player, combat, target) -> {
    ...
    THE_JUDGEMENT(AttackType.SLASH, new int[] {
        CORRUPTED_ARMADYL_GODSWORD,
        ItemId.ARMADYL_GODSWORD,
        20593,
        20368
    }, WEAPON_SPEED, MELEE,
        null, null, (player, combat, target) -> {
        ...
        var anim = JUDGEMENT_ANIM;
        if (weaponId == 20368)
            anim = ORNAMENT_JUDGEMENT_ANIM;
    ...
    HEALING_BLADE(AttackType.SLASH, new int[] {
        11806,
        20372
    }, WEAPON_SPEED, MELEE, null, new Graphics(1209), (player, combat, target) -> {
        final int weaponId = player.getWeapon() == null ? -1 : player.getWeapon().getId();
    ...
    PENANCE(AttackType.CRUSH, 13263, WEAPON_SPEED, MELEE, new Animation(3299), null, (player, combat, target) -> {
        ...
        final Hit hit = new Hit(player, combat.getRandomHit(player, target, maxHit + extra, 1), HitType.MELEE);
    ...
    TUMEKENS_LIGHT(AttackType.STAB, ItemId.KERIS_PARTISAN_OF_THE_SUN, WEAPON_SPEED, MELEE, TUMEKENS_LIGHT_ANIM, TUMEKENS_LIGHT_GFX, (player, combat, target) -> {
        ...
        for (final int skill : RESTORED_SKILLS) {
    ...
    EVISCERATE(AttackType.SLASH, new int[] {
        ItemId.OSMUMTENS_FANG,
        ItemId.OSMUMTENS_FANG_OR
    }, WEAPON_SPEED, MELEE, EVISCERATE_ANIM, EVISCERATE_GFX, (player, combat, target) -> {
        World.sendSoundEffect(player, OSMUMTEN_FANG_SOUND);
        combat.delayHit(0, combat.getHit(player, target, 1.50F, 1.0F, 1.00F, false));
    }),

    SLICE_AND_DICE(AttackType.SLASH, new int[] {
        CORRUPTED_DRAGON_CLAWS,
        DRAGON_CLAWS,
        DRAGON_CLAWS_20784,
        DRAGON_CLAWS_OR,
        DRAGON_CLAWS_CR
    }, WEAPON_SPEED, MELEE, new Animation(7514, 10), new Graphics(1171, 10, 0), (player, combat, target) -> {
        int probability = -1;
        for (int i = 0; i < 4; i++) {
    ...
    RAMPAGE(AttackType.SLASH, new int[] {
        DRAGON_BATTLEAXE,
        DRAGON_BATTLEAXE_CR
    }, WEAPON_SPEED, MELEE, new Animation(1056), new Graphics(246), (player, combat, target) -> {
    ...
    SHOVE(AttackType.SLASH, new int[] {
        1249,
        1263,
        3176,
        5716,
        ...
        DRAGON_SPEAR_CR,
        DRAGON_SPEAR_PCR,
        DRAGON_SPEAR_PCR_28045,
        DRAGON_SPEAR_PCR_28047
    }, WEAPON_SPEED, MELEE, new Animation(1064), new Graphics(253, 0, 96), (player, combat, target) -> {
        ...
        if (target.getSize() == 1) {
            ...
            if (target instanceof Player tp) {
                ...
                if (WorldThread.getCurrentCycle() < lastShoveTick) {
                    return;
                }
                tp.getTemporaryAttributes().put("Last shove push", WorldThread.getCurrentCycle() + 7);
            }
            if (px > target.getX()) {
                x--;
            }
    ...
    ABYSSAL_PUNCTURE(AttackType.STAB,
        new int[] {
            ABYSSAL_DAGGER,
            ABYSSAL_DAGGER_P,
            ABYSSAL_DAGGER_P_13269,
            ABYSSAL_DAGGER_P_13271,
            ABYSSAL_DAGGER_BH,
            ABYSSAL_DAGGER_BHP,
            ABYSSAL_DAGGER_BHP_27865,
            ABYSSAL_DAGGER_BHP_27867
        },
    ...
    CHAINHIT(AttackType.RANGED, 805, 4, RANGED, new Animation(1068), new Graphics(257, 0, 90), (player, combat, target) -> {
        ...
        final int maxHit = combat.getMaxHit(player, 1, 1, false);
        final Hit hit = new Hit(player, combat.getRandomHit(player, target, maxHit, 1), HitType.RANGED);
        if (hit.getDamage() == maxHit) {
            hit.setMax(true);
        }
        ...
        if (player.isMultiArea()) {
            WorldTasksManager.scheduleOrExecute(new WorldTask() {
                ...
                public void run() {
                    ...
                    int delay = CHAINHIT_CHAIN_PROJ.getTime(previousTarget.getLocation(), t.getLocation());
                    ...
                    combat.delayHit(t, (delay < 0 ? (delay + 1) : delay), new Hit(player, combat.getRandomHit(player, target, maxHit, 1),
                        HitType.RANGED).putAttribute("chainhit_prev", t).setSpecialAttack());
                }
            }, CHAINHIT_PROJ.getTime(player.getLocation(), target.getLocation()) - 1, 0);
        }
    }),

    SWEEP(AttackType.SLASH, new int[] {
        DRAGON_HALBERD_CR,
        3204,
        13080,
        13081,
        ...
        13100,
        13101,
        CrystalWeapon.Halberd.INSTANCE.getProductItemId(),
    ...
    POWERSTAB(AttackType.SLASH, new int[] {
        DRAGON_2H_SWORD,
        DRAGON_2H_SWORD_20559,
        DRAGON_2H_SWORD_CR
    }, WEAPON_SPEED, MELEE, new Animation(3157), new Graphics(1214), (player, combat, target) -> {
        final List<Entity> possibleTargets = player.getPossibleTargets(EntityType.BOTH);
    ...
    ROCK_KNOCKER(AttackType.SLASH, new int[] {
        11920,
        12797,
        13243,
        20014,
    ...
    }, 0, MELEE, null, null, (player, combat, target) -> {
        final int weaponId = player.getWeapon() == null ? -1 : player.getWeapon().getId();
    ...
    LUMBER_UP(AttackType.SLASH, new int[] {
        6739,
        13241,
        20011,
        13242,
    ...
    }, 0, MELEE, new Animation(2876), null, (player, combat, target) -> {
        final int weaponId = player.getWeapon() == null ? -1 : player.getWeapon().getId();
    ...
    FISHSTABBER(AttackType.STAB, new int[] {
        21028,
        21031,
        21033,
        CrystalTool.Harpoon.INSTANCE.getProductItemId(),
        CrystalTool.Harpoon.INSTANCE.getInactiveId()
    }, 0, MELEE, null, null, (player, combat, target) -> {
        final int weaponId = player.getWeapon() == null ? -1 : player.getWeapon().getId();
        if (weaponId == 21028) {
    ...
    MOMENTUM_THROW(AttackType.RANGED, new int[] {
        20849,
        21207
    }, 4, RANGED, new Animation(7521), new Graphics(1317, 0, 96), (player, combat, target) -> {
    ...
    WILD_STAB(AttackType.STAB, new int[] {
        21009,
        21206,
        DRAGON_SWORD_CR
    }, WEAPON_SPEED, MELEE, new Animation(7515), new Graphics(1369, 0, 96), (player, combat, target) -> {
        final Hit hit = combat.getHit(player, target, 1.25, 1.25, 1, true);
        player.sendSound(WILD_STAB_SOUND);
        combat.delayHit(0, hit);
    }),

    UNLEASH(AttackType.STAB, new int[] {
        ItemId.DRAGON_HASTA,
        ItemId.DRAGON_HASTAKP,
        ItemId.DRAGON_HASTAP,
        ItemId.DRAGON_HASTAP_22737,
        ItemId.DRAGON_HASTAP_22740
    }, WEAPON_SPEED, MELEE, new Animation(7515), new Graphics(1369, 0, 96), (player, combat, target) -> {
    ...
    WARSTRIKE(AttackType.SLASH, new int[] {
        11804,
        20370,
        20782,
        21060
    }, WEAPON_SPEED, MELEE, null, new Graphics(1212), (player, combat, target) -> {
        final int weaponId = player.getWeapon() == null ? -1 : player.getWeapon().getId();
    ...
    ICE_CLEAVE(AttackType.SLASH, new int[] {
        ItemId.ZAMORAK_GODSWORD,
        ItemId.ZAMORAK_GODSWORD_OR
    }, WEAPON_SPEED, MELEE, null, new Graphics(1210), (player, combat, target) -> {
        final int weaponId = player.getWeapon() == null ? -1 : player.getWeapon().getId();
    ...
    SUNDER(AttackType.CRUSH, new int[] {
        BARRELCHEST_ANCHOR,
        BARRELCHEST_ANCHOR_BH
    }, WEAPON_SPEED, MELEE, new Animation(5870), new Graphics(1027), (player, combat, target) -> {
        var weaponId = player.getWeapon() == null ? -1 : player.getWeapon().getId();
    ...
    WEAKEN(AttackType.STAB, new int[] {
        6746,
        19675
    }, WEAPON_SPEED, MELEE, new Animation(2890), new Graphics(483), (player, combat, target) -> {
        combat.delayHit(0, combat.getHit(player, target, 1, 1, 1, false));
    ...
    BACKSTAB(AttackType.STAB, new int[] {
        8872,
        8874,
        8876,
        8878
    }, WEAPON_SPEED, MELEE, new Animation(4198), new Graphics(704), (player, combat, target) -> {
        double accuracy = 1;
    ...
    SARADOMINS_LIGHTNING(AttackType.SLASH, ItemId.SARADOMIN_SWORD, WEAPON_SPEED, MELEE, new Animation(1132), new Graphics(1213), (player, combat, target) -> {
        ...
        final int maxHit = combat.getMaxHit(player, 1.1, 1, false);

        final int meleeDamage = combat.getRandomHit(player, target, maxHit, 1, AttackType.MAGIC);
        final int magicDamage = Utils.random(1, 16);
        Hit meleeHit = new Hit(player, meleeDamage, HitType.MELEE);
        if (meleeHit.getDamage() == maxHit) {
            meleeHit.setMax(true);
        }
        Hit magicHit = new Hit(player, magicDamage, HitType.MAGIC);
        if (magicHit.getDamage() == 16) {
            magicHit.setMax(true);
        }
    ...
    BLESSED_SARADOMINS_LIGHTNING(AttackType.SLASH, new int[] {
        12808,
        12809
    }, WEAPON_SPEED, MELEE, new Animation(1133), new Graphics(1213), (player, combat, target) -> {
        target.setGraphics(SARADOMINS_LIGHTNING_GFX);
    ...
    POWERSHOT(AttackType.RANGED, new int[] {
        859,
        10284
    }, 5, RANGED, new Animation(426), new Graphics(250, 0, 95), (player, combat, target) -> {
        World.sendProjectile(player, target, SNAPSHOT_FIRST_PROJ);
        ...
        final Hit hit = new Hit(player, Utils.random(1, damage), HitType.RANGED);
    ...
    SOULSHOT(AttackType.RANGED, 6724, 4, RANGED, new Animation(426), new Graphics(472, 0, 90), (player, combat, target) -> {
        ...
        combat.delayHit(SOULSHOT_PROJ.getTime(player.getLocation(), target.getLocation()), hit);
    ...
    CONCENTRATED_SHOT(AttackType.RANGED, new int[] {
        ItemId.LIGHT_BALLISTA,
        ItemId.HEAVY_BALLISTA,
        ItemId.HEAVY_BALLISTA_OR
    }, 6, RANGED, new Animation(7222), null, (player, combat, target) -> {
    ...
    SPEAR_WALL(AttackType.CRUSH, new int[] {
        VESTAS_SPEAR,
        VESTAS_SPEAR_BH
    }, WEAPON_SPEED, MELEE, new Animation(8184), new Graphics(1627), (player, combat, target) -> {
    ...
    FEINT(AttackType.STAB, new int[] {
        VESTAS_LONGSWORD,
        VESTAS_LONGSWORD_BH
    }, WEAPON_SPEED, MELEE, new Animation(246), null, (player, combat, target) -> {
        ...
        if (hit.getDamage() > 0) {
            var maxHit = combat.getMaxHit(player, 1, 1, false);
            var min = (int) (maxHit * 0.2F);
            var max = (int) (maxHit * 1.2F);
            hit.setDamage(Utils.random(min, max));
        }
        player.sendSound(FEINT_SOUND);
        combat.delayHit(target, 0, hit);
    }),

    SWH_SMASH(AttackType.CRUSH, new int[] {
        STATIUSS_WARHAMMER,
        STATIUSS_WARHAMMER_BH
    }, WEAPON_SPEED, MELEE, new Animation(1378), new Graphics(844), (player, combat, target) -> {
    ...
    HAMSTRING(AttackType.RANGED, new int[] {
        MORRIGANS_THROWING_AXE,
        MORRIGANS_THROWING_AXE_BH
    }, 4, RANGED, new Animation(929), new Graphics(1626, 0, 90), (player, combat, target) -> {
    ...
    PHANTOM_STRIKE(AttackType.RANGED, new int[] {
        MORRIGANS_JAVELIN,
        MORRIGANS_JAVELIN_BH
    }, 4, RANGED, new Animation(806), new Graphics(1621, 0, 90), (player, combat, target) -> {
    ...
    ANNIHILATE(AttackType.RANGED, new int[] {
        DRAGON_CROSSBOW,
        DRAGON_CROSSBOW_CR
    }, 5, RANGED, new Animation(4230), null, (player, combat, target) -> {
    ...
    IMMOLATE(AttackType.MAGIC, new int[] {
        ...
        new Animation(8532), new Graphics(1760), (player, combat, target) -> {
    ...
    SWARM(AttackType.RANGED, new int[] {27655}, WEAPON_SPEED, RANGED, new Animation(9964), new Graphics(2354, 0, 90), (player, combat, target) -> {
        ...
        if (firstHit.getDamage() == max) {
            firstHit.setMax(true);
        }
    ...
    @Override
    public boolean testPredicate(@NotNull Player player) {
        return true;
    }
}
