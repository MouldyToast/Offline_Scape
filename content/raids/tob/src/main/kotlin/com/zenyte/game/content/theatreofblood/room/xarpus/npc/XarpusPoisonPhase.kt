...
internal class XarpusPoisonPhase(xarpus: Xarpus) : XarpusPhase(xarpus) {
    ...
    override fun onTick() {
        if (ticks >= startDelay && ticks % attackInterval == 0) {
            val partyNames = xarpus.room.raid.party.members
            if (partyNames.isEmpty()) {
                return
            }

            val oldTarget = (xarpus.combat.target as? Player)
            val oldIndexInList = if (oldTarget != null) {
                partyNames.indexOf(oldTarget.username)
            } else {
                -1
            }

            var idx = (oldIndexInList + 1).let { if (it < 0) 0 else it } % partyNames.size

            var newTarget: Player? = null
            repeat(partyNames.size) {
                val candidate = World.getPlayerByUsername(partyNames[idx])
                if (candidate != null && xarpus.room.isValidTarget(candidate)) {
                    newTarget = candidate
                    return@repeat
                }
                idx = (idx + 1) % partyNames.size
            }

            val target = newTarget ?: return
            xarpus.setTarget(target)
            xarpus.animation = attackAnim
            sendPoisonOrb(Location(xarpus.middleLocation), target.location.copy(), newTarget, false)
        }
    }
