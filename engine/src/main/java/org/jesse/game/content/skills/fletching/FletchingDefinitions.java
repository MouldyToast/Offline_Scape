package org.jesse.game.content.skills.fletching;
...
public class FletchingDefinitions {
	...
	public static final double[][] EXPERIENCE = new double[][] {
		{5, 5, 5, 10, 6},
		{10, 16.5, 25, 16, 50},
		{15, 33.3, 41.5, 22, 83},
		{20, 50, 58.3, 32, 116.5},
	...
	public enum AmmunitionFletchingData {
		...
		public String getProcessMessage(final AmmunitionFletchingData data, final int sets, final int quantity, final int amount) {
            return switch (data.getType()) {
                case DART ->
                    "You attach " + (amount == 1 ? "a feather" : "feathers") + " to " + (amount == 1 ? "an" : amount >= sets ? sets : quantity) + " " + data.getMaterials()[0].getDefinitions().getName().toLowerCase() + (amount > 1 && !data.getMaterials()[0].getDefinitions().getName().endsWith("s") && !data.getMaterials()[0].getDefinitions().getName().endsWith("(unf)") ? "s." : ".");
                case JAVELIN ->
                    "You attach " + (amount == 1 ? "a javelin head" : "javelin heads") + " to " + (amount == 1 ? "an" : amount >= sets ? sets : quantity) + " " + data.getMaterials()[1].getDefinitions().getName().toLowerCase() + (amount > 1 && !data.getMaterials()[1].getDefinitions().getName().endsWith("s") && !data.getMaterials()[1].getDefinitions().getName().endsWith("(unf)") ? "s." : ".");
                case TIPPED_BOLT ->
                    "You attach " + (amount == 1 ? "a bolt tip" : "bolt tips") + " to " + (amount == 1 ? "an" : amount >= sets ? sets : quantity) + " " + data.getMaterials()[1].getDefinitions().getName().toLowerCase() + (amount > 1 && !data.getMaterials()[1].getDefinitions().getName().endsWith("s") && !data.getMaterials()[1].getDefinitions().getName().endsWith("(unf)") ? "s." : ".");
                default ->
                    "You attach " + (amount == 1 ? "a feather" : "feathers") + " to " + (amount == 1 ? "an" : amount >= sets ? sets : quantity) + " " + data.getMaterials()[0].getDefinitions().getName().toLowerCase() + (amount > 1 && !data.getMaterials()[0].getDefinitions().getName().endsWith("s") && !data.getMaterials()[0].getDefinitions().getName().endsWith("(unf)") ? "s." : ".");
            };
		}
