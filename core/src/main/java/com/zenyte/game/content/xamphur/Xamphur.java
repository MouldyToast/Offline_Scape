...
public class Xamphur extends NPC implements CombatScript {
    ...
    private static final Logger LOGGER = NearRealityLogger.getLogger(Xamphur.class);

    public static Location center = new Location(3360, 7071);
    ...
    public void handleIngoingHit(final Hit hit) {
        ...
        if (hit.getHitType().equals(HitType.MAGIC) || !handNpcs.isEmpty()) {
            hit.setDamage(0);
        }

        if (hit.getSource() instanceof Player player && !handNpcs.isEmpty()) {
    ...
    public void finishFight() {
        ...
        WorldBroadcasts.sendMessage("<img=" + 22 + "><col=" + "e59400" + ">" + "<shad=000000>" + "Event: Xamphur has been defeated! Only 150 more votes until Xamphur returns!", BroadcastType.XAMPHUR, false);
    ...
    public int attack(Entity target) {
        ...
        if(currentPhase == SpecialAttack.HANDS_MINION && handNpcs.isEmpty()) {
            despawnHands();
            sendHands(players);
            attackCounter = 0;
            currentPhase = null;
