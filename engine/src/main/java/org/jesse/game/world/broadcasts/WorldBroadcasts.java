package org.jesse.game.world.broadcasts;

import org.jesse.api.model.ApiGameMode;
import org.jesse.tools.logging.ChallengeLog;
import org.jesse.tools.logging.ChallengeLogger;
...
public class WorldBroadcasts {
    ...
    public static final String[] HELPFUL_TIPS = {
            ...
            "Did you know: You can imbue items such as slayer helmets and dagannoth rings using Scroll of Imbuing purchasable from the slayer rewards shop.",
            "Did you know: You can use regular teleports, fairy rings, spirit trees, gnome gliders and the teleport interface to travel around the world.",
            "Did you know: You can complete daily challenges to get cash and experience.",
...
//    public static void handleDiscordBroadcast(Player player, BroadcastType type, Object... args) {
//        switch (type) {
//            case RARE_DROP -> sendRareDrop(player, args);
//            case LVL_99 -> sendLvl99(player, args);
//            case MAXED -> sendMaxed(player);
//            case XP_200M -> sendXp200M(player, args);
//            case PET -> sendPet(player, args);
//            case GAMBLE_FIRECAPE -> sendTzokPet(player);
//            case HCIM_DEATH -> sendHCIMDeath(player, args);
//            case GROUP_HCIM_DEATH -> sendGroupHCIMDeath(player, args);
    ...
    public static void broadcast(final Player player, final BroadcastType type, final Object... args) {
        ...
        switch (type) {
            ...
            case MYSTERY_BOX_RARE_ITEM: {
                if (!(args[0] instanceof Integer) || !(args[1] instanceof String boxName)) return;
            ...
            case TREASURE_TRAILS: {
                if (!(args[0] instanceof Integer) || !(args[1] instanceof String)) return;
                final int id = Integer.parseInt(args[0].toString());
                if (!BroadcastedTreasure.isBroadcasted(id)) return;
                ...
                secondaryBuilder.append(" from ").append(Utils.getAOrAn(tier)).append(" ")
                        .append(tier).append(" clue scroll on casket ")
            ...
            case RARE_DROP: {
                ...
                if (rewardFrom.endsWith("Chambers of Xeric")) {
                    ...
                    secondaryBuilder.append("from ")
                            .append(rewardFrom)
                            .append(" on chest ")
                            .append("#").append(totalCompletions);
                }
            ...
            case LVL_99: {
                if (!(args[0] instanceof Integer skill)) return;
            ...
            case MAXED: {
                ...
                if(player.getGameMode().toApi() == ApiGameMode.ULTIMATE_IRON_MAN)
                    gamemode = "uim";
                if(player.getCombatXPRate() == 5)
                    gamemode = "realist";
                boolean iron;
                if(player.isIronman() && !player.isGroupIronman())
                    iron = true;
                else {
                    iron = false;
                }
                String finalGamemode = gamemode;
                ChallengeLogger.log(Level.INFO, () -> new ChallengeLog.MaxedAccount(player.getDbUsername(), finalGamemode, iron));
                break;
            }
            case XP_200M: {
                if (!(args[0] instanceof Integer skill)) return;
            ...
            case PET: {
                if (!(args[0] instanceof Pet pet)) return;
                builder.append("News: ");
                secondaryBuilder.append(player.getGameModeCrown().getCrownTag());
                secondaryBuilder.append(player.getName());
            ...
            case HCIM_DEATH: {
                if (player.getSkills().getTotalLevel() < 500) return;
                builder.append("News: ");
                secondaryBuilder.append(player.getGameModeCrown().getCrownTag());
                secondaryBuilder.append(player.getName());
            ...
            case GROUP_HCIM_DEATH: {
                ...
                if (remainingLives > 0) {
                    ...
                    if (remainingLives > 1) secondaryBuilder.append("lives");
                    else secondaryBuilder.append("life");
            ...
            case HELPFUL_TIP: {
                if (!(args[0] instanceof String tip)) return;
                builder.append(tip);
                break;
            }
...
//            DiscordBroadcastKt.broadcast(DiscordCommunityBot.INSTANCE, player, type, args);
    ...
    public static boolean isGlove(String name) {
        return name.contains("gloves") || name.contains("vambraces")
