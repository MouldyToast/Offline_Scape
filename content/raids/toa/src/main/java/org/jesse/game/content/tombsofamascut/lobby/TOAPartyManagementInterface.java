...
import org.jesse.game.content.tombsofamascut.TOAPartySettings;
...
public class TOAPartyManagementInterface extends Interface {
	...
	@Override protected void attach() {
		...
		for (int slotId = 36; slotId < 52; slotId++) {
			put(1, slotId, "Applicants "  + slotId);
		}
		for (int slotId = 52; slotId < 52 + InvocationType.VALUES.length; slotId++) {
	...
	@Override protected void build() {
		...
		for (int slot = 36; slot < 52; slot++) {
			bind("Applicants " + slot, (player, slotId, itemId, option) -> {
				boolean accept = slotId < 44;
				final TOALobbyParty viewingParty = checkViewingParty(player);
				if (viewingParty == null) {
					return;
				}
				final Player p = viewingParty.getApplicant(slotId - (accept ? 36 : 44));
		...
		for (int slot = 52; slot < 52 + InvocationType.VALUES.length; slot++) {
