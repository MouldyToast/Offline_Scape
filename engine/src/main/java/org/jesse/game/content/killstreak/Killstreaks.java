package org.jesse.game.content.killstreak;
...
public class Killstreaks {
    ...
    public static void onServerLaunchEvent(final ServerLaunchEvent event) {
        event.getWorldThread().getHooks().register(PlayerEvent.Died.class, it -> {
            final Entity killer = it.getKiller();
            if (!(killer instanceof Player attacker)) {
                return;
            }
            handlePlayerKill(attacker, it.getPlayer());
        });
    }
    ...
    public static void handlePlayerKill(Player attacker, Player victim) {
        ...
        attacker.sendMessage("You have received " + Colour.RED.wrap(totalBloodmoney) + " blood money. Your killstreak is now " + Colour.RED.wrap(attackerCurrentStreak) + ".");
        attacker.getInventory().addOrDrop(new Item(ItemId.BLOOD_MONEY, totalBloodmoney));
        ...
        announceKillstreak(attacker, attackerCurrentStreak);
    }
    ...
    private static int getTotalBloodmoneyForKill(Player attacker, Player victim) {
        ...
        if (level == 0)
            return 0;
        return getBloodmoneyForKill((float) (killstreakBoost * 1.2), level) + shutdownBonus;
    }

    private static int getBloodmoneyForKill(float percentageBoost, int wildernessLevel) {
    ...
    private static double getRankBoost(MemberRank rank) {
        return switch (rank) {
            case TOPAZ -> 0.05;
            case SAPPHIRE -> 0.08;
            case EMERALD -> 0.12;
