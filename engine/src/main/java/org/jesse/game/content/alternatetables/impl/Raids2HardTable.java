...
public class Raids2HardTable implements AlternateTableDropProvider {
    ...
    private void buildEntries() {
        ...
        genUnique(ItemId.AVERNIC_DEFENDER_HILT, 3);
        genUnique(ItemId.GHRAZI_RAPIER, 3);
        genUnique(ItemId.SANGUINESTI_STAFF_UNCHARGED, 3);
        genUnique(ItemId.JUSTICIAR_FACEGUARD, 3);
        genUnique(ItemId.JUSTICIAR_CHESTGUARD, 3);
        genUnique(ItemId.JUSTICIAR_LEGGUARDS, 3);
        genUnique(ItemId.SCYTHE_OF_VITUR_UNCHARGED, 1);
        genUnique(ItemId.HOLY_ORNAMENT_KIT, 1);
        genUnique(ItemId.SANGUINE_ORNAMENT_KIT, 1);
        ...
        entries.add(new OtherDropViewerEntry(ItemId.SANGUINE_DUST, 1, 1, 1, 225, ""));
    }

    @Override
    public String getName() {
        return "Raids - ToB Speed";
    }
    ...
    private void genUnique(int id, int top) {
        entries.add(new OtherDropViewerEntry(id, 1, 1, top, 21 / 0.30, "This item's rate assumes no deaths and maximum participation points by all party members in each encounter."));
    }
}
