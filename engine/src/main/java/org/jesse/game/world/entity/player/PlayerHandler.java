...
import org.jesse.game.world.entity.player.action.PeltAction;
...
public final class PlayerHandler {
    ...
    private static final Logger logger = NearRealityLogger.getLogger(PlayerHandler.class);
    ...
    public static void handle(final Player player, final Player p2, final boolean forcerun, final int optionId) {
        ...
        if (option.equals("Pelt")) {
            if (player.getNumericTemporaryAttribute("pelt lock").longValue() >= WorldThread.getCurrentCycle()) {
                return;
            }
            player.addTemporaryAttribute("pelt lock", WorldThread.getCurrentCycle() + 4);
