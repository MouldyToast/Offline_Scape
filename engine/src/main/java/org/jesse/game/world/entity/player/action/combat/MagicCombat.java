...
import org.jesse.game.content.buffs.BuffCategory;
import org.jesse.game.content.buffs.BuffSubcategory;
...
import org.jesse.game.content.skills.magic.spells.arceuus.MarkOfDarknessEffectKt;
...
import org.jesse.game.world.entity.player.action.combat.magic.TumekensShadowCombat;
...
import org.jesse.plugins.item.TomeOfFire;
...
import static org.jesse.game.item.ids.ItemId.CHAOS_GAUNTLETS;
import static org.jesse.game.world.entity.player.action.combat.CombatUtilities.AHRIMS_SET_GFX;
import static org.jesse.game.world.entity.player.action.combat.CombatUtilities.isWieldingZurielsStaff;
...
public class MagicCombat extends PlayerCombat {
    ...
    public Hit getHit(Player player, Entity target, double accuracyModifier, double passiveModifier, double activeModifier, final boolean ignorePrayers) {
        final int maxHit = getMaxHit(player, passiveModifier, activeModifier, false);
        final int damage = getRandomHit(player, target, maxHit, accuracyModifier);
        final Hit hit = new Hit(player, damage, getHitType());
        if (damage == maxHit) {
            hit.setMax(true);
        }
        hit.setWeapon(spell);
    ...
    private static final CombatSpell[] fireSpells = new CombatSpell[]{CombatSpell.FIRE_BLAST, CombatSpell.FIRE_BOLT,
            CombatSpell.FIRE_STRIKE, CombatSpell.FIRE_SURGE, CombatSpell.FIRE_WAVE};
    ...
    private Integer getGodSpellDamage(int baseDamage) {
        final int cape = player.getEquipment().getId(EquipmentSlot.CAPE);
        if (spell == CombatSpell.CLAWS_OF_GUTHIX && (cape == 2413 || cape == 21793 || cape == 13335 || cape == 21784) ||
                spell == CombatSpell.SARADOMIN_STRIKE && (cape == 2412 || cape == 21791 || cape == 13331 || cape == 21776) ||
                spell == CombatSpell.FLAMES_OF_ZAMORAK && (cape == 2414 || cape == 21795 || cape == 13333 || cape == 21780)) {
            return 30;
        }
        return baseDamage;
    }

    private Boolean isElementalBoltSpell() {
        return spell == CombatSpell.EARTH_BOLT ||
                spell == CombatSpell.FIRE_BOLT ||
                spell == CombatSpell.WATER_BOLT ||
                spell == CombatSpell.WIND_BOLT;
    }


    @Override
    public int getMaxHit(Player player, double passiveModifier, double activeModifier, final boolean ignorePrayers) {
        double damage = baseDamage(); // Highest: 30

        damage = player.buffManager.applyBuffs(BuffCategory.MAGIC, BuffSubcategory.MAGIC_DAMAGE_OVERWRITE, damage);


        if (player.getEquipment().getId(EquipmentSlot.HANDS) == CHAOS_GAUNTLETS) {
            if (isElementalBoltSpell())
                damage += 3;
        }
        // base magic damage boost
        int bonus = player.getBonuses().getBonus(Bonuses.Bonus.MAGIC_DAMAGE); // Best Case: 19%

        // Add Virtus multiplier
        bonus = evalVirtusBoost(bonus); // Highest case: bonus= 15%
        // Add Tumeken's modifier
        bonus = evalTumekensBoost(bonus); // 19 * 3 = 57 | 19 * 4 = 76

        // Convert bonus from an integer value (%) into a multiplier
        double gearBoost = 1.0 + (Math.min(100.0, bonus) / 100.0); // 1 + (76/100=0.76) = 1.76

        // Step 2: Calculate prayer boost (separately if prayers are not ignored)
        ...
        // Apply both modifiers in sequence (additively, not compounding their effects unnecessarily)
        // Apply gear and prayer boosts together
        damage = (int) Math.floor(damage * gearBoost * prayerBoost); // 33*1.76=58*1.25 = 72.6

        // Step 3: Apply other situational modifiers
        double situationalModifier = 1.0;
        final int amuletId = player.getEquipment().getId(EquipmentSlot.AMULET);
        final String amuletName = ItemDefinitions.nameOf(amuletId).toLowerCase();

        if (amuletId == ItemId.AMULET_OF_AVARICE && CombatUtilities.isRevenant(target)) {
            situationalModifier += 0.2F;
        ...
        // Step 4: Apply additional passive modifiers
        if (isThammaronsSceptre() || isAccursedSceptre()) {
            if (target instanceof NPC && ((NPC) target).isInWilderness()) {
                final Item wep = player.getWeapon();
                if (wep.getCharges() > 1000) {
        ...
        damage = (int) Math.floor(damage * passiveModifier);

        // Step 5: Handle specific item or spell effects
        ...
        if (player.getTemporaryAttributes().containsKey("castle wars bracelet effect") && player.inArea("Castle wars instance")) {
            if (target instanceof Player) {
                ...
                if (targetWeapon == 4037 || targetWeapon == 4039) {
                    damage = (int) Math.floor(damage * 1.2F);
                }
            }
        }

        AncientScepterType scepterType = getAncientScepterType();
        if (scepterType != AncientScepterType.NONE && isSpellOfElement(spell, scepterType)) {
            damage = (int) Math.floor(damage * 1.1F);
        }

        if (CombatUtilities.hasFullMagicVoid(player, true)) {
            damage = (int) Math.floor(damage * 1.05F);
        }
    ...
    // calculate the boost for Virtus Magic Armour
    private int evalVirtusBoost(int initial) {
        // Check if we're not on the Ancient book, then return the initial boost
        if (player.getCombatDefinitions().getSpellbook() != Spellbook.ANCIENT)
            return initial;

        /*
         * Each piece of Virtus robes gives a 2% magic damage bonus for a total of 6%.
         * When using combat spells from Ancient Magicks, an additional 3% magic damage is given per piece,
         * taking the bonus to 5% per piece for a total of 15% for the full set.
         */
        int bonusAdded = 0;
        ...
        if (player.getChest() != null) {
            int chest = player.getChest().getId();
            if (chest == ItemId.VIRTUS_ROBE_TOP) bonusAdded += 3;
        }
    ...
    private boolean isTargetDead() {
        return target instanceof Player || castType == CastType.AUTO_CAST ? target.isDead() : (target.isDead() && ((NPC) target).getTimeOfDeath() != WorldThread.getCurrentCycle());
    }
    ...
    public int processAfterMovement() {
        ...
        if (Utils.random(3) == 0
                && (CombatUtilities.hasFullBarrowsSet(player, "Ahrim's")
                || CombatUtilities.hasFullBarrowsSet(player, "Echo ahrim's"))) {
    ...
    protected void hit(final int delay) {
        ...
        float passiveModifier = (amuletId == 12851 || amuletId == 12853) && (CombatUtilities.hasFullBarrowsSet(player, "Ahrim's") || CombatUtilities.hasFullBarrowsSet(player, "Echo ahrim's")) ? 1.3F : 1;
    ...
    private enum AncientScepterType {
        ICE, BLOOD, SHADOW, SMOKE, NONE;
    }

    private AncientScepterType getAncientScepterType() {
        int weaponId = player.getEquipment().getId(EquipmentSlot.WEAPON);
        if (weaponId == ItemId.ICE_ANCIENT_SCEPTRE_28262 || weaponId == ItemId.ICE_ANCIENT_SCEPTRE_L)
            return AncientScepterType.ICE;
        if (weaponId == ItemId.BLOOD_ANCIENT_SCEPTRE_28260 || weaponId == ItemId.BLOOD_ANCIENT_SCEPTRE_L)
            return AncientScepterType.BLOOD;
        if (weaponId == ItemId.SHADOW_ANCIENT_SCEPTRE_28266 || weaponId == ItemId.SHADOW_ANCIENT_SCEPTRE_L)
            return AncientScepterType.SHADOW;
        if (weaponId == ItemId.SMOKE_ANCIENT_SCEPTRE_28264 || weaponId == ItemId.SMOKE_ANCIENT_SCEPTRE_L)
            return AncientScepterType.SMOKE;
        return AncientScepterType.NONE;
    }

    private boolean isIceSpell(CombatSpell spell) {
        return spell == CombatSpell.ICE_RUSH || spell == CombatSpell.ICE_BURST ||
                spell == CombatSpell.ICE_BLITZ || spell == CombatSpell.ICE_BARRAGE;
    }

    private boolean isBloodSpell(CombatSpell spell) {
        return spell == CombatSpell.BLOOD_BLITZ || spell == CombatSpell.BLOOD_BARRAGE ||
                spell == CombatSpell.BLOOD_BURST || spell == CombatSpell.BLOOD_RUSH;
    }

    private boolean isShadowSpell(CombatSpell spell) {
        return spell == CombatSpell.TUMEKENS_SHADOW ||
                spell == CombatSpell.DARK_LURE || spell == CombatSpell.MARK_OF_DARKNESS;
    }

    private boolean isSmokeSpell(CombatSpell spell) {
        return spell == CombatSpell.SMOKE_BURST || spell == CombatSpell.SMOKE_BARRAGE;
    }

    private boolean isSpellOfElement(CombatSpell spell, AncientScepterType type) {
        switch (type) {
            case ICE:
                return isIceSpell(spell);
            case BLOOD:
                return isBloodSpell(spell);
            case SHADOW:
                return isShadowSpell(spell);
            case SMOKE:
                return isSmokeSpell(spell);
            default:
                return false;
        }
    }
