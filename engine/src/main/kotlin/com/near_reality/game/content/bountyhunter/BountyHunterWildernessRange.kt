...
enum class BountyHunterWildernessRange (val minimum: Int, val max: Int, val display: String) {
    ...
    companion object {
        private val RANGES: Set<BountyHunterWildernessRange> =
            Collections.unmodifiableSet(EnumSet.allOf(BountyHunterWildernessRange::class.java))

        @JvmStatic fun getForWildernessLevel(wildylvl: Int) : BountyHunterWildernessRange {
            return RANGES.stream()
                .filter {
                    wildylvl >= it.minimum && wildylvl <= it.max
                }
                .findFirst()
                .orElse(UNKNOWN)
        }
