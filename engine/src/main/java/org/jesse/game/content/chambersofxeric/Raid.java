...
import org.jesse.game.content.chambersofxeric.room.ScavengerRoom;
import org.jesse.game.content.chambersofxeric.score.Scoreboard;
import org.jesse.game.content.chambersofxeric.skills.RaidFarming;
import org.jesse.game.content.chambersofxeric.storageunit.PrivateStorage;
import org.jesse.game.content.chambersofxeric.storageunit.SharedStorage;
...
import org.jesse.game.content.consumables.drinks.GourdPotion;
...
import static org.jesse.game.GameInterface.RAID_OVERLAY;
...
public final class Raid {
    private static final Logger log = NearRealityLogger.getLogger(Raid.class);
    ...
    private static float getRaidsBoost(Player player) {
        return switch (player.getMemberRank()) {
            ...
            case RUBY -> 1.56F;
            case DIAMOND -> 1.6F;
            case DRAGONSTONE -> 1.65F;
            case ONYX, ZENYTE -> 1.7F;
            default -> 1.45F;
        };
    }
...
//                        RAID_HELPER_OVERLAY.open(player);
                        ...
                        RaidHelperOverlay.setVisibility(player, true);
...
//        RAID_HELPER_OVERLAY.open(player);
        ...
        WorldTasksManager.schedule(() -> {
            ...
            dispatcher.sendClientScript(948, 0, 255, 0, 0, 15);//Fade
            dispatcher.sendClientScript(1513);//fade in
//            dispatcher.sendClientScript(4689,  12, 3, 140, 270);
//            dispatcher.sendClientScript(4653, "Test_1|Test_2|Test_3", false);
            readd(player);
            player.unlock();
        }, 1);
    }
...
//        RAID_HELPER_OVERLAY.open(player);
    ...
    public void load() {
        ...
        party.getChannel().getMembers()
            .forEach(p -> p.sendMessage("<col=fc02e7>The raid has begun!</col>"));
        // Close the bank Chest
        var chest = getOpenBankChest();
        if (chest != null)
            closeBankChest(chest);
        if (!players.isEmpty()) {
            int level = 0;
            for (final Player player : players) {
    ...
    private WorldObject getOpenBankChest() {
        var ownerName = getParty().getPlayer();
        var optionalOwner = World.getPlayer(ownerName);
        var chest = new AtomicReference<WorldObject>();
        if (optionalOwner.isPresent()) {
            var owner = optionalOwner.get();
            var regionId = owner.getLocation().getRegionId();
            var regionObjects = World.getRegion(regionId).getObjects();
            regionObjects.values().forEach(worldObject -> {
                if (worldObject.getId() == 47420)
                    chest.set(worldObject);
            });
        }
        return chest.get();
    }

    private void closeBankChest(WorldObject chest) {
        chest.setId(47419);
    }
    ...
    public void leaveRaid(@NotNull final Player player, final boolean logout, final boolean destroyIfEmptyOnly) {
        ...
        player.getVarManager().sendVar(1427, -1);
        refreshTab();
        fadeOut(player, () -> {
