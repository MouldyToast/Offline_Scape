package org.jesse.cache_tool.dumping

import org.jesse.cache.group
import org.jesse.cache_tool.cacheTo
...
fun main() {
    ...
    blueColours.map {
        ...
        println("Encoded blue: ${hsbToColor(it)}")
    }
    newColours.map {
        colorToHsb(it).first.toShort()
    }.joinToString {
...
fun hsbToColor(hsb: Short) = hsbToColor(hsb.toInt())

fun hsbToColor(hsb: Int): Color {

//    var transparency = alpha?.toUByte()?.toDouble() ?: 255.0

    val hue = (hsb shr 10) and 0x3f
    val sat = (hsb shr 7) and 0x07
    val bri = (hsb and 0x7f)
    val awtCol = Color.getHSBColor(hue.toFloat() / 63, sat.toFloat() / 7, bri.toFloat() / 127)
    return awtCol
}
