...
public class Setting {
    ...
    private int getCurrentSettingValue(Player player) {
        return switch (type) {
            case TOGGLE -> getToggleSetting(player);
            case SLIDER -> getSlider(player);
            case DROPDOWN -> getDropdown(player);
            case KEYBIND -> getKeybind(player);
            default -> 0;
        };
    }
    ...
    private int getSlider(Player player) {
        return switch (structId) {
            case SettingStructs.VIEW_DISTANCE_STRUCT_ID -> throw new IllegalStateException("Enhanced client not supported.");
            case SettingStructs.MUSIC_VOLUME_STRUCT_ID, SettingStructs.SOUND_EFFECT_VOLUME_STRUCT_ID, SettingStructs.AREA_SOUND_VOLUME_STRUCT_ID,
                 SettingStructs.SCREEN_BRIGHTNESS_STRUCT_ID -> SettingVariables.getVariableValue(this, player) / 5;
            default -> SettingVariables.getVariableValue(this, player);
        };
    }
