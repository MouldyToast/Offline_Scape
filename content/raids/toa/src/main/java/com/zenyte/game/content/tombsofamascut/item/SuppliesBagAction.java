...
public class SuppliesBagAction extends ItemPlugin {
	...
	@Override public void handle() {
		...
		bind("Withdraw 1", (player, item, container, slotId) -> TOAAccess.getToaManager(player).withdrawSupplies(1));
		bind("Withdraw All", (player, item, container, slotId) -> TOAAccess.getToaManager(player).withdrawSupplies(28));
		bind("Resupply", (player, item, container, slotId) -> TOAAccess.getToaManager(player).reSupply());
	}
