...
import static org.jesse.game.model.ui.testinterfaces.advancedsettings.SettingVariables.SHOW_ACTIVITY_ADVISER_VARBIT_ID;
import static org.jesse.game.model.ui.testinterfaces.advancedsettings.SettingVariables.SHOW_DATA_ORBS_VARBIT_ID;
...
public final class VarManager {
    ...
    public static final JsonDeserializer<VarManager> deserializer() {
        return (json, typeOfT, context) -> {
            ...
            final org.jesse.game.world.entity.player.VarManager manager = new VarManager(null);
    ...
    public static final void onInitialization(final InitializationEvent event) {
        final org.jesse.game.world.entity.player.Player player = event.getPlayer();
        final org.jesse.game.world.entity.player.Player saved = event.getSavedPlayer();
    ...
    public VarManager(final Player player) {
        this.player = player;
        values = new int[30000];//CacheManager.getCache().getArchive(ArchiveType.CONFIGS).findGroupByID(GroupType.VARBIT).getHighestFileId() + 1];
    }
    ...
    public void sendVarInstantLowPri(final int id, final int value) {
        if (id == -1) {
            return;
        }
        values[id] = value;
        ...
        player.getPacketDispatcher().sendConfigLowPriority(id, value);
    }
    ...
    public void resetSendBitInstant(final int id, final int value) {
        sendBitInstant(id, 0);
        sendBitInstant(id, value);
    }
