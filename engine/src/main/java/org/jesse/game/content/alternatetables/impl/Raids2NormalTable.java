...
public class Raids2NormalTable implements AlternateTableDropProvider {
    ...
    private void buildEntries() {
        ...
        genUnique(ItemId.AVERNIC_DEFENDER_HILT, 1, 1, 8);
        genUnique(ItemId.GHRAZI_RAPIER, 1, 1, 3);
        genUnique(ItemId.SANGUINESTI_STAFF_UNCHARGED, 1, 1, 3);
        genUnique(ItemId.JUSTICIAR_FACEGUARD, 1, 1, 3);
        genUnique(ItemId.JUSTICIAR_CHESTGUARD, 1, 1, 3);
        genUnique(ItemId.JUSTICIAR_LEGGUARDS, 1, 1, 3);
        genUnique(ItemId.SCYTHE_OF_VITUR_UNCHARGED, 1, 1, 1);
    ...
    private void genUnique(int id, int min, int max, int top) {
        entries.add(new OtherDropViewerEntry(id, min, max, top, 24 / 0.20, "This item's rate assumes no deaths and maximum participation points by all party members in each encounter."));
    }
}
