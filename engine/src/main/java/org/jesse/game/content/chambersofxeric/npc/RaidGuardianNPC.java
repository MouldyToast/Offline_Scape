...
import org.jesse.game.content.chambersofxeric.room.GuardiansRoom;
...
public final class RaidGuardianNPC extends RaidNPC<GuardiansRoom> implements CombatScript {
	...
	@Override
	public void flinch() {
		if (flinchTime > WorldThread.getCurrentCycle()) {
			return;
		}
		final int attackSpeed = combatDefinitions.getAttackSpeed();
		combat.setCombatDelay(attackSpeed / 2);
		flinchTime = WorldThread.getCurrentCycle() + attackSpeed;
	}
