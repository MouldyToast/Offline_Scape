...
import org.jesse.game.world.Border
...
enum class BountyHunterHotspot(val zoneId: Int, val zoneName: String, vararg val boundaries: Boundary) {
    ...
    EDGEVILLE(
        -1,
        "Edgeville",
        Boundary(3041, 3522, 3105, 3560),
        Boundary(3101, 3549, 3108, 3522),
        Boundary(3123, 3522, 3102, 3530),
        Boundary(3103, 3540, 3117, 3529),
        Boundary(3122, 3530, 3116, 3532),
        Boundary(3116, 3535, 3121, 3532),
        Boundary(3117, 3538, 3119, 3535),
        Boundary(3107, 3547, 3115, 3540)
    );
    ...
    val OBJ_ID = 60010
    val spacing = 3
    private val edgevilleHotspotAlwaysActive = true
    ...
    fun checkPlayer(player: Player): Boolean {
        ...
        if (edgevilleHotspotAlwaysActive) {
            for (boundary in EDGEVILLE.boundaries) {
                if (boundary.isIn(player)) {
                    return true
                }
            }
    ...
    private fun getPositions(spot: BountyHunterHotspot) {
        for (border in getBorders(spot)) {
    ...
    companion object {
        private val HOTSPOTS: Set<BountyHunterHotspot> =
            Collections.unmodifiableSet(EnumSet.allOf(BountyHunterHotspot::class.java))
        ...
        fun getBorders(spot: BountyHunterHotspot): MutableList<Border> {
            ...
            when (spot) {
                ...
                EDGEVILLE -> {
                    borders.add(Border.ofX(3041, 3135, 3561))
                    borders.add(Border.ofY(3522, 3561, 3041))
                    borders.add(Border.ofY(3522, 3561, 3135))
                }
        ...
        fun random(): BountyHunterHotspot {
            val hs = HOTSPOTS.iterator().asSequence().shuffled().find { it.zoneId != -1 } ?: ZONE_1
            hs.spawnBoundary()
            return hs
        }

        fun randomExcludeCurrent(): BountyHunterHotspot {
            return HOTSPOTS.iterator().asSequence().shuffled()
                .find { it != BountyHunterController.currentHotspot && it.zoneId != -1 } ?: ZONE_1
        }
    }
