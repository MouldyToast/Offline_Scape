package org.jesse.game.content.chambersofxeric.rewards;
...
public final class RaidRewards {
	private static final Logger log = NearRealityLogger.getLogger(RaidRewards.class);
	/**
	...
	public RaidRewards(final Raid raid) {
		...
		players = new ObjectArrayList<>(size);
	...
	public void generate() {
		...
		final long totalSeconds =  TimeUnit.TICKS.toSeconds(ticks);
		if(originalPlayers == 1) {
			ChallengeLogger.log(Level.INFO, () -> new ChallengeLog.SoloCOXCMClear(players.stream().findFirst().get().getDbUsername(), (int) totalSeconds));
		}
	 ...
	 */
	private void rollRareRewardsVanilla() {
		...
		for (int i = 0; i < length; i++) {
			...
			try {
				...
				if (pet) {
					originalRewards.computeIfAbsent(player.getUsername(), __ -> new ObjectArrayList<>()).add(new Item(OLMLET));
				}
	...
	private void addReward(final Player player, final Item reward) {
		...
		switch(unnoted.getId()) {
			case TWISTED_BOW, OLMLET, METAMORPHIC_DUST:
		}
