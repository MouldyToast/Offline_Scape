...
public class VenatorBow extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        ...
        player.sendInputInt("How many charges do you wish to add to your " + bow.getName() + "? (0 - " + chargesToAdd + ")", value -> {
            if (value < 1) {
                return;
            }
            ...
            player.getInventory().ifDeleteItem(new Item(ItemId.ANCIENT_ESSENCE, addedCharges), () -> {
                ...
                bow.setCharges(bow.getCharges() + addedCharges);
                player.getDialogueManager().start(
					new ItemChat(player, bow,
						"You use " + addedCharges + " " + essence.getName() + " to charge your " + bow.getName() + ". It now has " + Utils.pluralizedFormattedColorized("charge", bow.getCharges(), Colour.RED) + ".")
				);
            });
        });
    }
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[] {
            ItemPair.of(ItemId.ANCIENT_ESSENCE, ItemId.VENATOR_BOW),
            ItemPair.of(ItemId.ANCIENT_ESSENCE, ItemId.VENATOR_BOW_UNCHARGED)
        };
    }
}
