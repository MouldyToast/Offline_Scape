...
public enum DonationToggle {
    ...
    PICKUP_KEYS(true) {
        ...
        public boolean matches(Item item) {
            return switch (item.getId()) {
                case ItemId.LARRANS_KEY, ItemId.BRIMSTONE_KEY, ItemId.CRYSTAL_KEY, ItemId.TOOTH_HALF_OF_KEY, ItemId.LOOP_HALF_OF_KEY -> true;
                default -> false;
            };
        }
    },
    ...
    public static final DonationToggle[] PICKUP = {PICKUP_DHIDE, PICKUP_HERBS, PICKUP_BONES, PICKUP_CLUES, PICKUP_GEMS, PICKUP_RUNES, PICKUP_ENSOULED_HEADS, PICKUP_TOKKUL, PICKUP_NUMULITE, PICKUP_ANCIENT_SHARDS, PICKUP_DARK_TOTEMS, PICKUP_KEYS};
    public static final DonationToggle[] NOTES = {NOTE_BONES, NOTE_DHIDES, NOTE_GEMS, NOTE_HERBS, NOTE_ENSOULED_HEADS};
    ...
    public static boolean processPickup(Player killer, Item item) {
        for (DonationToggle value : PICKUP) {
    ...
    public static void processNote(Player killer, Item item) {
        for (DonationToggle value : NOTES) {
            if(value.matches(item)) {
                if(!value.isEnabled(killer))
                    return;
                if(!value.chance(killer))
