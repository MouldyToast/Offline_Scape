...
public final class MagicWardrobe {
	...
	@Expose private final Map<Integer, int[]> items = new HashMap<>(MagicWardrobeData.VALUES.length);
	
	/**
	 * Takes a robe set from the magic wardrobe if possible.
	 * @param itemId the display item id of the set on the interface.
	 */
	public void takeSet(final int itemId) {
		MagicWardrobeData set = MagicWardrobeData.DISPLAY_MAP.get(itemId);
		if (set == null) set = MagicWardrobeData.MAP.get(itemId);
		if (set == null) {
			return;
		}
	...
	private void refresh() {
		...
		player.getPacketDispatcher().sendClientScript(3532, 3289, 1, 0);
	}

	/**
	 * Opens the magic wardrobe interface.
	 */
	public void open() {
		...
		player.getPacketDispatcher().sendClientScript(3532, 3289, 1, 1);
	}
