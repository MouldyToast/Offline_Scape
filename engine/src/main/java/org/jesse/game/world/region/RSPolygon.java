...
public final class RSPolygon {
    ...
    @NotNull
    public Location getRandomPosition(int attempts, @NotNull Predicate<Location> predicate) {
        Location position;
        int plane = planes.length == 0 ? 0 : Utils.random(planes);
        do {
            position = getRandomPosition(plane);
        } while (!predicate.test(getRandomPosition(plane)) && --attempts > 0);
        return position;
    }
