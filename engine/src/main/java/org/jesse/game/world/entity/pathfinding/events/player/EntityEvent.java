package org.jesse.game.world.entity.pathfinding.events.player;
...
public class EntityEvent extends RouteEvent<Player, EntityStrategy> {
	...
	public boolean processAfterMovement() {
		...
		if (!entity.hasWalkSteps()) {
			final RouteResult steps = RouteFinder.findRoute(entity, entity.getSize(), strategy, false);
			if (interruptUponApproach && calculateBufferDistance(steps) <= 4) {
				if (target instanceof NPC && ((NPC) target).isPathfindingEventAffected()) {
					target.freeze(entity.isRun() ? 1 : 2);
				}
	...
	private int calculateBufferDistance(final RouteResult steps) {
		int x = entity.getX();
		int y = entity.getY();
		int dx = 0;
		int dy = 0;
