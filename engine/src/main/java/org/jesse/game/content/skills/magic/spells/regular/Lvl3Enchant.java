 ...
 */
public class Lvl3Enchant implements ItemSpell {
	@Override
	public int getDelay() {
		return 2000;
	}
	...
	public boolean spellEffect(final Player player, final Item item, final int slot) {
		final JewelleryEnchantment.JewelleryEnchantmentItem data = JewelleryEnchantmentItem.getDataByMaterial(item, JewelleryEnchantmentType.LVL3_ENCHANTMENT);
		if (data != null && JewelleryEnchantment.check(player, data)) {
			...
			player.sendMessage("This spell can only be cast on ruby or topaz amulets, rings, necklaces, bracelets and on shapes in the Mage Training Arena.");
			return false;
		}
	}
