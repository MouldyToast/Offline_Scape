...
public class Container extends _Container<Item> {
    private static final Logger log = NearRealityLogger.getLogger(Container.class);
    ...
    public void set(final int slot, final Item item) {
        ...
        if (oldItem != null) {
            final float oldWeight = oldItem.getDefinitions().getWeight();
            if (oldWeight > 0) {
                weight -= oldWeight;
            }
        }
    ...
    public Map<Integer, Item> findAllByIds(int[] ids) {
        var map = new LinkedHashMap<Integer, Item>();
    ...
    public long calculateValue() {
        return items.values().stream().mapToLong(value -> value.getSellPrice() * ((long) value.getAmount())).sum();
    }
}
