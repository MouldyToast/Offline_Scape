package org.jesse.game.content.treasuretrails.coordinateutils;
...
import static org.jesse.game.content.treasuretrails.clues.CoordinateClue.*;
...
public class SextantInterface extends Interface {
    ...
    private final void refreshSextantPosition(@NotNull final Player player) {
        ...
        player.getPacketDispatcher().sendComponentPosition(getId(), getComponent("Grass"), BASE_VIEW_HORIZONTAL_COORD, getGrassVerticalPosition(player));
        player.getPacketDispatcher().sendComponentPosition(getId(), getComponent("Sun"), BASE_VIEW_HORIZONTAL_COORD, getSunVerticalPosition(player));
        player.getPacketDispatcher().sendComponentAnimation(getId(), getComponent("Arm"), SextantPosition.BASE_ARM_POSITION + armPosition);
        player.getPacketDispatcher().sendComponentAnimation(getId(), getComponent("Frame"), SextantPosition.BASE_FRAME_POSITION + sextantPosition.getFramePosition());
        player.getPacketDispatcher().sendComponentAnimation(getId(), getComponent("Index mirror"), SextantPosition.BASE_FRAME_POSITION + sextantPosition.getFramePosition());
    }
     ...
     */
    private final void move(@NotNull final Player player, @MagicConstant(intValues = {-1,
            1}) final int horizontalOffset, @MagicConstant(intValues = {-1, 0, 1}) final int verticalOffset) {
