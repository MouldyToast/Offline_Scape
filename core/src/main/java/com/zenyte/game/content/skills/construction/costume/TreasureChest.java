...
/**
 * @author Kris | 6. march 2018 : 2:29.40
...
public final class TreasureChest {
	...
	public void takeSet(final int itemId) {
		...
		player.sendMessage("You remove the " + val.toLowerCase() + " from the treasure chest.");
	...
	public boolean addSet(final int itemId) {
		final TreasureChestData set = TreasureChestData.MAP.get(itemId);
		if (set == null) {
			return false;
		}
		final int display = set.getDisplayItem();
		if (items.containsKey(display)) {
			player.sendMessage("Your treasure chest already contains that.");
			return false;
		}
		...
		player.sendMessage("You place the " + val.toLowerCase() + " into the treasure chest.");
	 ...
	 */
	private void notify(final TreasureChestData set) {
		final EnumDefinitions map = EnumDefinitions.get(380);
		...
		player.sendMessage("You need the following items to enqueue the " + val.toLowerCase() + " to your treasure chest:");
