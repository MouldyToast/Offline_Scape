...
import org.jesse.game.content.tombsofamascut.npc.IMovingWarden;
import org.jesse.game.content.tombsofamascut.npc.IWardenCore;
...
import org.jesse.game.world.entity.npc.combat.impl.CombatDummy;
...
import org.jesse.game.world.entity.npc.impl.Shade;
import org.jesse.game.world.entity.npc.impl.slayer.Vampyre;
...
import org.jesse.game.world.region.area.TzHaarCity;
...
public class CombatUtilities {
    ...
    private static final String[] DEMON_NPCS = {
            ...
            "ice demon", "duke sucullus", "tormented demon", "nezikchened"
    };
    ...
    private static final String[] BARROWS_NAMES = {"Verac's", "Ahrim's", "Karil's", "Dharok's", "Guthan's", "Torag's", "Echo ahrim's"};

    public static float getInquisitorSetBoost(final Player player) {
    ...
    public static boolean isAlwaysTakeMaxHit(@NotNull final Entity target, HitType hitType) {
    ...
    public static boolean isRevenant(@NotNull final Entity target) {
        ...
        return name.contains("revenant") || name.contains("combat dummy");
    }
    ...
    public static boolean isDraconic(@NotNull final Entity entity) {
        ...
        final String name = npc.getDefinitions().getName().toLowerCase();
        return (!name.contains("elvarg") && !name.contains("revenant")) &&
            (name.contains("dragon") ||
                name.contains("wyvern") ||
                name.contains("wyrm") ||
                name.contains("drake") ||
                name.contains("hydra") ||
                name.contains("great olm") ||
                name.contains("left claw") ||
                name.contains("right claw") ||
                name.contains("vorkath") ||
                name.contains("combat dummy"));
    }

    public static boolean isDagannoth(@NotNull final Entity entity) {
        if (!(entity instanceof NPC npc)) {
            return false;
        }
        final String name = npc.getDefinitions().getName().toLowerCase();
        return name.contains("dagannoth");
    }

    public static boolean isHydra(@NotNull final Entity entity) {
        if (!(entity instanceof NPC npc)) {
            return false;
        }
        final String name = npc.getDefinitions().getName().toLowerCase();
    ...
    public static boolean hasFullTorva(final Player player) {
        final int helm = player.getEquipment().getId(EquipmentSlot.HELMET);
        if (helm != ItemId.TORVA_FULLHELM && helm != ItemId.SANGUINE_TORVA_FULL_HELM) {
            return false;
        }
        final int body = player.getEquipment().getId(EquipmentSlot.PLATE);
        if (body != ItemId.TORVA_PLATEBODY && body != ItemId.SANGUINE_TORVA_PLATEBODY) {
            return false;
        }
        final int legs = player.getEquipment().getId(EquipmentSlot.LEGS);
        return legs == ItemId.TORVA_PLATELEGS || legs == ItemId.SANGUINE_TORVA_PLATELEGS;
    }
    ...
    public static boolean isWearingSerpentineHelmet(final Player player) {
        final int helm = player.getEquipment().getId(EquipmentSlot.HELMET);
        return switch (helm) {
            case ItemId.SERPENTINE_HELM, ItemId.MAGMA_HELM, ItemId.TANZANITE_HELM -> true;
            default -> false;
        };
    }

    public static boolean isWearingVenomDamagingEquipment(Player player) {
        ...
        return weaponName.contains("toxic staff of the dead");
    }

    public static boolean isWearingVenomMageEquipment(final Player player) {
        ...
        return weaponName.equalsIgnoreCase("trident of the swamp") && weapon.hasCharges();
    }
    ...
    public static boolean isWieldingZurielsStaff(final Player player) {
        final int weaponId = player.getEquipment().getId(EquipmentSlot.WEAPON);
        return isWieldingZurielsStaff(weaponId);
    }

    public static boolean isWieldingZurielsStaff(final int weaponId) {
    ...
    public static boolean isKerisAffected(@NotNull final Entity target) {
        ...
        final String name = npc.getDefinitions().getName().toLowerCase();
        return name.contains("kalphite") || name.contains("scarab") || name.contains("locust rider") || name.contains("combat dummy");
    }
    ...
    public static boolean isDemon(@NotNull final Entity target) {
        ...
        return ArrayUtils.contains(DEMON_NPCS, name)
                || (DeveloperCommands.INSTANCE.getEnableCombatDummyOther() && name.contains("combat dummy"))
    ...
    static boolean isDragonsBreath(PlayerCombat rangedCombat) {
        if (rangedCombat.target instanceof Player p) {
    ...
    public static boolean hasQuiverEquipped(Player player) {
        return hasActiveQuiverEquipped(player, false);
    }

    public static boolean hasActiveQuiverEquipped(Player player, boolean checkCharges) {
        Item cape = player.getCape();
        if (cape == null) {
            return false;
        }
