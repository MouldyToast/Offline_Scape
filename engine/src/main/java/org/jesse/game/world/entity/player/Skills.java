...
import org.jesse.game.world.region.area.SlayerTower;
import org.jesse.game.world.region.area.plugins.ExperiencePlugin;
...
import org.jesse.plugins.dialogue.LevelUpDialogue;
...
public final class Skills extends SkillConstants implements TempPlayerStatePlugin.State {
    ...
    public static int getLevelForXP(final int experience) {
        if (experience >= 13_034_431) {
            return 99;
        }

        final int searchResultIndex = Arrays.binarySearch(XP_TABLE, experience);
        if (searchResultIndex >= 0) {
            return searchResultIndex + 1;
        }
        final int level = -(searchResultIndex + 1);
    ...
    private static final int[] XP_TABLE = new int[126];

    static {
        int accumulatedXp = 0;
        for (int level = 1; level < XP_TABLE.length; level++) {
            accumulatedXp += (int) (level + 300 * Math.pow(2, level / 7.0));
            XP_TABLE[level] = accumulatedXp / 4;
        }
    }
    ...
    public int getLevelForXp(final int skill) {
        ...
        return getLevelForXP((int) experience);
    }
    ...
    public void addXp(final int skill, double exp, final boolean additionalBoost, final boolean fake, final boolean rollPet) {
        ...
        var maxExp = oldExperience >= 200_000_000;
        //The chance will be 1/5m xp on average on 1x xp rate.
        final double chance = baseXp > 2_500_000 ? 0.5F : (baseXp / 5_000_000);
        var odds = (int) (1.0F / chance) - 1;
        pet.roll(player, odds);
    }
    ...
    public enum ExperienceBoost {
        ...
        PROSPECTOR(SkillConstants.MINING, 0.005F, new BoostItem(0.004F, 12013), new BoostItem(0.008F, 12014, ItemId.VARROCK_ARMOUR_4), new BoostItem(0.006F, 12015), new BoostItem(0.002F, 12016)),
        ANGLER(SkillConstants.FISHING, 0.005F, new BoostItem(0.004F, 13258), new BoostItem(0.008F, 13259), new BoostItem(0.006F, 13260), new BoostItem(0.002F, 13261)),
        FARMER(SkillConstants.FARMING, 0.005F, new BoostItem(0.004F, 13646, 13647, 21253, 21254), new BoostItem(0.008F, 13642, 13643), new BoostItem(0.006F, 13640, 13641), new BoostItem(0.002F, 13644, 13645)),
        PYROMANCER(SkillConstants.FIREMAKING, 0.005F, new BoostItem(0.004F, RewardCrate.PYROMANCER_HOOD), new BoostItem(0.008F, RewardCrate.PYROMANCER_GARB), new BoostItem(0.006F, RewardCrate.PYROMANCER_ROBE), new BoostItem(0.002F, RewardCrate.PYROMANCER_BOOTS));
    ...
    public void refresh(final int skill) {
        ...
        if (skill == SkillConstants.HITPOINTS && player.isTorvaHpBoosted()) {
            player.getPacketDispatcher().sendSkillUpdateForce(skill, getLevelForXp(SkillConstants.HITPOINTS) + 21);
        }
        else {
    ...
    public void refresh() {
        ...
        for (int i = 0; i < SkillConstants.COUNT; i++) {
            if (i == SkillConstants.HITPOINTS && player.isTorvaHpBoosted()) {
                dispatcher.sendSkillUpdateForce(i, getLevelForXp(SkillConstants.HITPOINTS) + 21);
            }
            else {
    ...
    public void setSkills(final Skills skills) {
        for (int skillId = 0; skillId < SkillConstants.COUNT; skillId++) {
