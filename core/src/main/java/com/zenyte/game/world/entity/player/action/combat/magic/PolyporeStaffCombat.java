 ...
 */
public final class PolyporeStaffCombat extends MagicCombat {
	public PolyporeStaffCombat(final Entity target, final CombatSpell spell, final CastType type) {
		super(target, spell, type);
	}

	@Override
	protected int baseDamage() {
		return (int) Math.max(25, Math.floor((player.getSkills().getLevel(SkillConstants.MAGIC) / 3.0F) - 2));
	}
	...
	protected boolean canAttack() {
		...
		if (DegradableItem.getDefaultCharges(weapon.getId(), -1) != charges && charges <= 0) {
			player.sendMessage("Your polypore staff is out of charges.");
			return false;
		}
