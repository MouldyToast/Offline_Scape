...
import org.jesse.game.content.chambersofxeric.room.LizardmanShamanRoom;
...
import org.jesse.game.world.entity.npc.impl.LizardmanSpawn;
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					for (final Player p : CharacterLoop.find(tt, 2, Player.class, player -> !player.isDead() && (npc.isMultiArea() || player == target))) {
						...
						delayHit(-1, p, new Hit(npc, (int) (Utils.random(20, 35) * org.jesse.game.world.entity.npc.impl.LizardmanShaman.protectionModifier(p)), HitType.POISON).onLand(hit -> {
							if (Utils.random(3) == 0) {
