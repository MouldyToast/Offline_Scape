...
//        combatDefinitions.setAttackDefinitions(attackDefinitions);
    ...
    public int attack(Entity target) {
        if (!(target instanceof Player targetPlayer)) return 6;
        ...
        if (outgoingHitCount == 4) {
            ...
            WorldTasksManager.schedule(() -> {
                if (!targetPlayer.getAraneaBoots().isPlayerWebImmune()) {
                    targetPlayer.freeze(6);
                    targetPlayer.sendFilteredMessage("Sarachnis has webbed you in place!");
                }
            ...
            Set<Location> destinations = new HashSet<>(Objects.nonNull(walkDestinations) ? walkDestinations : getSpawnLocations());
