...
class TheatreOfBloodRewardsInterface : Interface() {
	...
	override fun build() {
		bind("Bank all") { player ->
			val container: Container = getContainer(player) ?: return@bind
			for (slot in 0 until (container.containerSize + 1)) {
				val item = container[slot] ?: continue
				player.bank.container.deposit(player, container, slot, item.amount)
			}
	...
	private fun getContainer(player: Player): Container? {
		val party = VerSinhazaArea.getParty(player) ?: return null
		party.raid ?: return null
		val room = player.area as? RewardRoom ?: return null
		val container: Container = room.playersLoot[player] ?: return null
