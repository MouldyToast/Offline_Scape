...
public abstract class ItemPlugin implements Plugin {
	...
	public static final ItemDeathHandler DEFAULT_ITEM_DEATH_HANDLER = (player, item, protectedCount, deepWilderness, pvp) -> {
		...
		boolean safeTradable;
		if (item.getDefinitions() != null) {
			safeTradable = item.getDefinitions().isGrandExchange();
		} else {
            GameLogger.log(Level.INFO, () -> new GameLogMessage.ServerError(Clock.System.INSTANCE.now(), "HIGH", "Null item definition for item id: " + item.getId() + " on player kill."));
            safeTradable = false;
		}

        plugin.lost(() -> List.of(item));
        if (safeTradable) {
            plugin.status(() -> pvp ? ItemDeathStatus.DROP_ON_DEATH : ItemDeathStatus.GO_TO_GRAVESTONE);
		} else {
