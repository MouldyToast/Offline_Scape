...
public class Akkha extends TOANPC implements CombatScript {
	...
	public static final SoundEffect[] QUADRANT_HIT_SOUNDS = {
		new SoundEffect(5591, 9),
		new SoundEffect(3887, 9),
		new SoundEffect(173, 9),
		new SoundEffect(156, 9)
	};
	...
	private final Queue<Runnable> memoryActions = new ArrayDeque<>();
	...
	@Override public void listenScheduleHit(Hit hit) {
		if (!HitType.HEALED.equals(hit.getHitType())) {
			...
			if ((id == MELEE_ID && !HitType.MAGIC.equals(hit.getHitType())) ||
				(id == RANGE_ID && !HitType.MELEE.equals(hit.getHitType())) ||
	...
	public static void sendProjectile(TOARaidArea area, final TOANPC shooter, boolean mage, CombatScript script) {
	...
	@Override public int attack(Entity target) {
		...
		if (attackCycles > 0 && --attackCycles <= 0) {
			...
			specialIndex++;
			if (!stayVigilant || Utils.random(4) == 0) {
				changeAttackStyle();
			}
		} else {
			...
			if (stayVigilant && Utils.random(3) == 0) {
				World.sendSoundEffect(getMiddleLocation(), CHANGE_STYLE_SOUND);
				changeAttackStyle();
			}
		}
