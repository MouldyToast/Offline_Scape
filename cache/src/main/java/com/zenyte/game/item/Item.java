...
public class Item extends _Item {
    ...
    public static ConcurrentHashMap<Integer, ItemConfig> itemConfigs = new ConcurrentHashMap<>();
    ...
    public boolean isRangedAmmo() {
        return getName().toLowerCase().contains(" bolts") ||
    ...
    public int getUnnotedId() {
        return getDefinitions().getUnnotedOrDefault();
    }
    ...
    public int getSellPrice() {
        if(itemConfigs.containsKey(id) && itemConfigs.get(id).getSellPrice() != -99) {
            return itemConfigs.get(id).getSellPrice();
        }
    ...
    public boolean isPinRestricted() {
        return !isTradable() ||
            itemBlacklist.contains(id) ||
            List.of(
                "fire cape",
                "inferno",
                "max cape",
