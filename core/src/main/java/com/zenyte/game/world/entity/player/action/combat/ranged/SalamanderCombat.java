...
public final class SalamanderCombat extends RangedCombat {
	...
	public SalamanderCombat(final Entity target) {
		super(target);
	}
	...
	public int processAfterMovement() {
		...
		final AmmunitionDefinition ammunitionDefinitions = ammunitionSource.getAmmunitionDefinition();
		final SalamanderCombat.Tar tar = Tar.map.get(ammunitionDefinitions);
		assert tar != null : "No tar found for ammunition " + ammunitionDefinitions;
		resetFlag();
	...
	public Hit getMeleeHit(final Player player, final Entity target, final double accuracyModifier, final double strengthModifier) {
		final int maxHit = getMeleeMaxHit(player, strengthModifier);
		final int damage = getRandomMeleeHit(player, target, maxHit, accuracyModifier);
