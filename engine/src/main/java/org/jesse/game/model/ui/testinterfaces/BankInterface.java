...
import org.jesse.game.world.entity.player.container.impl.BankUtil;
...
import static org.jesse.game.GameInterface.BANK;
...
public class BankInterface extends Interface implements SwitchPlugin {
    ...
    public void open(Player player) {
        if (player.getGameMode().equals(GameMode.ULTIMATE_IRON_MAN)) {
            player.getDialogueManager().start(new PlainChat(player, "As an Ultimate Iron Man, you can only bank Pets."));
        }
        ...
        final var interfaceId = getInterface().getId();
        final var varManager = player.getVarManager();
        final var interfaceHandler = player.getInterfaceHandler();
        final var bankInventory = InventoryDefinitions.get(ContainerType.BANK.getId());
        ...
        dispatcher.sendComponentSettings(BANK, getComponent("Tab display"), 0, 8, CLICK_OP1);
    ...
    protected void build() {
        ...
        bind("Tab display", (player, slotId, itemId, option) -> player.getBank().setDisplayType((slotId - 1) / 2));
        ...
        bind("Deposit equipment", (player, slotId, itemId, option) -> {
            if (player.getGameMode() == GameMode.ULTIMATE_IRON_MAN) {
                player.sendMessage("You cannot use this feature in Ultimate Iron Man mode.");
                return;
            }
        ...
        bind("Interact with tab", (player, slotId, itemId, option) -> BankUtil.safePermutation(player, bank -> {
            if (option == 1) {
        ...
        bind("Interact with item", (player, slotId, itemId, option) -> {
            ...
            if (id == 20594) {
                ...
                player.sendInputInt("How many would you like to withdraw?", value -> BankUtil.safePermutation(player, bank -> {
        ...
        bind("Interact with item", "Interact with item", ((player, fromSlot, toSlot) -> BankUtil.safePermutation(player, bank -> {
