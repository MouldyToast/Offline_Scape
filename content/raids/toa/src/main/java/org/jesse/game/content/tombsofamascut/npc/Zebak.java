...
import org.jesse.game.content.tombsofamascut.InvocationType;
import org.jesse.game.content.tombsofamascut.encounter.ZebakEncounter;
import org.jesse.game.content.tombsofamascut.raid.EncounterStage;
import org.jesse.game.content.tombsofamascut.raid.TOAPathType;
...
public class Zebak extends TOANPC implements CombatScript {
	...
	private static final Location BOULDER_MIN_LOCATION = new Location(3925, 5401);
	private static final Location BOULDER_MAX_LOCATION = new Location(3935, 5415);
	...
	private void handleNormalCombat(final Player[] players) {
		...
		if (!meleeTargets.isEmpty() && Utils.random(2) == 0) {
			...
			WorldTasksManager.schedule(zebakEncounter.addRunningTask(() -> {
				meleeTargets.stream().filter(p -> !p.isDying() && zebakEncounter.insideChallengeArea(p)).forEach(p -> {
					delayHit(this, 0, p, new Hit(this, getRandomMaxHit(this, getMaxHit(38), AttackType.SLASH, p), HitType.MELEE));
					if (Utils.random(3) == 0) {
				...
				if (++ticks == 0) {
					...
					if (boulderLocations != null) {
						...
						if (jugsLocations != null) {
							...
							if (boulderLocations != null && !poisonBoulderLocations.isEmpty())
	...
	private List<Location> getBoulderLocations() {
		...
		return possibleBoulderTiles.isEmpty() ? null : possibleBoulderTiles.subList(0, Math.min(possibleBoulderTiles.size(), boulderAmounts));
	}
			...
			if (updatedPlayers != null && updatedPlayers.length > 0) {
				...
				WorldTasksManager.schedule(zebakEncounter.addRunningTask(() -> {
					...
					for (Player player : updatedPlayers1) {
						var attackType = mage ? AttackType.MAGIC : AttackType.RANGED;
						var hitType = mage ? HitType.MAGIC : HitType.RANGED;
						var damage = getRandomMaxHit(Zebak.this, getMaxHit(16), attackType, player);
						var hit = new Hit(Zebak.this, damage, hitType);

						player.scheduleHit(Zebak.this, hit,-1);
					}
				}), 2);
			}
		}), 3);
