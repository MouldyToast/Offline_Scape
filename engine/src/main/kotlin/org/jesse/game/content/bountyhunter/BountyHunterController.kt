...
import org.jesse.game.content.bountyhunter.tasks.BountyHunterHotspotScheduler
import org.jesse.game.content.bountyhunter.tasks.BountyHunterPlayerCooldown
import org.jesse.game.content.bountyhunter.tasks.BountyHunterUpdatePlayerInterfaces
...
object BountyHunterController {
     ...
     */
    @JvmStatic val activeBountyZones: MutableList<Boundary> = mutableListOf(Boundary.ARTIO, Boundary.WILDERNESS, Boundary.WILDERNESS_UNDERGROUND1, Boundary.WILDERNESS_UNDERGROUND2, Boundary.WILDERNESS_EDGE_ISLAND)
    ...
    @JvmStatic val excludedBountyZones: MutableList<Boundary> = mutableListOf(
        ...
        Boundary.FEROX_ENCLAVE10, Boundary.SLAYER_BASEMENT, Boundary.EDGE_ISLAND_BANK
    )
    ...
    @JvmStatic fun process() {
        ...
        processOverheads()
        processEarningPotential()
    }

    /**
     * Executes the matchmaking process for players in a wilderness environment.
     *
     * This method facilitates pairing eligible players together for a bounty hunting scenario
     * by applying a series of matchmaking rules and conditions. The matchmaking process is
     * conducted in the following steps:
     *
     * 1. Invokes `handleEligibility()` to populate the matchmaking queue with eligible players
     *    who meet predefined conditions.
     * 2. Checks if there are fewer than two players in the matchmaking queue; if true, the process exits.
     * 3. Iteratively pairs players based on the size of the matchmaking queue, ensuring compliance
     *    with eligibility conditions such as:
     *    - Players must both be located within the wilderness.
     *    - Players must not have been recently targeting each other (`isNotTargetingLastUser`).
     *    - Players should have a compatible combat level range (`isWithinLevelRange`).
     * 4. Upon a valid match, the players are removed from the matchmaking queue, and a target
     *    is assigned to each using `assignTarget()`.
     */
    private fun matchMakingProcess() {
        handleEligibility()
        if (matchmakingPlayers.size < 2) return
        repeat(floor(matchmakingPlayers.size / 2.0).toInt()) {
            val pair = matchmakingPlayers.shuffled().take(2)
            val playerA = pair[0]
            val playerB = pair[1]
            if (playerA.getWildernessLevel() > 0 && playerB.getWildernessLevel() > 0) {
                if (isNotTargetingLastUser(playerA, playerB)) {
                    if (isWithinLevelRange(playerA, playerB))
                        if (matchmakingPlayers.containsAll(pair) && matchmakingPlayers.removeAll(pair))
                            assignTarget(playerA, playerB)
                }
            }
        }
    }
    ...
    /**
     * Handles the eligibility of players for bounty matchmaking in the wilderness.
     *
     * This method iterates through a list of eligible players in the wilderness and
     * evaluates their eligibility for bounty matchmaking based on a series of conditions.
     * Players who pass all the conditions are added to the matchmaking queue. During
     * this process, the player's bounty interface is displayed if it is missing, and their
     * target name is temporarily set to indicate that a target is being searched for.
     *
     * Conditions to be met for a player to be added to matchmaking:
     * - The player must not skip matchmaking due to their current state (e.g., nulled, finished, or dead).
     * - The player must not already have a bounty pair.
     * - The player must not have an active bounty cooldown.
     * - The player must not already be in bounty matchmaking.
     * - The player must be present in a wilderness level of 1 or higher.
     *
     * Eligible players are added to the `matchmakingPlayers` list for further processing.
     */
    private fun handleEligibility() {
        for (player in eligiblePlayersInWilderness) {
            player.showBountyInterfaceIfMissing()

            if (player.isDead || player.isFinished) continue
            if (player.skipMatchmakingDueToState()) continue
            if (player.isBountyPaired()) continue
            if (player.hasBountyCooldown()) continue
            if (player.inBHMatchmaking()) continue
            if (player.getWildernessLevel() < 1) continue
    ...
    private fun isNotTargetingLastUser(playerA: Player, playerB: Player): Boolean =
        !playerA.bountyHunterLastTarget.equals(playerB.username, true) &&
        !playerB.bountyHunterLastTarget.equals(playerA.username, true)

    private fun isWithinLevelRange(playerA: Player, playerB: Player): Boolean {
        val range: Int = min(playerA.bountyTargetLevelRange, playerB.bountyTargetLevelRange)
        return abs(playerA.combatLevel - playerB.combatLevel) <= range
    }
    ...
    private fun processEarningPotential() {
        World.getPlayers().filter(Objects::nonNull).forEach { player ->
            run {
                if (player.getWildernessLevel() < 1) return@forEach
                player.bountyEarningPotentialTicker.tick()
                player.bountyEarningPotentialDeathTicker.tick()
                if (player.bountyEarningPotentialTicker.finished && hasPair(player)) {
                    val basePoints = 2
                    player.bountyHunterEarningPotential += basePoints
                    player.bountyEarningPotentialTicker.reset()
                }
            }
        }
    }
    ...
    private fun updateWilderness() {
        ...
        World.getPlayers().filter(Objects::nonNull).forEach { player ->
            run {
                if (player.inActiveBountyZone())
                    player.updateWildernessRange()
     ...
     */
    @JvmStatic
    fun completeBounty(killer: Player, killed: Player) {
        /* Handle incrementing stats first */
        killer.bountyHunterKills++
        killer.bountyHunterKillstreak++

        handleBountyHunterPoints(killer)

        handleEmblem(killer, killed)

        unassignTarget(killer, false)
        updateInfoPanel(killer, true)

        // Earning potential
        processDeathEarningPotential(killed)
        processBountyCrate(killer)
    }

    private fun handleEmblem(killer: Player, killed: Player) {
        ...
        // If the killer killed their target, drop their best emblem
        if (killed.getBestEmblem().isPresent) {
            if (killed.inventory.deleteItem(killed.getBestEmblem().get().itemId, 1).succeededAmount == 1)
                World.spawnFloorItem(Item(BountyHunterEmblem.TIER_1.itemId), killer, killer.position)
        }
    }

    private fun handleBountyHunterPoints(killer: Player) {
        val basePoints = killer.getKillstreakBonus() + killer.getMilestonePoints()
        var totalPoints = basePoints + 3
    ...
    private fun processBountyCrate(killer: Player) {
        val earningPotential = 100.coerceAtMost(killer.bountyHunterEarningPotential)
        if (earningPotential < 20) return
        val create = getCrateForEarningPotential(earningPotential)
        if (create != null) {
            // Obtaining a Bounty Crate will reset your EP value to 0%.
            killer.bountyHunterEarningPotential = 0
            killer.bountyHunterEarningPotentialDeathEarned = 0
            killer.inventory.addOrDrop(create)
        }
    }

    private fun processDeathEarningPotential(killed: Player) {
        if (killed.bountyEarningPotentialDeathTicker.finished) {
            killed.bountyHunterEarningPotentialDeathEarned += 20
            // you may only accumulate a maximum of 60% EP in this manner.
            if (killed.bountyHunterEarningPotentialDeathEarned < 60) {
                killed.bountyHunterEarningPotential += 20
                killed.bountyEarningPotentialDeathTicker.reset()
            }
        }
    }
