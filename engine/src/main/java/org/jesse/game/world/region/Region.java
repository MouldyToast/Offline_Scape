...
import org.jesse.utils.efficientarea.Polygon;
...
public class Region {
    ...
    private org.jesse.utils.efficientarea.Area getRepositionedArea(final org.jesse.utils.efficientarea.Area area,
        ...
        return new org.jesse.utils.efficientarea.Area(polygon);
    }
    ...
    public final byte getTileHeight(final int plane, final int localX, final int localY) {
        if (map == null || loadStage != 2) {
            return -1;
        }
        return map.getLocalTileHeight(plane, localX & 63, localY & 63);
    }

    public final byte getMapSetting(final int plane, final int localX, final int localY) {
        if (map == null || loadStage != 2) {
            return -1;
        }
        return map.getLocalMapSetting(plane, localX & 63, localY & 63);
    }

    public final short getOverlayId(final int plane, final int localX, final int localY) {
        if (map == null || loadStage != 2) {
            return -1;
        }
        return map.getLocalOverlayId(plane, localX & 63, localY & 63);
    }

    public final short getUnderlayId(final int plane, final int localX, final int localY) {
        if (map == null || loadStage != 2) {
            return -1;
        }
        return map.getLocalUnderlayId(plane, localX & 63, localY & 63);
    }
    ...
    private final void clip(final WorldObject object, final int x, final int y) {
        ...
        if (type == 22 ? objectDefinition.getInteractType() != 1 : objectDefinition.getInteractType() == 0) {
            return;
        }
        if (type >= 0 && type <= 3) {
    ...
    public void spawnObject(final WorldObject object, final int plane, final int localX, final int localY,
        ...
        ZoneManager.INSTANCE.locAddChange(chunk, object);
    }
    ...
    public void removeObject(final WorldObject object, final int plane, final int localX, final int localY) {
        ...
        ZoneManager.INSTANCE.locDel(chunk, object);
    }
    ...
    public void loadRegionMap() {
        try {
                        ...
                        for (int y = 0; y < 64; y++) {
                            while (true) {
                                ...
                                if (value == 0) {
                                    ...
                                    final byte tileHeight = mapBuffer.readByte();
                                    getRegionMap(true).setTileHeight(plane, x, y, tileHeight);
                                    break;
                                } else if (value <= 49) {
                                    final short overlayId = (short) mapBuffer.readShort();
                                    getRegionMap(true).setOverlayId(plane, x, y, overlayId);
                                } else if (value <= 81) {
                                    mapSettings[plane][x][y] = (byte) (value - 49);
                                } else {
                                    final short underlayId = (short) (value - 81);
                                    getRegionMap(true).setUnderlayId(plane, x, y, underlayId);
                                }
                            }
                        }
                    }
            ...
            if (log.isErrorEnabled())
                log.error("Failed to load region " + regionId, e);
        }
    }
    ...
    public WorldObject getObjectWithPredicate(final int plane, final int x, final int y, Predicate<WorldObject> predicate) {
        if (objects == null) {
            return null;
        }
        for (int i = 0; i < 4; i++) {
    ...
    public WorldObject getObjectOfSlot(final int locationZ, final int locationX, final int locationY, final int type) {
        return getObjectWithPredicate(locationZ, locationX, locationY, object -> Regions.OBJECT_SLOTS[object.getType()] == Regions.OBJECT_SLOTS[type]);
    }
