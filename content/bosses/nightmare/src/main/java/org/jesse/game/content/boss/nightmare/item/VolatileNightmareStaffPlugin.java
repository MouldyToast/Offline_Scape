package org.jesse.game.content.boss.nightmare.item;

import org.jesse.game.item.Item;
import org.jesse.game.item.ids.ItemId;
import org.jesse.game.model.item.PairedItemOnItemPlugin;
import org.jesse.game.model.item.pluginextensions.ItemPlugin;
import org.jesse.game.world.entity.player.Player;
import org.jesse.game.world.entity.player.dialogue.Dialogue;

public class VolatileNightmareStaffPlugin extends ItemPlugin implements PairedItemOnItemPlugin {

	private static final Item STAFF = new Item(ItemId.VOLATILE_NIGHTMARE_STAFF);
	private static final Item[] COMPONENTS = {new Item(ItemId.NIGHTMARE_STAFF), new Item(ItemId.VOLATILE_ORB)};

	@Override
	public void handle() {
		bind("Dismantle", (player, item, slotId) -> {
			if (!player.getInventory().hasFreeSlots()) {
				player.sendMessage("Not enough space in your inventory to dismantle the staff.");
				return;
			}

			player.getInventory().deleteItem(item);
			player.getInventory().addItems(COMPONENTS);
			player.getDialogueManager().start(new Dialogue(player) {
				@Override
				public void buildDialogue() {
					doubleItem(COMPONENTS[0], COMPONENTS[1], "You remove the orb from the staff.");
				}
			});
		});
	}

	@Override
	public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
		player.getInventory().deleteItemsIfContains(COMPONENTS, () -> {
			player.getInventory().addItem(STAFF);
			player.getDialogueManager().start(new Dialogue(player) {
				@Override
				public void buildDialogue() {
					item(STAFF, "You add the orb to the staff.");
				}
			});
		});
	}

	@Override
	public int[] getItems() {
		return new int[]{ItemId.VOLATILE_NIGHTMARE_STAFF};
	}

	@Override
	public ItemPair[] getMatchingPairs() {
		return new ItemPair[]{new ItemPair(ItemId.NIGHTMARE_STAFF, ItemId.VOLATILE_ORB)};
	}

}
