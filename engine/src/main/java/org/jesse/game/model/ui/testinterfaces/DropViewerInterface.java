...
import org.jesse.game.content.alternatetables.AlternateTable;
...
import org.jesse.game.world.entity.npc.drop.matrix.ItemDrop;
import org.jesse.game.world.entity.npc.drop.matrix.NPCDrops;
import org.jesse.game.world.entity.npc.drop.matrix.*;
import org.jesse.game.world.entity.npc.drop.viewerentry.DropViewerEntry;
import org.jesse.game.world.entity.npc.drop.viewerentry.ItemDropViewerEntry;
import org.jesse.game.world.entity.npc.drop.viewerentry.NPCDropViewerEntry;
...
public final class DropViewerInterface extends Interface {
    private static final Logger log = NearRealityLogger.getLogger(DropViewerInterface.class);
    ...
    protected void build() {
        ...
        bind("Rarity Display", (player, slotId, itemId, option) -> {
            ...
            if (!(rowsAttr instanceof List)) {
                ...
                final List<DropViewerEntry> entries = getEntries(player, table);
                populateRows(player, true, table, entries);
                player.getPacketDispatcher().sendClientScript(2239);//keyboard_hide
                return;
            }
    ...
    public static void populateRows(final Player player, final boolean completeRefresh,
        ...
        for (final DropViewerEntry r : rows) {
            ...
            if (player.getBooleanAttribute("drop_viewer_fractions")) {
                double basePercent = r.rate();
                double bonusFactor = 1.0 + (player.getDropRateBonus() / 100.0);
                double effectivePercent = Math.min(100.0, basePercent * bonusFactor);
                if (effectivePercent >= 100.0) {
                    rarity = "Always";
                } else {
                    int denominator = (int) Math.round(100.0 / effectivePercent);
                    String formatted = Utils.formatNumberWithCommas(denominator);
                    rarity = "1 / " + (formatted.length() > 8 ? formatted.charAt(0) + " million" : formatted);
                }
            } else {
                var playerRateModifier = player.getDropRateBonus();
                var rate = Math.min(100, r.rate() * (1.00D + (playerRateModifier / 100D)));
                rarity = r.rate() == 100 ? "Always" : formatDropRate(rate) + "%";
            }
            if (r instanceof ItemDropViewerEntry row) {
                if (row.getItem() == ItemId.TZREKJAD) {
                    rarity = "1 / 100 (Task); 1 / 200";
                } else if (row.getItem() == ItemId.JALNIBREK) {
                    rarity = "1 / 75 (Task); 1 / 100";
                }
            }
            if (r instanceof ItemDropViewerEntry row) {
    ...
    public static void populateDropViewerData() {
        ...
        log.info("Populated {} searchable item definitions in {}ms.",
                searchableItemDefinitions.size(),
