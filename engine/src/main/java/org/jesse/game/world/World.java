...
import org.jesse.game.model.item.containers.DragonhidePouch;
import org.jesse.game.model.item.containers.GemBag;
...
import org.jesse.game.world.entity.player.login.LoginPacketIn;
import org.jesse.game.world.entity.player.login.LoginRequest;
...
import org.jesse.game.world.region.zone.ObjAddDummyEvent;
import org.jesse.game.world.region.zone.ObjTurnPublicDummyEvent;
import org.jesse.game.world.region.zone.ObjUpdateDummyEvent;
...
import org.jesse.plugins.events.FloorItemSpawnEvent;
import org.jesse.plugins.events.ServerShutdownEvent;
import org.jesse.plugins.item.Blowpipe;
import org.jesse.plugins.item.SeedBox;
...
import static org.jesse.game.model.ui.testinterfaces.advancedsettings.SettingVariables.AMMO_PICKING_BEHAVIOUR_VARBIT_ID;
...
public final class World {
    ...
    public static final Long2ObjectMap<Player> namedPlayers =
            new Long2ObjectOpenHashMap<>(PLAYER_CAPACITY);
    ...
    @Nullable
    public static Player getNamedPlayer(final long nameLong) {
        return namedPlayers.get(nameLong);
    }

    @Nullable
    public static Player getNamedPlayer(@NotNull final String name) {
        final long nameLong = stringToLong(name);
        return getNamedPlayer(nameLong);
    }

    @Nullable
    public static Player setNamedPlayer(final long nameLong,
                                        @Nullable final Player player) {
        return player == null
                ? namedPlayers.remove(nameLong)
                : namedPlayers.put(nameLong, player);
    }

    @Nullable
    public static Player setNamedPlayer(@NotNull final String name,
                                        @Nullable final Player player) {
        final long nameLong = stringToLong(name);
        return setNamedPlayer(nameLong, player);
    }

    public static Optional<Player> getPlayer(final long nameLong) {
        return Optional.ofNullable(getNamedPlayer(nameLong));
    }

    public static Optional<Player> getPlayer(@NotNull final String name) {
        final long nameLong = stringToLong(name);
        return getPlayer(nameLong);
    }
    ...
    public static volatile double playerCountMod = 1.6;
    ...
    public static void sendUpdateRebootTimer(final int ticks) {
        ...
        for (final Player player : getPlayers()) {
            try {
                player.getPacketDispatcher().rebootTimer(ticks);
    ...
    private static void saveAndKick(final Player player) {
        try {
            player.getPacketDispatcher().sendLogout(LogoutType.UPDATING);
            player.finish();
    ...
    public static void addLoginRequest(final LoginBlock<?> block, final GameLoginResponseHandler<Session> handler) {
        final long time = System.currentTimeMillis();
        final LoginPacketIn loginPacketIn = new LoginPacketIn(block);
        final PlayerInformation playerInformation = new PlayerInformation(block, loginPacketIn);
        final LoginRequest loginRequest = new LoginRequest(
                loginPacketIn,
                playerInformation,
                handler.getCtx()
        );
        CoresManager.getLoginManager().load(time, playerInformation,
                player -> {
                    loginRequest.loaded(player, System.currentTimeMillis() - time >= 30_000, block, handler);
                });
    }
    ...
    public static NPC spawnNPC(
        ...
        try {
            ...
            log.error("Failed to spawn npc" +
                            "(id=" + id + ", location=" + location + ", direction=" + direction + ", radius=" + radius + ")",
                    e);
    ...
    public static void sendAttachedObject(final Player target, final AttachedObject object) {
        ZoneManager.INSTANCE.locMerge(object.getObject().getChunkHash(), target, object);
    }
    ...
    public static int sendProjectile(final Position shooter, final Position receiver, final Projectile projectile,
        ...
        final int index = receiver instanceof Entity e ? e.getProjectileIndex() : 0;
        final int projSpeed = speed != Integer.MIN_VALUE ? speed : projectile.getProjectileDuration(from, receiver);
    ...
    public static boolean canPlaceObjectWithoutCollisions(@NotNull final Location tile, final int objectType) {
        return canPlaceObjectWithoutCollisions(tile.getX(), tile.getY(), tile.getPlane(), objectType);
    }

    public static boolean canPlaceObjectWithoutCollisions(final int x, final int y, final int z,
    ...
    @Nullable
    public static WorldObject getObjectWithPredicate(@NotNull Location tile, @NotNull Predicate<WorldObject> function) {
        return getRegion(tile.getRegionId()).getObjectWithPredicate(tile.getPlane(), tile.getXInRegion(),
                tile.getYInRegion(), function);
    }

    public static boolean containsPlayer(final String username) {
        return getNamedPlayer(username) != null;
    }
    ...
    public static void removeGraphicalDoor(final WorldObject object) {
        ZoneManager.INSTANCE.locDel(object.getChunkHash(), object);
    }
    ...
    public static void spawnTemporaryObject(final WorldObject object, final int ticks, final Runnable onRemoval) {
        spawnObject(object);
        WorldTasksManager.schedule(() -> {
            removeObject(object);
            onRemoval.run();
    ...
    public static void spawnTemporaryObjectWithoutClip(final WorldObject object, final int ticks) {
    ...
    public static void sendSoundEffect(final Position tile, final SoundEffect sound) {
        ZoneManager.INSTANCE.soundArea(tile.getPosition().getChunkHash(), sound, tile.getPosition());
    }
    ...
    public static void sendObjectAnimation(final WorldObject object, final Animation animation) {
        ...
        ZoneManager.INSTANCE.locAnim(object.getChunkHash(), object, animation);
    }
    ...
    public static void sendObjectAnimation(final int id, final int type, final int rotation, final Location location, final Animation animation) {
        Preconditions.checkArgument(animation != null);
        ZoneManager.INSTANCE.locAnim(location.getChunkHash(), new WorldObject(id, type, rotation, location), animation);
    }
    ...
    private static void updateGroundItem(final FloorItem item, final int amount, final int ticks) {
        ...
        ZoneManager.INSTANCE.appendUpdate(chunk, new ObjUpdateDummyEvent(item, oldQuantity));
    }
    ...
    private static void turnFloorItemPublic(final FloorItem item) {
        ...
        ZoneManager.INSTANCE.appendUpdate(chunk, new ObjTurnPublicDummyEvent(item));
    }
    ...
    public static void spawnFloorItem(final Item item, final Location tile, final int maxStack, final Player owner,
        ...
        for (int num = 0; num < amt; num++) {
            ...
            ZoneManager.INSTANCE.appendUpdate(chunkId, new ObjAddDummyEvent(fItem));
        }
    }
    ...
    public static boolean destroyFloorItem(final FloorItem item, final boolean removeFromGlobal) {
        ...
        ZoneManager.INSTANCE.objDel(chunkId, item);
        return true;
    }
    ...
    public static void takeFloorItem(final Player player, final FloorItem item) {
        ...
        final var lootingBag = player.getLootingBag();
        final var seedBox = player.getSeedBox();
        final var herbSack = player.getHerbSack();
        final var bonePouch = player.getBonePouch();
        final var gemBag = player.getGemBag();
        final var dragonhidePouch = player.getDragonhidePouch();
        final int id = item.getId();
        ...
        else if (herbSack != null && player.getInventory().containsItem(org.jesse.plugins.item.HerbSack.HERB_SACK_OPEN) && herbSack.isOpen()
        ...
        if (addToBlowpipe) {
            ...
            // If we have turned on Ammo-Picking
            if (player.getVarManager().getBitValue(AMMO_PICKING_BEHAVIOUR_VARBIT_ID) == 1) {
                if (it.isRangedAmmo()) {
                    // if we're wearing this ammo
                    if (player.getEquipment().containsItem(it.getId())) {
                        container = player.getEquipment().getContainer();
                    }
                    // TODO: else if our bolt-pouch has this ammo
                    // TODO: else if our secondary in Dizana's Quiver
                }
            }
            // If we have turned on Rune-Picking
            else if (player.getVarManager().getBitValue(RUNE_PICKING_BEHAVIOUR_VARBIT_ID) == 1) {
                final var rune = Rune.getRune(item);
            ...
            final var result = container.add(it);
        ...
        if (value > 150_000) {
            if(WORLD_PROFILE.isLogsDatabaseEnabled())
                GameLogger.log(Level.INFO, () -> new GameLogMessage
                        .GroundItem.Pickup(Clock.System.INSTANCE.now(), player.getDbUsername(), item, player.getLocation())
                );
        }
        if (succeeded >= 1) {
    ...
    public static void sendGraphics(final Graphics graphics, final Location tile) {
        ...
        ZoneManager.INSTANCE.mapAnim(tile.getChunkHash(), graphics, tile);
    }

    public static int registerPlayer(final Player player) {
        if (players.size() >= PLAYER_CAPACITY) {
            return -1;
        }
        int index = players.addImpl(player);
        if (index >= 0) {
            setNamedPlayer(player.getUsername(), player);
            WorldsManager.setOurWorldPlayerCount(players.size());
        }
        return index;
    }
    ...
    public static void removePlayer(
        ...
        setNamedPlayer(player.getUsername(), null);
        WorldsManager.setOurWorldPlayerCount(players.size());
    }
    ...
    public static byte getTileHeight(@NotNull final Location tile) {
        return getTileHeight(tile.getPlane(), tile.getX(), tile.getY());
    }

    public static byte getTileHeight(final int plane, final int x, final int y) {
        final int regionId = (((x & 16383) >> 6) << 8) | ((y & 16383) >> 6);
        final Region region = World.getRegion(regionId);
        return region.getTileHeight(plane & 3, x & 63, y & 63);
    }

    public static byte getMapSetting(@NotNull final Location tile) {
        return getMapSetting(tile.getPlane(), tile.getX(), tile.getY());
    }

    public static byte getMapSetting(final int plane, final int x, final int y) {
        final int regionId = (((x & 16383) >> 6) << 8) | ((y & 16383) >> 6);
        final Region region = World.getRegion(regionId);
        return region.getMapSetting(plane & 3, x & 63, y & 63);
    }

    public static short getOverlayId(@NotNull final Location tile) {
        return getOverlayId(tile.getPlane(), tile.getX(), tile.getY());
    }

    public static short getOverlayId(final int plane, final int x, final int y) {
        final int regionId = (((x & 16383) >> 6) << 8) | ((y & 16383) >> 6);
        final Region region = World.getRegion(regionId);
        return region.getOverlayId(plane & 3, x & 63, y & 63);
    }

    public static short getUnderlayId(@NotNull final Location tile) {
        return getUnderlayId(tile.getPlane(), tile.getX(), tile.getY());
    }

    public static short getUnderlayId(final int plane, final int x, final int y) {
        final int regionId = (((x & 16383) >> 6) << 8) | ((y & 16383) >> 6);
        final Region region = World.getRegion(regionId);
        return region.getUnderlayId(plane & 3, x & 63, y & 63);
    }
    ...
    public static void unregisterPlayer(
        ...
        player.finish();
        CoresManager.getLoginManager().save(player);
        WorldTasksManager.schedule(() -> removePlayer(player, networkService), 1);
    }
    ...
    public static void sendClosestNPCDialogue(final Player player, final int npcId, final int radius,
        ...
        findNPC(player.getLocation(), radius, npc -> npc.getId() == npcId).ifPresent(dialogue::setNpc);
    ...
    public static List<Player> getStaff() {
        return players.stream().filter(it ->
                !it.isNulled() && it.getPrivilege().inherits(PlayerPrivilege.SUPPORT)).toList();
    }
