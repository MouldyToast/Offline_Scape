...
import org.jesse.game.content.consumables.drinks.DivinePotion;
...
import org.jesse.game.content.skills.farming.Seedling;
...
import org.jesse.game.world.entity.player.action.combat.PowerOfDeathVariableTask;
...
public enum TickVariable {
    ...
    STAMINA_ENHANCEMENT((player, ticks) -> {
        if (ticks == 0) {
            ...
            player.getVarManager().sendBit(24, 0);
        }
    ...
    SAGES_GREAVES_AGILITY((player, ticks) -> {
        ...
        if (greaves && ticks == 0) {
            ...
            try {
                locationMatch = player.getLocation().get18BitHash() == (int) player.getTemporaryAttributes().getOrDefault("sage_last_tick_location", -1);
            } catch (Exception ignored) {}
            if(!locationMatch) {
                var agilityLevel = player.getSkills().getLevel(SkillConstants.AGILITY);
                var expToGain = agilityLevel * 0.2;
                player.getSkills().addXp(SkillConstants.AGILITY, expToGain);
                player.getTemporaryAttributes().remove("sage_last_tick_location");
            }
        }
    }),
