 ...
 */
public final class CapeRack {
	
	private transient Player player;
	@Expose private final Map<Integer, int[]> items = new HashMap<>(CapeRackData.VALUES.length);

	/**
	 * Takes an armour set from the cape rack if possible.
	 * @param itemId the display item id of the set on the interface.
	 */
	public void takeSet(final int itemId) {
		CapeRackData set = CapeRackData.DISPLAY_MAP.get(itemId);
		if (set == null) set = CapeRackData.MAP.get(itemId);
		if (set == null) {
			return;
		}
		...
		player.sendMessage("You remove the " + val.toLowerCase() + " from the cape rack.");
	...
	public boolean addSet(final int itemId) {
		final CapeRackData set = CapeRackData.MAP.get(itemId);
		if (set == null) {
			return false;
		}
		final int display = set.getDisplayItem();
		if (items.containsKey(display)) {
			player.sendMessage("Your cape rack already contains that.");
			return false;
		}
		...
		player.sendMessage("You place the " + val.toLowerCase() + " into the cape rack.");
	 ...
	 */
	private void notify(final CapeRackData set) {
		final EnumDefinitions map = EnumDefinitions.get(380);
		...
		player.sendMessage("You need the following items to enqueue the " + val.toLowerCase() + " to your cape rack:");
	...
	private void refresh() {
		...
		player.getPacketDispatcher().sendClientScript(3532, 3292, 1, 0);
	}

	/**
	 * Opens the cape rack interface.
	 */
	public void open() {
		player.getTemporaryAttributes().put("costumeRoomObject", "CAPE_RACK");
		...
		player.getPacketDispatcher().sendClientScript(3532, 3292, 1, 1);
	}
