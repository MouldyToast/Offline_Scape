...
public interface Diary {
	...
	static void sendJournal(final Player player, final String title, final List<String> text) {
		...
		GameInterface.DIARY_LIST.open(player);

		dispatcher.sendComponentVisibility(INTERFACE, 8, true);
		dispatcher.sendClientScript(2498, 1, 1);
		final int size = text.size();
		dispatcher.sendComponentText(INTERFACE, 4, title);
		for (int i = LIMIT + OFFSET; i >= OFFSET; i--) {
