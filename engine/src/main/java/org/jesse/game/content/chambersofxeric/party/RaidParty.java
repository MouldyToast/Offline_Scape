...
public final class RaidParty {
     ...
     */
    public RaidParty(@NotNull final Player host, @NotNull final ClanChannel channel) {
        player = host.getUsername();
        raid = new Raid(this);
        this.channel = channel;
        milliseconds = Utils.currentTimeMillis();
        try {
            uid = (int) host.getUser().getId();
        } catch(Exception ex) {
            uid = 1;
        }
    }
    ...
    public void refresh(final Player player) {
        ...
        player.getVarManager().sendVar(1427, uid);
        ...
        // OSRS has 2 varbits set for the scaled size,
        // https://cdn.discordapp.com/attachments/1306388437653393428/1347553680605319168/image.png?ex=67cc3ea9&is=67caed29&hm=cd3e593d3d91df20533889ca13b73c71a0dbe767a527443a8d16a148b3259c06&
        player.getVarManager().sendBit(9539, getScaledPartySize());
        player.getVarManager().sendBit(9541, getScaledPartySize());
