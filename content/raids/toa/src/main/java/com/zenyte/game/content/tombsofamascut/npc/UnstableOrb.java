...
public class UnstableOrb extends NPC {
	...
	@Override public void processNPC() {
        ...
        Arrays.stream(players)
			.filter(p -> p != null && getLocation().equals(p.getLocation()))
			.forEach(p -> {
				playersHit.add(p);
				finish = true;
			});
		if (!playersHit.isEmpty()) {
			var hit = new Hit(encounter.getAkkha(), encounter.getAkkha().getMaxHit(13) + Utils.random(2), HitType.DEFAULT);
			playersHit.forEach(p -> p.applyHit(hit));
		}
		if (finish || isLocked()) return;
