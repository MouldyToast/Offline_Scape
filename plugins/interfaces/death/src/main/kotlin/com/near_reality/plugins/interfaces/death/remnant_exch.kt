    ...
    init {
        ...
        REMNANT_EXCHANGE {
            ...
            "Confirm".suspend(13) {
                ...
                if(price == 0) {
                    player.sendMessage("This doesn't seem like a smart thing to do.")
                    return@suspend
                }
                ...
                if (GameConstants.WORLD_PROFILE.isLogsDatabaseEnabled())
                    GameLogger.log {
                        GameLogMessage.RemnantExchange(username = player.dbUsername, item = Item(itemInSlot.id, successfulAmount), value = price)
                    }
                player.resetSelectedItem(itemComponent.componentID, portalComponent.componentID)
                player.sendSound(1595)
                player.sendSound(SoundEffect(2115, 0, 10))
