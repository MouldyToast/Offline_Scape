...
data class Boundary(
    ...
    fun `in`(x: Int, y: Int, z: Int): Boolean {
        return x in minX..highX && y in minY..highY && z == height
    }
    ...
    fun getInner(): Set<Location> {
        val innerSet: MutableSet<Location> = HashSet()
        (minX..highX).forEach { x ->
            (minY..highY).forEach { y ->
                innerSet.add(Location(x, y))
                innerSet.add(Location(x, y))
            }
        }
        return innerSet
    ...
    companion object {
        val WILDERNESS = Boundary(2941, 3525, 3392, 4030)
        val WILDERNESS_EDGE_ISLAND = Boundary(3392, 3645, 3456, 3710)
        val EDGE_ISLAND_BANK = Boundary(3416, 3678, 3423, 3689)
        val WILDERNESS_UNDERGROUND1 = Boundary(2989, 9985, 3543, 10379)
        val WILDERNESS_UNDERGROUND2 = Boundary(3072, 9918, 3135, 9984)
        val ARTIO = Boundary(1741, 11527, 1774, 11559)
        ...
        private fun createSurroundingBoundary(position: Position, offset: Int, size: Int): Boundary {
            val minDeltaX: Int = position.position.x - offset
            val minDeltaY: Int = position.position.y - offset
            val maxDeltaX: Int = position.position.x + (size -1) + offset
            val maxDeltaY: Int = position.position.y + (size -1) + offset
