...
import org.jesse.game.model.ui.testinterfaces.AutocastInterface;
import org.jesse.game.net.packet.PacketDispatcher;
import org.jesse.game.world.entity.npc.combatdefs.AttackType;
import org.jesse.game.world.entity.player.action.combat.AttackStyle;
...
import org.jesse.plugins.events.SpellbookChangeEvent;
...
public final class CombatDefinitions {
	...
	private static final Logger log = NearRealityLogger.getLogger(CombatDefinitions.class);
	...
	public boolean hasFullAhrimsAndDamned() {
		...
		return containsAhrims(hood.getName(), "hood")
				&& containsAhrims(top.getName(), "robetop")
				&& containsAhrims(skirt.getName(), "robeskirt")
				&& containsAhrims(staff.getName(), "staff")
				&& amulet.getName().equalsIgnoreCase("Amulet of the damned");
	}

	private boolean containsAhrims(String name, String piece) {
		name = name.toLowerCase();
		return (name.contains("ahrim's " + piece) || name.contains("echo ahrim's " + piece));
	}
	...
	public void refresh() {
		...
		dispatcher.sendComponentText(GameInterface.COMBAT_TAB.getId(), 2, weapon == null ? "Unarmed" : weapon.getName());
		dispatcher.sendComponentText(GameInterface.COMBAT_TAB.getId(), 3, "Category: " + styleDefinition.getPrettyName());
