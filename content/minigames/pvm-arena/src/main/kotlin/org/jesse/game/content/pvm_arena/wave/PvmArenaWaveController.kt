...
internal class PvmArenaWaveController(
    ...
    var teamBlueWaveController: PvmArenaTeamWaveController? = null
    var teamRedWaveController: PvmArenaTeamWaveController? = null
...
//        logger.info("Cycled both team's wave controllers (blue=$stateBlue, red=$stateRed)")
    ...
    class PvmArenaTeamWaveController(
        private val team: PvmArenaTeam,
        private val waveProducers: ArrayDeque<PvmArenaWave.Config>,
    ) {
        ...
        fun reset() {
            ...
            when (val currentState = state) {
                is State.ActiveWave -> {
                    logger.info("${currentState.wave} - resetting current wave...")
                    currentState.wave.cancel()
                }
                else -> logger.info("${currentState.javaClass.name} - no waves to reset.")
            }
            state = State.Reset
        }
