...
public final class SpotAnimationDefinition implements Definitions {
    ...
    public int getSequenceDelay() {
        if (animationId == -1) return 0;

        var animationDefinition = AnimationDefinitions.get(animationId);
        if (animationDefinition == null) return 0;

        return animationDefinition.getClientTicks();
    }
