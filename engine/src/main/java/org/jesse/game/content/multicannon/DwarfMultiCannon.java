package org.jesse.game.content.multicannon;
...
import org.jesse.plugins.events.CannonRemoveEvent;
...
public final class DwarfMultiCannon {
	...
	private static final Logger log = NearRealityLogger.getLogger(DwarfMultiCannon.class);
				...
				public void run() {
					...
					switch (setupStage = (byte) stage++) {
						case 0 -> {
							...
							player.getVarManager().sendVar(2, 1);
						...
						case 1 -> {
							...
							player.getVarManager().sendVar(2, 2);
							return;
						}
						case 2 -> {
	...
	public void take(final int id, final boolean message) {
		...
		player.getVarManager().sendVar(2, 0);
		if (cannon != null) {
			PluginManager.post(new CannonRemoveEvent(cannon));
	...
	public static int getCannonBallsMax(Player player) {
		int maxBalls = switch (player.getMemberRank()) {
			case ONYX, ZENYTE -> 100;
			case DIAMOND, DRAGONSTONE -> 90;
			case RUBY -> 70;
			case EMERALD -> 60;
			case SAPPHIRE -> 50;
			case TOPAZ -> 40;
