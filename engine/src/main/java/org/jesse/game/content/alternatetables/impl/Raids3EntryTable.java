...
public class Raids3EntryTable implements AlternateTableDropProvider {
    ...
    private void buildEntries() {
        ...
        genUnique(ItemId.OSMUMTENS_FANG, 1, 40);
        genUnique(ItemId.LIGHTBEARER, 1, 40);
        genUnique(ItemId.ELIDINIS_WARD, 3, 20000);
        genUnique(ItemId.MASORI_MASK, 1, 10000);
        genUnique(ItemId.MASORI_BODY, 1, 10000);
        genUnique(ItemId.MASORI_CHAPS, 1, 10000);
        genUnique(ItemId.TUMEKENS_SHADOW_UNCHARGED, 1, 20000);
    ...
    public String getName() {
        return "Raids - ToA Entry";
    }

    private void genCommon(int id, int divisor) {
        entries.add(new OtherDropViewerEntry(id, (int) (double) (points / divisor), (int) (double) (points / divisor), 1, 26, "This item's rate reflects a lv 100 raid ending with 32,000 points."));
    }

    private void genUnique(int id, int top, int bottom) {
        entries.add(new OtherDropViewerEntry(id, 1, 1, top, bottom, "This item's rate reflects a lv 100 raid ending with 32,000 points."));
    }
}
