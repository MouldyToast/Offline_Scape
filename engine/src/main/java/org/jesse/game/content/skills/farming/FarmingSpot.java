package org.jesse.game.content.skills.farming;
...
import static org.jesse.game.content.skills.farming.FarmingPatch.*;
import static org.jesse.game.content.skills.farming.PatchFlag.*;
import static org.jesse.game.content.skills.farming.PatchState.*;
...
public final class FarmingSpot {
    ...
    FarmingSpot(final Player player, final FarmingPatch patch) {
        ...
        setTime(0);
        setValue(0);
        setLives((player.getMemberRank().equalToOrGreaterThan(MemberRank.EMERALD) && this.getPatch().getType() == PatchType.HERB_PATCH) || getPatch().getType() == PatchType.BUSH_PATCH ? 4 : 3);
        resetTimer();
        ...
        if (!regeneratingFruit) {
            ...
            if (newState == GROWN && newState != oldState) {
                getCompostFlag().ifPresent(flag -> {
                    final int extra = (player.getMemberRank().equalToOrGreaterThan(MemberRank.EMERALD) && this.getPatch().getType() == PatchType.HERB_PATCH) || getPatch().getType() == PatchType.BUSH_PATCH ? 1 : 0;
                    if (flag == COMPOST) {
                        setLives(extra + 4);
    ...
    private boolean disease(final PatchState state) {
        if (player.getAchievementDiaries().isAllCompleted(KourendDiary.EASY)) {
            ...
            if (patch == FarmingPatch.KOUREND_ALLOTMENT_NE ||
                    ...
                    patch == FarmingPatch.KOUREND_FLOWER ||
                    patch == FarmingPatch.KOUREND_HERB) {
                return false;
            }
        }
    ...
    public double successProbability() {
        ...
        if (player.getInventory().containsItem(ItemId.QUEENS_SECATEURS) || player.getEquipment().getId(EquipmentSlot.WEAPON) == ItemId.QUEENS_SECATEURS) {
            base += 0.3F;
        } else if (player.getInventory().containsItem(ItemId.MAGIC_SECATEURS) || player.getEquipment().getId(EquipmentSlot.WEAPON) == ItemId.MAGIC_SECATEURS) {
            base += 0.1F;
        }
