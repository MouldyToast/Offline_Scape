...
public class BaboonNPC extends TOANPC implements CombatScript {
		...
		if (EncounterStage.STARTED.equals(encounter.getStage()) && !isDead() && !isFinished()) {
			...
			if (explodeTicks > 0) {
				if (--explodeTicks == 3) {
					...
					var entities = new ArrayList<Entity>(encounter.getSpawnedBaboons());
					if (entities.addAll(List.of(encounter.getChallengePlayers())))
						entities.stream()
							.filter(Objects::nonNull)
							.filter(this::closeEnough)
							.forEach(this::applyExplosionDamage);
	...
	private Boolean closeEnough(Entity entity) {
		return entity.getLocation().getTileDistance(getLocation()) <= 1;
	}

	private void applyExplosionDamage(Entity target) {
		var explodingHit = new Hit(this, getMaxHit(14) + Utils.random(2), HitType.DEFAULT);
		target.applyHit(explodingHit);
	}
