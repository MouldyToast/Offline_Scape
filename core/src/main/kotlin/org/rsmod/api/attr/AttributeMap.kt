    ...
    private fun normalizePersistenceValue(value: Any): Any =
        when (value) {
            ...
            is Number ->
                when (value) {
                    is Int -> value
                    // DIVERGENCE from OpenRune: this codebase's Gson uses
                    // ToNumberPolicy.LONG_OR_DOUBLE, so persisted whole numbers
                    // arrive as Long. Narrow to Int when it fits so
                    // AttributeKey<Int> round-trips. (OpenRune keeps Long as Long.)
                    is Long -> if (value in Int.MIN_VALUE..Int.MAX_VALUE) value.toInt() else value
