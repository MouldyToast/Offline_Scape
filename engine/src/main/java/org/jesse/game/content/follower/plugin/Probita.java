package org.jesse.game.content.follower.plugin;
                    ...
                    public void buildDialogue() {
                        ...
                        if (canInsureForFree) {
                            ...
                            npc("Once you've insured the pet, you can reclaim it here unlimited times for a reclamation " +
                                    "fee of " + (player.getMemberRank().equalToOrGreaterThan(MemberRank.RUBY) ? "nothing." : (StringFormatUtil.format(PetInsurance.RECLAIM_PRICE.getAmount()) + " coins."))).executeAction(() -> {
                                if (basePetItemId == -1) {
                                    setKey(70);
                                    return;
                                }
