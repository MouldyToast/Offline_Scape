...
public class FamiliarsFortune extends Boon {
    ...
    public static boolean shouldBoostXp(Player player, int skill) {
        ...
        if (!player.getBoonManager().hasBoon(FamiliarsFortune.class)) {
            return false;
        }

        var pet = SkillingPet.getBySkill(skill);
        if (pet == null) {
            return false;
        }
    ...
    public String description() {
        return "Skilling pets provide a 50% xp boost to their associated skill when equipped";
    }
