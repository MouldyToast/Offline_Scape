...
import org.jesse.game.content.theatreofblood.party.PartyRights
...
class PartyInformationInterface : Interface() {
    ...
    override fun build() {
        ...
        bind("Move up 1") { player: Player -> moveArrow(player, 1, true)};
        bind("Move down 1") { player: Player -> moveArrow(player, 1, false)};
        bind("Move up 2") { player: Player -> moveArrow(player, 2, true)};
        bind("Move down 2") { player: Player -> moveArrow(player, 2, false)};
        bind("Move up 3") { player: Player -> moveArrow(player, 3, true)};
        bind("Move down 3") { player: Player -> moveArrow(player, 3, false)};
        bind("Move up 4") { player: Player -> moveArrow(player, 4, true)};
        bind("Move down 4") { player: Player -> moveArrow(player, 4, false)};
        bind("Move up 5") { player: Player -> moveArrow(player, 5, true)};
        bind("Move down 5") { player: Player -> moveArrow(player, 5, false)};
    }

    private fun moveArrow(player: Player, row: Int, up: Boolean) {
        val party: RaidingParty? = VerSinhazaArea.getParty(player)
        if (party != null && party.isLeader(player)) {
            val fromIndex: Int = Math.floorMod(row - 1, party.members.size)
            val toIndex: Int = Math.floorMod(fromIndex + (if (up) -1 else 1), party.members.size)

            val replaceIndex = if (toIndex == 0) fromIndex else toIndex
            val replacement = party.getMember(replaceIndex)

            val swapDialogue: Dialogue = object : Dialogue(player) {
                override fun buildDialogue() {
                    if (replacement != null) {
                        plain("Swapping place with another player in the group will transfer the<br>" +
                                "leadership to that player. You are about to give <col=ff0000>${replacement.name}</col> leadership<br>" +
                                "of the group."
                        )
                        options("Continue?", "Yes", "No")
                        .onOptionOne {
                            Collections.swap(party.members, 0, replaceIndex)
                            party.run { player.updateInformation() }
            ...
            if (toIndex == 0 || fromIndex == 0)
                player.dialogueManager.start(swapDialogue)
            else {
                Collections.swap(party.members, fromIndex, toIndex)
                party.run { player.updateInformation() }
            }
        }
        else player.sendMessage("You must be party leader to do that.")
    }
