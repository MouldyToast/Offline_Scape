...
import org.jesse.game.content.treasuretrails.challenges.*;
import org.jesse.game.content.treasuretrails.clues.*;
import org.jesse.game.content.treasuretrails.clues.emote.ItemRequirement;
import org.jesse.game.content.treasuretrails.npcs.*;
...
import org.jesse.game.world.entity.player.dialogue.EmptyDialogue;
...
public class TreasureTrail {
    ...
    private static final void progress(@NotNull final Player player, @NotNull final Item item, final int slot, @NotNull final Optional<Dialogue> optionalDialogue, final int index) {
        ...
        final Dialogue dialogue = optionalDialogue.orElseGet(() -> new EmptyDialogue(player));
        if (dialogue.getOnBuild() == null) {
            boolean finalAdvanceStep = advanceStep;
            dialogue.setOnBuild(() -> dialogue.item(dialogueItem, finalAdvanceStep ? "You've found another clue scroll!" : "You've found a casket!"));
        }
        player.getDialogueManager().start(dialogue);
    }
