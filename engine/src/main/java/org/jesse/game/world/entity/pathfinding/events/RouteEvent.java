package org.jesse.game.world.entity.pathfinding.events;
...
import org.jesse.game.world.entity.pathfinding.RouteStrategy;
...
public abstract class RouteEvent<E extends Entity, S extends RouteStrategy> {
	...
	public RouteEvent<E, S> setOnFailure(final Runnable runnable) {
	...
	public boolean process() {
		...
		if (!entity.hasWalkSteps()) {
			final RouteResult steps = RouteFinder.findRoute(entity, entity.getSize(), strategy, isMoveNear());
	...
	public boolean processAfterMovement() {
		return CONTINUE;
	}
	...
	protected boolean initiate() {
		...
		final int maxWalkSteps = getMaxWalkSteps();
		final boolean shouldCheckWalkSteps = shouldCheckWalkSteps();
		for (int step = steps.getSteps() - 1; step >= 0; step--) {
			if (!entity.addWalkSteps(bufferX[step], bufferY[step], maxWalkSteps, shouldCheckWalkSteps)) {
				cancel();
				return STOP;
			}
		}
	...
	public boolean shouldCheckWalkSteps() {
		return true;
	}
