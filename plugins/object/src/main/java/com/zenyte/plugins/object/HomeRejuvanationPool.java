...
public class HomeRejuvanationPool implements ObjectAction {
    ...
    public static void rejuvenate(final Player player) {
        player.setGraphics(HEAL_GFX);
        player.heal(player.getMaxHitpoints());
        player.sendFilteredMessage("The Pool of Rejuvenation restores your health, skills and stamina.");
        ...
        int baseCd = getUsageCooldownDurationTicks(player);
        long currentCycle = WorldThread.getCurrentCycle();
        long ticks = player.getNumericAttribute("box of restoration delay").longValue();
        if (ticks - currentCycle > baseCd * 2L) {
            ticks = 0; // force cooldown to be considered expired
        }

        if (ticks <= currentCycle || GameConstants.REMOVE_RESTRICTIONS_ON_POOLS) {
            player.getCombatDefinitions().setSpecialEnergy(100);
            player.addAttribute("box of restoration delay", currentCycle + baseCd);
        } else {
            final int totalSeconds = (int) TimeUnit.TICKS.toSeconds(ticks - currentCycle);
            final int seconds = totalSeconds % 60;
            final int minutes = totalSeconds / 60;
            player.getDialogueManager().plain("You need to wait another " + (minutes != 0 ? Utils.pluralizedFormatted("minute", minutes) : Utils.pluralizedFormatted("second", seconds)) + " until you can recharge your special energy again.");
        }
    }
    ...
    private static int getUsageCooldownDurationTicks(final Player player) {
        final MemberRank memberRank = player.getMemberRank();
        if (memberRank.equalToOrGreaterThan(MemberRank.DIAMOND)) {
            return 0;
        } else if (memberRank.equalToOrGreaterThan(MemberRank.RUBY)) {
            return (int) TimeUnit.SECONDS.toTicks(30);
        } else if (memberRank.equalToOrGreaterThan(MemberRank.EMERALD)) {
            return (int) TimeUnit.SECONDS.toTicks(60);
        } else if (memberRank.equalToOrGreaterThan(MemberRank.SAPPHIRE)) {
            return (int) TimeUnit.SECONDS.toTicks(90);
        } else if (memberRank.equalToOrGreaterThan(MemberRank.TOPAZ)) {
            return (int) TimeUnit.SECONDS.toTicks(120);
        }
        return (int) TimeUnit.SECONDS.toTicks(180);
    }
