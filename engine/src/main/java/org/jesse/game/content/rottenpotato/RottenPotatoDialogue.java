package org.jesse.game.content.rottenpotato;

import com.google.common.base.Preconditions;
import org.jesse.game.content.rottenpotato.handler.NpcRottenPotatoActionHandler;
import org.jesse.game.content.rottenpotato.handler.ObjectRottenPotatoActionHandler;
...
public class RottenPotatoDialogue extends OptionsMenuD {
    ...
    public void handleClick(int slotId) {
        ...
        else if (action.type() == RottenPotatoActionType.ITEM_ON_OBJECT)
            handleObjectAction((ObjectRottenPotatoActionHandler) action);
    }
    ...
    private void handleObjectAction(final ObjectRottenPotatoActionHandler handler) {
        if (target.isPresent()) {
            final Object target = this.target.get();
            Preconditions.checkArgument(target instanceof WorldObject);
            handler.execute(player, (WorldObject) target);
        }
    }
