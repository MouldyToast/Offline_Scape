...
import org.jesse.plugins.dialogue.MenuD;
...
import static org.jesse.game.world.entity.player.collectionlog.CollectionLogCategoryType.get;
import static org.jesse.game.world.entity.player.collectionlog.CollectionLogCategoryType.getMultiple;
...
public class CompletionistCape {
	...
	public static void onLogin(final LoginEvent event) {
		...
		int compCapeTier = getCompletionistCapeTier(capeId);
		if (compCapeTier > 0) {
	...
	public static int checkRequirements(Player player) {
		if (player.isDeveloper() || ALLOWED_PLAYERS.contains(player.getUsername().toLowerCase())) {
			return 3;
		}
		...
		if (!player.getCombatAchievements().hasAllTiersCompleted()) {
			return 1;
		}

		boolean hasAllCustomRequirements = requirements.stream().allMatch(req -> req.test(player));
		if (!hasAllCustomRequirements) {
			return 2;
		}
	...
	public static int getCompletionistCapeTier(int id) {
		return switch (id) {
			case MASTER_COMP_CAPE -> 3;
			case COMPLETIONIST_CAPE_T -> 2;
			case COMPLETIONIST_CAPE -> 1;
			default -> 0;
		};
	}
	...
	public static int tierToCape(int id) {
		return switch (id) {
			case 1 -> COMPLETIONIST_CAPE;
			case 2 -> COMPLETIONIST_CAPE_T;
			case 3 -> MASTER_COMP_CAPE;
			default -> -1;
		};
	}
