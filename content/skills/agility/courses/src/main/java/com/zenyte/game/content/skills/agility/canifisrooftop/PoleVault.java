...
public final class PoleVault extends AgilityCourseObstacle {
	...
	private static final Location START = new Location(3479, 3484, 2);
	private static final Location FINISH = new Location(3489, 3476, 3);
	private static final Location LEDGE = new Location(3488, 3476, 2);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if(ticks == 0) {
					...
					player.setForceMovement(new ForceMovement(object, 45, ForceMovement.EAST));
				} else if(ticks == 1)
