...
public final class ItemExtractor implements Extractor {
	...
	public void extract() {
		try {
			final BufferedWriter writer = new BufferedWriter(new FileWriter(new File("info/#" + GameConstants.REVISION + " item-defs.txt")));
			//final int len = CacheUtil.getStore().getFilesSystem(2).findFolderByID(10).filesCount();//Cache.STORE.getIndexes()[2].getLastFileId(10);
			final int len = CollectionUtils.getIndiceSize(Indice.ITEM_DEFINITIONS);
			for (int i = 0; i < len; i++) {
