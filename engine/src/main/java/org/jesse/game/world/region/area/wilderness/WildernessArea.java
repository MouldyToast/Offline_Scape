...
import org.jesse.game.world.entity.npc.impl.wilderness.ChaosFanatic;
...
public class WildernessArea extends PolygonRegionArea implements
    ...
    private static final RSPolygon edgeIsland = new RSPolygon(3387, 3639, 3463, 3710);
    public static final RSPolygon edgeIslandBank = new RSPolygon(3416, 3678, 3423, 3689);
    ...
    private static boolean rawWithinWilderness(final int x, final int y) {
        return !forexEnclavePolygon.contains(x, y) && !edgeIslandBank.contains(x, y)
                && (mainlandWildernessPolygon.contains(x, y)
                || edgeIsland.contains(x, y)
    ...
    protected RSPolygon[] polygons() {
        return new RSPolygon[]{mainlandWildernessPolygon, edgeIsland, dungeonsWildernessPolygon, minorWildernessBossesDungeons};
    }
    ...
    public void leave(final Player player, boolean logout) {
        ...
        player.getVarManager().sendBit(11028, 0);
        if (player.isXPDropsMultiplied() && player.isXPDropsWildyOnly()) {
    ...
    @Override
    public boolean attack(Player attacker, Entity entity, PlayerCombat combat) {
        if (entity instanceof Player defender) {
            if (defender.inArea(FeroxEnclaveDangerArea.class) && defender.getVariables().getTime(TickVariable.TELEBLOCK) <= 0 && attacker.getVariables().getTime(TickVariable.TELEBLOCK) <= 0) {
                attacker.sendMessage("You cannot fight another player whilst next to the Enclave, please move further out.");
                return false;
            }
            boolean attackerIsBlackSkull = PlayerAttributesKt.getBlackSkulled(attacker);
            boolean defenderIsBlackSkull = PlayerAttributesKt.getBlackSkulled(defender);
            if (!attacker.getVariables().isSkulled() && attacker.getVarManager().getBitValue(SettingVariables.PK_SKULL_PREVENTION_VARBIT_ID) == 1 && canSkull(attacker, defender)) {
                attacker.sendMessage("You cannot attack this target as it would result in you getting skulled.");
                return false;
            }
            if (attacker.isCanPvp() && !defender.isCanPvp()) {
                attacker.sendMessage("That player is not in the wilderness.");
                return false;
            }
            final int level = getWildernessLevel(attacker.getLocation()).orElse(-1);
            final int otherLevel = getWildernessLevel(entity.getLocation()).orElse(-1);
            final int minimumLevel = Math.min(level, otherLevel);
            if (minimumLevel >= 1) {
                if(defenderIsBlackSkull && !attackerIsBlackSkull) {
                    attacker.sendMessage(Colour.RED.wrap("The black skull on your target spreads to you upon attack."));
                    PlayerAttributesKt.setBlackSkulled(attacker, true);
                    attacker.getVariables().setSkull(true);
                }
                if (defenderIsBlackSkull)
                    return true;
                if (attackerIsBlackSkull && attacker.getAttackedBy() == defender)
                    return true;
                if (Math.abs(attacker.getSkills().getCombatLevel() - defender.getSkills().getCombatLevel()) > minimumLevel) {
                    attacker.sendMessage("The difference between your Combat level and the Combat level of "
                            + defender.getPlayerInformation().getDisplayname() + " is too great.");
                    attacker.sendMessage((defender.getAppearance().isMale() ? "He" : "She") + " needs to move deeper into" +
