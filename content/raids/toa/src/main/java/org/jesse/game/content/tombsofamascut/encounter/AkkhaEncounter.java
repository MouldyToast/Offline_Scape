...
import org.jesse.game.content.tombsofamascut.npc.*;
...
public class AkkhaEncounter extends TOARaidArea implements PartialMovementPlugin, CycleProcessPlugin {
	...
	private Akkha akkha;
	private final List<TrailOrb> trailOrbList = new ArrayList<>();
	...
	private final List<UnstableOrb> unstableOrbs = new ArrayList<>();
	...
	@Override public void enter(Player player) {
		runEntryScripts(player);
		super.enter(player);
	}

	static void runEntryScripts(Player player) {
		...
		if (WorldThread.getCurrentCycle() > time + 30) {
			...
			WorldTasksManager.schedule(addRunningTask(new WorldTask() {
				...
				@Override public void run() {
					...
					for (Player p : getChallengePlayers()) {
						if (p == null) continue;
						if (p.getUsername().equals(player.getUsername())) continue;
						if (!p.getLocation().withinDistance(player.getLocation(), 1)) continue;
						addBlastEffect(p);
					}
					if (cycles++ >= 4) {
	...
	record OrbPath(Location location, Direction direction) {}
}