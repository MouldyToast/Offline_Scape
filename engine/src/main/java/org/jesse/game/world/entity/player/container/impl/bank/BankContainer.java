...
import org.jesse.game.world.entity.player.container.ContainerState;
...
import static org.jesse.game.world.entity.player.container.impl.bank.Bank.unbankableItems;
import static org.jesse.game.world.entity.player.privilege.GameMode.ULTIMATE_IRON_MAN;
...
public final class BankContainer extends Container {
	...
	public void deposit(final Player player, final Container container, final int slotId, int amount) {
		...
		// If the player's GameMode is UIM, then only allow pets to be deposited
		if (player != null && player.getGameMode() == ULTIMATE_IRON_MAN) {
			var skillPet = SkillingPet.getByItem(item.getId());
			var bosspet = BossPet.getByItem(item.getId());
			var miscPet = MiscPet.getByItem(item.getId());
			// If the item we're depositing is not a pet or any kind; STOP
			if (skillPet == null && bosspet == null && miscPet == null) {
				player.sendMessage("You can ONLY deposit pets as a UIM player.");
				return;
			}
		}
	...
	public void withdraw(final Player player, final Container container, int slot, int amount, final boolean placeholder) {
		...
		final boolean stackable = definitions.isStackable() || (!item.hasAttributes() && bank.getSetting(BankSetting.WITHDRAW_MODE) == 1 && definitions.getNotedId() != -1 && ItemDefinitions.get(definitions.getNotedId()).isStackable());
