...
public final class PlayerVariables {
    ...
    private static final Logger log = NearRealityLogger.getLogger(PlayerVariables.class);
    ...
    public void schedule(final int duration, final TickVariable variable) {
        schedule(duration, variable, false);
        player.sendDeveloperMessage("PlayerVariables | Scheduling PlayerVariable " + variable.name() + " for duration: " + duration);
    }

    public void schedule(final int duration, final TickVariable variable, boolean override) {
        final Variable existing = scheduled.get(variable.toString());
        if (existing != null) {
            if (override) {
                existing.ticks = duration;
                return;
            } else {
    ...
    public void onLogin() {
        if (getTime(TickVariable.STAMINA_ENHANCEMENT) > 0) {
            ...
            player.getVarManager().sendBit(24, getTime(TickVariable.STAMINA_ENHANCEMENT));
        }
    }
    ...
    public void synchNoticeboardVars() {
        ...
        varManager.sendVar(3800, (int) TimeUnit.NANOSECONDS.toSeconds(System.nanoTime() - Main.getServerStartTime()));
    ...
    public int getTrollheimTeleports() {
        return trollheimTeleports;
    }

    public void setTrollheimTeleports(int trollheimTeleports) {
        this.trollheimTeleports = trollheimTeleports;
    }

    public int getMorUikRekTeleports() {
        return morUikRekTeleports;
    }

    public void setMorUikRekTeleports(int morUikRekTeleports) {
        this.morUikRekTeleports = morUikRekTeleports;
    }
    ...
    public boolean hasTeleblockImmunity() {
        return getTime(TickVariable.TELEBLOCK_IMMUNITY) > 0;
    }
