...
import static org.jesse.game.world.entity.player.VarManager.appendPersistentVarp;
...
public class SettingsInterface extends Interface {
    ...
    public static final int MAX_HIT_HITSPLATS_THRESHOLD_VARBIT_ID = 14195;
    ...
    static {
        ...
        appendPersistentVarbit(SIDE_PANELS_VARBIT_ID);
    }
    ...
    protected void build() {
        ...
        bind("Show less/more information", player -> player.getVarManager().flipBit(SHOW_LESS_OR_MORE_INFORMATION_VARBIT));
        bind("Search", player -> {
    ...
    public void open(Player player) {
        ...
        player.getPacketDispatcher().sendComponentSettings(getInterface().getId(), getComponent("Setting"), 0, Settings.ALL_SETTINGS.size(), AccessMask.CLICK_OP1);
        player.getPacketDispatcher().sendComponentSettings(getInterface().getId(), getComponent("Slider"), 0, Settings.SLIDER_NOTCH_COUNT_SUM, AccessMask.CLICK_OP1);
        player.getPacketDispatcher().sendComponentSettings(getInterface().getId(), getComponent("Category"), 0, Settings.CATEGORIES.size(), AccessMask.CLICK_OP1);
        player.getPacketDispatcher().sendComponentSettings(getInterface().getId(), getComponent("Drop-down menu"), 0, Settings.DROPDOWN_ENTRIES_MAX_COUNT * 3, AccessMask.CLICK_OP1);
    ...
    private void toggle(Player player, Setting setting) {
        switch (setting.getStructId()) {
            case SettingStructs.SHOW_DATA_ORBS_STRUCT_ID:
                ...
                break;
            case SettingStructs.AMMO_PICKING_BEHAVIOUR_STRUCT_ID:
                var autoEquipping = player.getVarManager().getBitValue(AMMO_PICKING_BEHAVIOUR_VARBIT_ID) == 0;
                player.sendMessage("Auto-equipping ammo is " + (autoEquipping ? "enabled." : "disabled."));
                break;
            case SettingStructs.RUNE_PICKING_BEHAVIOUR_STRUCT_ID:
                var enabled = player.getVarManager().getBitValue(RUNE_PICKING_BEHAVIOUR_VARBIT_ID) == 0;
                player.sendMessage("Auto-equipping runes is " + (enabled ? "enabled." : "disabled."));
                break;
            case SettingStructs.SHIFT_CLICK_TO_DROP_ITEMS_STRUCT_ID:
                var dropping = player.getVarManager().getBitValue(SHIFT_CLICK_TO_DROP_ITEMS_VARBIT_ID) == 0;
                player.sendMessage("Shift dropping items is " + (dropping ? "enabled." : "disabled."));
                break;
            case SettingStructs.LOOT_DROP_NOTIFICATIONS_STRUCT_ID:
                if (player.getVarManager().getBitValue(MINIMUM_LOOT_ITEM_VALUE_VARBIT_ID) == 0) {
                    final Setting otherSetting = Settings.findSettingByStructId(SettingStructs.MINIMUM_ITEM_VALUE_NEEDED_FOR_LOOT_NOTIFICATION_STRUCT_ID);
            ...
            case SettingStructs.DROP_ITEM_WARNING_STRUCT_ID:
                if (player.getVarManager().getBitValue(MINIMUM_DROP_ITEM_VALUE_VARBIT_ID) == 0) {
                    final Setting otherSetting = Settings.findSettingByStructId(SettingStructs.MINIMUM_ITEM_VALUE_NEEDED_FOR_DROP_ITEM_WARNING_STRUCT_ID);
                    input(player, otherSetting);
                    return;
                }
                break;
            case SettingStructs.COLLECTION_LOG_NEW_ADDITION_NOTIFICATION_STRUCT_ID: {
                ...
                player.getVarManager().sendBitInstant(COLLECTION_LOG_NEW_ADDITIONS_VARBIT_ID, (currentValue & ~0x1) | newValue);
                return;
            }
            case SettingStructs.COLLECTION_LOG_NEW_ADDITION_POPUP_STRUCT_ID: {
                final int currentValue = player.getVarManager().getBitValue(COLLECTION_LOG_NEW_ADDITIONS_VARBIT_ID);
                final int newValue = (currentValue + 2) & 0x2;
                player.getVarManager().sendBitInstant(COLLECTION_LOG_NEW_ADDITIONS_VARBIT_ID, (currentValue & ~0x2) | newValue);
                return;
            }
            case SettingStructs.MOVE_FOLLOWER_OPTIONS_LOWER_DOWN_STRUCT_ID: {
                var priortise = player.getVarManager().getBitValue(MOVE_FOLLOWER_OPTIONS_LOWER_DOWN_VARBIT_ID) == 0;
                player.sendMessage("Follower options de-priortising is " + (priortise ? "enabled." : "disabled."));
            }
        }
        ...
        switch (setting.getStructId()) {
            case SettingStructs.MINIMUM_ITEM_VALUE_NEEDED_FOR_LOOT_NOTIFICATION_STRUCT_ID:
                player.sendInputInt("Set threshold value:", value -> {
                    ...
                    player.getVarManager().sendBitInstant(SettingVariables.LOOT_DROP_NOTIFICATIONS_VARBIT_ID, result == 0 ? 0 : 1);
            ...
            case SettingStructs.MINIMUM_ITEM_VALUE_NEEDED_FOR_DROP_ITEM_WARNING_STRUCT_ID:
                player.sendInputInt("Set threshold value:", value -> {
                    final int result = Math.min(value, 500_000_000);
                    player.getVarManager().sendBitInstant(SettingVariables.DROP_ITEM_WARNING_VARBIT_ID, result == 0 ? 0 : 1);
    ...
    private void colour(Player player, Setting setting) {
        ...
        player.getInterfaceHandler().sendInterface(GameInterface.COLOUR_PICKER.getId(), InterfacePosition.COLOUR_PICKER.getResizableComponent(), PaneType.ADVANCED_SETTINGS, false);
        player.getPacketDispatcher().sendClientScript(4185, (getInterface().getId() << 16) | InterfacePosition.COLOUR_PICKER.getResizableComponent(), (currentColour == 0 ? defaultColour : currentColour) - 1);
        player.awaitInputInt(value -> {
            player.getInterfaceHandler().closeInterfaceSpecific(InterfacePosition.COLOUR_PICKER.getResizableComponent(), PaneType.ADVANCED_SETTINGS);
    ...
    private void promptDefaultKeybinds(Player player) {
        ...
        player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                options("Are you sure you want to reset your keybinds?", new DialogueOption("Yes.", () -> {
