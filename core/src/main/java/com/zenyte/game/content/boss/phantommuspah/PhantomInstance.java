 ...
 * @author Savions.
 */
public class PhantomInstance extends DynamicArea implements CycleProcessPlugin, CannonRestrictionPlugin, GravestoneLocationPlugin, PartialMovementPlugin, TeleportMovementPlugin, HitProcessPlugin, LootBroadcastPlugin, DeathPlugin {

	private static final Logger log = NearRealityLogger.getLogger(PhantomInstance.class);
	...
	public boolean sendDeath(Player player, Entity source) {
		var stats = ScoreboardModule.INSTANCE.getPhantomMuspahStatistics();
		var currentDeaths = stats.getGlobalDeathCount();
		stats.setGlobalDeathCount(currentDeaths + 1);
		player.incrementNumericAttribute("deaths_to_phantom_muspah", 1);
		return false;
	}
}
