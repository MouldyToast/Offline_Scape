...
public enum RepairableItem {
    ...
    DIZANAS_QUIVER(270_000, ItemId.DIZANAS_QUIVER, ItemId.DIZANAS_QUIVER_BROKEN),
    BLESSED_DIZANAS_QUIVER(270_000, ItemId.BLESSED_DIZANAS_QUIVER, ItemId.BLESSED_DIZANAS_QUIVER_BROKEN),
    DIZANAS_MAX_CAPE(270_000, ItemId.DIZANAS_MAX_CAPE, ItemId.DIZANAS_MAX_CAPE_BROKEN),
    ;
    ...
    public float getCost(Player player, Item item, boolean armourstand) {
        if (ordinal() <= VERACS_PLATESKIRT.ordinal()) {
            if (armourstand) {
                var levelDiscount = player.getSkills().getLevel(SkillConstants.SMITHING) / 200.0F;
                var baseCost = (1.0F - levelDiscount) * getRepairCost();
                var armourCharges = item.getCharges() / 90000.0F;
                return (getRepairCost() - baseCost * armourCharges) * getMemberDiscount(player);
            }
            else {
                return (getRepairCost() - (getRepairCost() * (item.getCharges() / 90000.0F))) * getMemberDiscount(player);
            }
        } else {
    ...
    private float getMemberDiscount(Player player) {
        // base discount
        var memberDiscount = 1.0F;

        switch(player.getMemberRank()) {
            case TOPAZ, SAPPHIRE -> memberDiscount = 0.9F;
            case EMERALD -> memberDiscount = 0.8F;
            case RUBY, DIAMOND -> memberDiscount = 0.7F;
            case DRAGONSTONE -> memberDiscount = 0.65F;
            case ONYX -> memberDiscount = 0.6F;
            case ZENYTE -> memberDiscount = 0.55F;
            case ENCHANTED, GOLD, ETERNAL, NEBULA, CATALYTIC -> memberDiscount = 0.5F;
        }
