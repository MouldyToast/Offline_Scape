...
import org.jesse.api.service.store.notify
import org.jesse.api.util.usernameSanitizer
...
/**
 * Handles the submission and revocation of [sanctions][org.jesse.api.model.Sanction].
 *
 * @author Stan van der Bend
 */
object SanctionPlayerHandler {

    private val logger = NearRealityLogger.getLogger(SanctionPlayerHandler::class.java)
    ...
    fun submitSanction(
        ...
        fun checkAffectedPlayers(predicate: (Player) -> Boolean, onSuccess: () -> Unit) {
            ...
            if (affectedPlayers.size > 1) {
                ...
                onSuccess()
            affectedPlayers.forEach { it.logout(true) }
        }
        ...
        when (level) {
            ...
            SanctionLevel.MAC -> {
                val hardwareHash = offender.playerInformation.hardware.getHardwareHash()
                checkAffectedPlayers({ it.playerInformation.hardware.getHardwareHash() == hardwareHash }) {
                    val sanction = HardwareHashSanction(hardwareHash, offender.name, null, time, type, reporter?.name, reason, duration)
                    submitSanctionToAPI(reporter, sanction, offender, type, level, offenderIp, reason, duration, time)
                }
    ...
    internal fun onSanctionReceived(sanction: Sanction) {
        ...
        when(sanction) {
            ...
            is HardwareHashSanction -> applyAffectedPlayers { it.playerInformation.hardware.hashCode() == sanction.hardwareHash }
        }
    }
