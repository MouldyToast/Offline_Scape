...
public class BossPetNPC extends NPCPlugin {
    ...
    public void handle() {
        bind("Metamorphosis", (player, npc) -> {
            ...
            if (getTheatreOfBloodPets().contains(npc.getId())) {
                if (!PlayerAttributesKt.getTobPetMetamorphUnlocked(player)) {
                    player.sendMessage("You must unlock the pet metamorphosis before you can use it.");
                    return;
                }
            }
            if (npc.getId() == NpcId.LITTLE_NIGHTMARE_9399 &&
                !PlayerAttributesKt.getNightmareMetamorphosisUnlocked(player)
            ) {
                player.sendMessage("You must unlock the pet metamorphosis before you can use it.");
                return;
            }
    ...
    public List<Integer> getTheatreOfBloodPets() {
        return List.of(
            NpcId.LIL_ZIK_8337,
            NpcId.LIL_MAIDEN_10870,
            NpcId.LIL_BLOAT_10871,
            NpcId.LIL_NYLO_10872,
            NpcId.LIL_SOT_10873,
