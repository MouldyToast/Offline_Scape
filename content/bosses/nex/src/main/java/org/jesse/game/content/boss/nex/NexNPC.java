...
public class NexNPC extends NPC implements CombatScript, Spawnable {
	...
	private void handleSoulSplit(Player entity, int damage) {
		...
		if (damage <= 0 || soulSplitCharges <= 0) {
			return;
		}

		int ticks = soulsplitProjectile.build(entity, this);
		int heal = (int) (getHitpoints() * 0.025);
		scheduleHit(this, new Hit(heal, HitType.HEALED), ticks);
		soulSplitCharges--;
	}
	...
	private int soulSplitCharges;

	public void resetSoulSplitCharges() {
		this.soulSplitCharges = 3;
	}

	private void zarosAuto(Player victim) {
		setNextAttack(getAttackSpeed());
		resetSoulSplitCharges();
			...
			forEachPlayerInPrison(player -> {
				...
				WorldTasksManager.schedule(() -> player.scheduleHit(this, magic(player, 40).onLand(hit -> {
					...
					if (player.getShield() != null
							&& player.getShield().getId() == ItemId.SPECTRAL_SPIRIT_SHIELD) {
						prayerDrain /= 2;
					}
	...
	protected void onDeath(Entity source) {
		try {
			setTimeOfDeath(WorldThread.getCurrentCycle());
			resetWalkSteps();
			combat.removeTarget();
			setAnimation(null);
