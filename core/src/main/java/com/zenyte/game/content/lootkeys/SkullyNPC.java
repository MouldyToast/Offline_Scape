...
public class SkullyNPC extends NPCPlugin {
    ...
    @Override
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(talkToOptions(player, npc)));
        bind("Value", (player, npc) -> player.getDialogueManager().start(changeThreshold(player, npc)));
        bind("Settings", (player, npc) -> player.getDialogueManager().start(changeLootkeysOptions(player, npc)));
    }
        ...
        return new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                options("Select an Option",
                    ...
                    "Who are you, exactly?",
                    "Goodbye.")
                    .onOptionOne(() -> player.getDialogueManager().start(chestWorksDialogue(player, npc)))
                    .onOptionTwo(() -> player.getDialogueManager().start(hasLootKeyEnabled ? changeLootkeysOptions(player, npc) : accessToChest(player, npc)))
                    .onOptionThree(() -> player.getDialogueManager().start(claimedLoot(player, npc)))
                    .onOptionFour(() -> player.getDialogueManager().start(whoAreYou(player, npc)))
                    .onOptionFive(() -> player.getDialogueManager().start(bye(player, npc)));
            }
        };
    }
            ...
            public void buildDialogue() {
                ...
                options("Select an Option",
                    ...
                    .onOptionOne(() -> {
                        ...
                        if (enabled) player.getDialogueManager().start(turnOffLootkeys(player, npc));
                        else player.getDialogueManager().start(turnOnLootkeys(player, npc));
                    })
                    .onOptionTwo(() -> {
                        settings.setDropFood(!dropToFloor);
                        if (!dropToFloor) player.getDialogueManager().start(new Dialogue(player, npc) {
                            @Override
                            public void buildDialogue() {
                        ...
                        else player.getDialogueManager().start(new Dialogue(player, npc) {
                            @Override
                            public void buildDialogue() {
                    ...
                    .onOptionThree(() -> {
                        ...
                        if (!dropValuablesToFloor) player.getDialogueManager().start(new Dialogue(player, npc) {
                            @Override
                            public void buildDialogue() {
    ...
    private Dialogue sendLootKeysNotEnabled(Player player, NPC npc) {
        return new Dialogue(player, npc) {
            @Override
            public void buildDialogue() {
                npc("You need to talk to me before doing that.");
            }
    ...
    private Dialogue claimedLoot(Player player, NPC npc) {
        return new Dialogue(player, npc) {
            @Override
            public void buildDialogue() {
        ...
        return new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                options("Select an Option",
                    ...
                    "How much?!")
                    .onOptionOne(() -> player.getDialogueManager().start(firstPrompt(player, npc)))
                    .onOptionTwo(() -> player.getDialogueManager().start(notRightNow(player, npc)))
                    .onOptionThree(() -> {
                        player("How much?!");
                        npc("5 million coins.");
                    });
        ...
        return new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                options("Pay 5 million coins to unlock Loot Keys?",
                    ...
                    .onOptionTwo(() -> player.getDialogueManager().start(new Dialogue(player, npc) {
                        @Override
                        public void buildDialogue() {
