...
import org.jesse.plugins.item.AbyssalTentacleOr;
...
public final class ChargesManager {
    ...
    private static final int[] SLOTS = new int[]{EquipmentSlot.HELMET.getSlot(), EquipmentSlot.CAPE.getSlot(), EquipmentSlot.PLATE.getSlot(), EquipmentSlot.AMULET.getSlot(), EquipmentSlot.LEGS.getSlot(), EquipmentSlot.WEAPON.getSlot(), EquipmentSlot.SHIELD.getSlot()};
    ...
    public void removeCharges(final DegradeType type) {
        ...
        for (int i = SLOTS.length - 1; i >= 0; i--) {
            ...
            player.sendDeveloperMessage("Your %s has lost a charge. Only %d more to go."
                .formatted(item.getName(), item.getCharges()));
