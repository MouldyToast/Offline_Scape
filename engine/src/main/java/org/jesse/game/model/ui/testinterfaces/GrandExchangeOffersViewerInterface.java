...
import static org.jesse.game.content.grandexchange.GrandExchange.INVENTORY_INTERFACE;
import static org.jesse.game.content.grandexchange.GrandExchange.OFFER_TIMEOUT_DELAY;
...
public class GrandExchangeOffersViewerInterface extends Interface {
    ...
    public static final void search(final Player player, final int itemId, final ExchangeType exchangeType, SortType sortType) {
        ...
        if (size > 0) {
            ...
            for (int index = 0; index < size; index++) {
                ...
                cs2$sendItem(player, index, itemId, result.getItem().getAmount() - result.getAmount(), (int) filled, result.getAmount(), result.getItem().getAmount(), StringFormatUtil.format(price), StringFormatUtil.format(totalPrice), StringFormatUtil.formatString(result.getUsername()));
            }
            cs2$sendRowCount(player, size);
        } else {
    ...
    public static void searchAll(final Player player, final ExchangeType exchangeType) {
        final ArrayList<ExchangeOffer> results = new ArrayList<ExchangeOffer>();
        player.getTemporaryAttributes().put("ge_offers_selected_exchangetype", exchangeType);
        final long lowestAcceptableTime = System.currentTimeMillis() - OFFER_TIMEOUT_DELAY;
        for (final Map.Entry<String, Int2ObjectOpenHashMap<ExchangeOffer>> group : GrandExchangeHandler.getOffers().entrySet()) {
        ...
        final int size = results.size();
        if (size > 0) {
            player.getPacketDispatcher().sendClientScript(10803, "Viewing recent offers for: <col=00FFFF>" + exchangeType.name().toLowerCase());
            player.getPacketDispatcher().sendClientScript(10805, exchangeType.ordinal());
            for (int index = 0; index < size; index++) {
                final ExchangeOffer result = results.get(index);
                final int price = result.getPrice();
                final int totalPrice = result.getPrice() * result.getItem().getAmount();
                final double filled = 110.0 / result.getItem().getAmount() * result.getAmount();
                cs2$sendItem(player, index, result.getItem().getId(), result.getItem().getAmount() - result.getAmount(), (int) filled, result.getAmount(), result.getItem().getAmount(), StringFormatUtil.format(price), StringFormatUtil.format(totalPrice), StringFormatUtil.formatString(result.getUsername()));
            }
            cs2$sendRowCount(player, size);
        } else {
            player.getPacketDispatcher().sendClientScript(10804, "No results were found with your search.");
    ...
    private static void cs2$sendItem(Player player, int index, int item, int availableQuantity, int filledPercentage, int resultAmount, int resultItemAmount, String price, String totalPrice, String username) {
        player.getPacketDispatcher().sendClientScript(10801, index, item, availableQuantity, filledPercentage, resultAmount, resultItemAmount, price, totalPrice, username);
    }

    private static void cs2$sendRowCount(Player player, int count) {
        player.getPacketDispatcher().sendClientScript(10802, count);
    }
