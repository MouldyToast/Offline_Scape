...
public enum Reanimation {
	...
	public static final Int2ObjectOpenHashMap<Reanimation> itemToReanimation = new Int2ObjectOpenHashMap<>();

	public static Optional<Reanimation> forItemId(final int itemId) {
		if(itemToReanimation.containsKey(itemId)) {
			return Optional.of(itemToReanimation.get(itemId));
		} else {
			Optional<Reanimation> opt = Arrays.stream(values()).filter(it -> Arrays.stream(it.itemId).allMatch(iid -> iid == itemId)).findFirst();
            opt.ifPresent(reanimation -> itemToReanimation.put(itemId, reanimation));
			return opt;
		}
	}
