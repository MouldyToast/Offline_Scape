...
import org.jesse.game.content.pvm_arena.PvmArenaManager.state
...
import org.jesse.game.content.pvm_arena.player.revive.PvmArenaRevivePlayerActionPlugin
import org.jesse.game.content.pvm_arena.player.revive.PvmArenaReviveState
import org.jesse.game.content.pvm_arena.player.updatePvmArenaVarbits
import org.jesse.game.content.pvm_arena.wave.PvmArenaWaveController
import org.jesse.game.content.pvm_arena.wave.pvmArenaWaveConfig
import org.jesse.game.util.inWholeTicks
import org.jesse.game.world.entity.player.pvmArenaAppearanceBeardOffset
import org.jesse.game.world.entity.player.pvmArenaMvpCountDuringGame
import org.jesse.game.world.entity.player.pvmArenaPoints
import org.jesse.game.world.entity.player.pvmArenaPointsGainedDuringGame
...
import org.jesse.game.util.Colour.RS_RED
...
internal object PvmArenaManager {
    ...
    internal fun changeState(newState: PvmArenaState) {
        ...
        when (newState) {
            is PvmArenaState.Idle -> {
                ...
                scheduleStateHandlerTask(tickCyclePeriod = 0) {
            ...
            is PvmArenaState.Ended -> {
                ...
                when(newState) {
                    is PvmArenaState.Ended.TimedOut -> {
                        val blueKills = PvmArenaTeam.Blue.killCount
                        val redKills = PvmArenaTeam.Red.killCount
                        val winner = if (blueKills > redKills) PvmArenaTeam.Blue else PvmArenaTeam.Red

                        logger.info("The PvM Arena activity has timed out, neither team finished in time.")
                        forEachPlayerInTeam { player -> messagesGameStatisticsToPlayer(player) }
                        logger.info("The PvM Arena activity was won by team $winner.")
                        updateWinningTeam(winner)
    ...
    private fun updateWinningTeam(winningTeam: PvmArenaTeam) {
        forEachPlayerInTeam { player ->
            messagesGameStatisticsToPlayer(player)
...
private fun messagesGameStatisticsToPlayer(player: Player) {
    player.sendMessage("You gained ${RS_RED.wrap(player.pvmArenaPointsGainedDuringGame)} PvM Arena points this game, you now have a total of ${RS_RED.wrap(player.pvmArenaPoints)} points.")
    player.sendMessage("You were the most valuable player ${RS_RED.wrap(player.pvmArenaMvpCountDuringGame)} times this game.")
}
