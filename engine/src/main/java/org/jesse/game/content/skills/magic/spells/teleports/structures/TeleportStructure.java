...
import org.jesse.game.content.donation.HomeTeleport;
...
import org.jesse.game.world.entity.InvalidLocationException;
...
import static org.jesse.utils.TimeUnit.SECONDS;
...
public interface TeleportStructure {
	...
	default void end(final Player player, final Teleport teleport) {
		...
		player.getBuildAreaManager().getChunksInScope().clear();
...
//		updateDiaries(player, teleport);
	...
	/**
	 * Gets a random location within the {@link Teleport#getRandomizationDistance()} radius to the {@link Teleport#destination()}
	 * location.
	 *
	 * @param player
	 *            the player who is teleporting.
	...
	default Location getRandomizedLocation(final Player player, final Teleport teleport) {
		...
		Location destination = teleport.destination();

		if(destination != null && teleport.destination().equals(3087, 3490, 0))
