package org.jesse.game.content.wilderness.revenant.npc

import org.jesse.game.content.wilderness.revenant.npc.RevenantMaledictus.Companion.onRevenantDeath
import org.jesse.game.content.wilderness.revenant.npc.drop.GoodRevenantDrop
import org.jesse.game.content.wilderness.revenant.npc.drop.MediocreReventantDrop
...
import org.jesse.game.world.entity.player.action.combat.magic.spelleffect.BindEffect
    ...
    Spawnable
{
    ...
    override fun isAcceptableTarget(entity: Entity): Boolean {
        return entity !is Player || entity.equipment.getItem(EquipmentSlot.HANDS)?.id != org.jesse.game.item.ids.BRACELET_OF_ETHEREUM
    }
    ...
    override fun drop(tile: Location) {
        try {
            ...
            if (killer.getNumericAttribute("ethereum absorption").toInt() == 1) {
                ...
                if (bracelet == org.jesse.game.item.ids.BRACELET_OF_ETHEREUM || bracelet == BRACELET_OF_ETHEREUM_UNCHARGED) {
                    ...
                    if (bracelet == BRACELET_OF_ETHEREUM_UNCHARGED) {
                        braceletItem.id = org.jesse.game.item.ids.BRACELET_OF_ETHEREUM
                        killer.equipment.refresh(EquipmentSlot.HANDS.slot)
                    }
                }
            }
            if(amount > 0)
                dropItem(killer, Item(REVENANT_ETHER, amount))
            ...
            when {
                ...
                a < (chanceB + 1) -> dropItem(killer, MediocreReventantDrop.get(), tile, false)
                    ...
                    dropItem(killer, Item(COINS_995, coinRewardDrop), tile, true)
                }
            }
        } catch (e: Exception) {
    ...
    companion object {
        ...
        private val BRACELET_OF_ETHEREUM = Item(org.jesse.game.item.ids.BRACELET_OF_ETHEREUM)
