...
import org.jesse.game.content.chambersofxeric.MountQuidamortemArea;
...
public class RaidingPartyInterface extends Interface {
    ...
    protected void attach() {
        ...
        put(3, 8, "Scaling");
        put(3, 9, "Toggle challenge mode");
        put(3, 10, "Toggle large raid");
    }
    ...
    protected void build() {
        bind("Scaling", this::requestScaling);
    ...
    /**
     * Requests a scaling value from the player and updates the raid party's scale accordingly.
     * Validates the input value to ensure it is between 1 and 100. If validation fails,
     * the player receives an appropriate message. If the player is not part of a recruiting
     * raid party, a message is sent indicating that scaling cannot be applied.
     *
     * @param player the player requesting the scaling adjustment.
     */
    private void requestScaling(Player player) {
        if (verifyConditions(player)) return;
        player.sendInputInt("Set a preferred party scale (or 0 to clear it):", value -> {
            final RaidParty party = player.getSettings().getChannel().getRaidParty();
            if (party.getRaid().getMap() != null) {
                player.sendMessage("Your party has already entered the chambers, you cannot edit the settings now.");
                return;
            }
            party.getRaid().setScale(Math.min(100, value));
            GameInterface.RAIDING_PARTY.open(player);
            party.refresh(player);
        });
    }
