...
import org.jesse.game.content.tombsofamascut.npc.Baba;
import org.jesse.game.content.tombsofamascut.npc.RollingBoulder;
import org.jesse.game.content.tombsofamascut.npc.TOANPC;
import org.jesse.game.content.tombsofamascut.raid.*;
...
public class BabaEncounter extends TOARaidArea implements CycleProcessPlugin, PartialMovementPlugin {
		...
		for (int i = 0; i < rubbleNpcs.length; i++) {
			if (rubbleNpcs[i] == null || rubbleNpcs[i].isDying() || rubbleNpcs[i].isFinished()) {
                ...
                for (Location possibleLocation : possibleLocations) {
                    final Location location = getLocation(possibleLocation);
	...
	public void resetAndKillRockFall() {
		rockFallingLocations.clear();
		rubbleFallingTicks = 0;
        for (RubbleNpc rubbleNpc : rubbleNpcs) {
            if (rubbleNpc != null && !rubbleNpc.isDead() && !rubbleNpc.isFinished()) {
                rubbleNpc.applyHit(new Hit(rubbleNpc, rubbleNpc.getMaxHitpoints(), HitType.DEFAULT));
            }
        }
	}
	...
	public void landRocks(Player[] players) {
		if(baba.isPerformingBoulderSpecial())
			return;
		final int baseDamage = baba.getMaxHit(24);
		playerLoop : for (Player p : players) {
	...
	public boolean spawnBaboons() {
		...
		if (availableBaboonIndices.isEmpty()) {
			return false;
		}
	...
	public void checkRollingBoulderCollision(Location location) {
		for (Player p : getChallengePlayers()) {
			if (!p.isLocked() && !p.isDying() && !p.getAppearance().isTransformedIntoNpc()
					&& p.getX() >= location.getX() && p.getX() <= location.getX() + 2 && (p.getY() >= location.getY() && p.getY() <= location.getY() + 2)) {
	...
	public static class SarcoPhagusNpc extends BoulderTankNpc {
