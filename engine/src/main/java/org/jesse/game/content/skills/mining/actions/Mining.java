...
import org.jesse.game.model.item.leagues.raging_echo.EchoPickaxe;
...
import org.jesse.game.content.skills.mining.MiningDefinitions.ShapeDefinitions;
import org.jesse.game.content.stars.ShootingStar;
...
import org.jesse.plugins.itemonobject.PotionOnCastlewarsRocks;
...
public class Mining extends Action {
    ...
    public boolean start() {
        ...
        } if (ore.isAmalgamation()) {
            player.sendFilteredMessage("You swing your pick at the amalgamation.");
        } else {
    ...
    public int processWithDelay() {
        ...
        if (ore == OreDefinitions.VOLCANIC_ASH) {
            ...
            var ore = new Item(this.ore.getOre(), amount);
        ...
        if (ore == OreDefinitions.GEM) {
            skills.addXp(SkillConstants.MINING, ore.getXp());
            Item gem = new Item(randomGem());
            if (tool.equals(PickaxeDefinitions.ECHO_PICKAXE)) {
                EchoPickaxe.Companion.processEchoPickaxe(player, gem.getId());
                return -1;
            }
        ...
        if (ore == OreDefinitions.SALAX_SALT) {
            ...
            if (player.getInventory().getAmountOf(ItemId.SALAX_SALT) > 60) {
                player.sendMessage("I should use some of this before mining any more.");
                return -1;
            }
        }

        if (ore == OreDefinitions.SMALL_AMALGAMATION ||
                ore == OreDefinitions.MEDIUM_AMALGAMATION ||
                ore == OreDefinitions.LARGE_AMALGAMATION)
            amount = Utils.random(1, 3);

        final int body = player.getEquipment().getId(EquipmentSlot.PLATE);
        if (body >= 13104 && body <= 13107) {
            final OreDefinitions limit = body == 13104 ? OreDefinitions.COAL : body == 13105 ? OreDefinitions.MITHRIL : body == 13106 ? OreDefinitions.ADAMANTITE : OreDefinitions.AMETHYST;
        ...
        var depleteRate = ore.getDepletionRate();
        var depleted = Utils.random(Math.max(0, depleteRate - 1)) == 0;
        boolean deplete = depleteRate > 0 && depleted;
        if (deplete && npc == null) {
        ...
        if (ore.getIncinerationExperience() > 0 && pickaxe.getCharges() > 0 && tool.equals(PickaxeDefinitions.INFERNAL) && Utils.random(2) == 0) {
        ...
        else if (tool.equals(PickaxeDefinitions.ECHO_PICKAXE)) {
            if (exp > 0)
            ...
            if (ore.equals(OreDefinitions.CLAY)) {
                ...
                if (bracelet != null && bracelet.getId() == ItemId.BRACELET_OF_CLAY) {
                    ...
                    var product = new Item(ore.getOre(), amount);
                    if (new Random().nextDouble() < player.getMemberRank().getNotedResourcePercentChance())
                        product = product.toNote();
                    inventory.addItem(product).onFailure(remainder -> World.spawnFloorItem(remainder, player));
                    handleCelestialRing();
                ...
                if (ore.getOre() != -1) {
                    if (ore == OreDefinitions.PAYDIRT) {
                        ...
                        if(ore.isShootingStar() && rock instanceof ShootingStar star) {
                            double xp = getShootingStarXp(skills.getLevelForXp(SkillConstants.MINING), star.getLevel().getNumeric());
                            skills.addXp(SkillConstants.MINING, xp);
                        } else {
                            if (exp > 0)
                                skills.addXp(SkillConstants.MINING, exp);
                        }

                        var product = new Item(ore.equals(OreDefinitions.ESSENCE) ? essence : ore.getOre(), amount);
                        if (new Random().nextDouble() < player.getMemberRank().getNotedResourcePercentChance())
                            product = product.toNote();
                        inventory.addItem(product).onFailure(remainder -> World.spawnFloorItem(remainder, player));
                        handleCelestialRing();
    ...
    public static int getShootingStarXp(int level, int starTier) {
        if (level < 1 || level > 99) return 0;

        if (starTier < 1) starTier = 1;
        else if (starTier > 9) starTier = 9;

        double lv = level;
        double baseXp = (0.0009 * lv * lv * lv) - (0.0615 * lv * lv) + (1.719 * lv) - 4.84;

        double multiplier = 0.5 + 0.0625 * (starTier - 1);

        return (int) Math.max(0, baseXp * multiplier);
    }
