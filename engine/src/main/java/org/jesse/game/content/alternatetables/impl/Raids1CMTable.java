...
import org.jesse.game.content.chambersofxeric.rewards.ChallengeRaidNormalReward;
import org.jesse.game.content.chambersofxeric.rewards.RaidRareReward;
...
public class Raids1CMTable implements AlternateTableDropProvider {
    ...
    private void buildEntries() {
        ...
        for(RaidRareReward reward: RaidRareReward.values()) {
            entries.add(new OtherDropViewerEntry(reward.getItem().getId(), 1,1, reward.getWeight(), RaidRareReward.TOTAL_WEIGHT * 3, "This item's rate is based on a 100,000 point raid completion and is awarded to a party member based on contribution."));
        }
