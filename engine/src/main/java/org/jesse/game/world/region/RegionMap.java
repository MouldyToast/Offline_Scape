...
public class RegionMap {
    ...
    private static final Logger log
            = LoggerFactory.getLogger(RegionMap.class);

    private final int[] masks;

    private final byte[] tileHeights;

    private final byte[] mapSettings;

    private final short[] overlayIds;
    private final short[] underlayIds;
    ...
    RegionMap(final int regionId) {
        ...
        tileHeights = new byte[4 * 64 * 64];

        mapSettings = new byte[4 * 64 * 64];

        overlayIds = new short[4 * 64 * 64];
        underlayIds = new short[4 * 64 * 64];
    }

    public static int getMaskIndex(final int plane, final int localX, final int localY) {
    ...
    synchronized int getLocalMask(final int plane, final int x, final int y) {
        final int index = getMaskIndex(plane, x, y);
        return masks[index];
    }

    synchronized byte getLocalTileHeight(final int plane, final int x, final int y) {
        final int index = getMaskIndex(plane, x, y);
        return tileHeights[index];
    }

    synchronized byte getLocalMapSetting(final int plane, final int x, final int y) {
        final int index = getMaskIndex(plane, x, y);
        return mapSettings[index];
    }

    synchronized short getLocalOverlayId(final int plane, final int x, final int y) {
        final int index = getMaskIndex(plane, x, y);
        return overlayIds[index];
    }

    synchronized short getLocalUnderlayId(final int plane, final int x, final int y) {
        final int index = getMaskIndex(plane, x, y);
        return underlayIds[index];
    }
    ...
    public void setTileHeight(
        ...
        final int index = getMaskIndex(plane, localX, localY);
        tileHeights[index] = height;
    }

    public void setOverlayId(
            final int plane,
            final int localX, final int localY,
            final short overlayId
    ) {
        if (localX >= 64 || localY >= 64 || localX < 0 || localY < 0) {
            final int x = getRegionX() + localX;
            final int y = getRegionY() + localY;
        ...
        final int index = getMaskIndex(plane, localX, localY);
        overlayIds[index] = overlayId;
    }

    public void setUnderlayId(
            final int plane,
            final int localX, final int localY,
            final short underlayId
    ) {
        if (localX >= 64 || localY >= 64 || localX < 0 || localY < 0) {
            final int x = getRegionX() + localX;
            final int y = getRegionY() + localY;
